...
public class QueryInfo {
  ...
  /**
   * Constructs a {@link QueryInfo} object for the given query or multi-query. The parsed result of
   * this query allows to determine the location of the placeholders, the query static parts and
   * whether this query can be rewritten as a multi-values clause query.
   *
   ...
   * @param encoding the characters encoding to use when extracting the query static parts as byte
   *     arrays.
   */
  public QueryInfo(String sql, Session session, String encoding) {
    if (sql == null) {
      throw ExceptionFactory.createException(
          ...
          Messages.getString("QueryInfo.NullSql"),
    ...
    boolean dontCheckOnDuplicateKeyUpdateInSQL =
        session
            .getPropertySet()
            .getBooleanProperty(PropertyKey.dontCheckOnDuplicateKeyUpdateInSQL)
            .getValue();
    ...
    if (isReplace) {
      strInspector.incrementPosition(
          REPLACE_STATEMENT.length()); // Advance to the end of "REPLACE".
    }

    // Check if the statement has potential to be rewritten as a multi-values clause statement,
    // i.e., if it is an INSERT or REPLACE statement and
    // 'rewriteBatchedStatements' is enabled.
    boolean rewritableAsMultiValues = (isInsert || isReplace) && rewriteBatchedStatements;

    // Check if should look for ON DUPLICATE KEY UPDATE CLAUSE, i.e., if it is an INSERT statement
    // and `dontCheckOnDuplicateKeyUpdateInSQL` is disabled.
    ...
    while (strInspector.indexOfNextChar() != -1) {
      if (strInspector.getChar() == '?') {
        ...
        strInspector.incrementPosition();
        generalEndpointStart =
            strInspector.getPosition(); // Next section starts after the placeholder.
        ...
        if (rewritableAsMultiValues) {
          ...
          } else if (valuesClauseEnd
              != -1) { // There's a placeholder after the end of the VALUES clause.
            rewritableAsMultiValues = false;
        ...
        if (strInspector.indexOfNextNonWsChar() != -1) {
          ...
          if (dontCheckOnDuplicateKeyUpdateInSQL || this.containsOnDuplicateKeyUpdate) {
            ...
            if (isInsert) {
              strInspector.incrementPosition(
                  INSERT_STATEMENT.length()); // Advance to the end of "INSERT".
            }
        ...
        if (valuesClauseBegin == -1
            && strInspector.matchesIgnoreCase(VALUE_CLAUSE) != -1) { // VALUE(S) clause found.
          strInspector.incrementPosition(VALUE_CLAUSE.length()); // Advance to the end of "VALUE".
          if (strInspector.matchesIgnoreCase("S")
              != -1) { // Check for the "S" in "VALUE(S)" and advance 1 more character if needed.
            strInspector.incrementPosition();
          }
          ...
          // later.
            ...
            && parensLevel == 0
            && isInsert
            && strInspector.matchesIgnoreCase(AS_CLAUSE) != -1) { // End of VALUES clause.
          ...
          if (rewritableAsMultiValues) {
            ...
            && parensLevel == 0
            && isInsert
            && (matchEnd = strInspector.matchesIgnoreCase(ODKU_CLAUSE))
                != -1) { // End of VALUES clause.
          if (valuesClauseEnd == -1) {
            valuesClauseEnd = strInspector.getPosition();
          }
          withinValuesClause = false;
          ...
          strInspector.incrementPosition(
              matchEnd
                  - strInspector.getPosition()); // Advance to the end of "ON DUPLICATE KEY UPDATE".
        ...
        } else if (rewritableAsMultiValues
            && valuesClauseBegin != -1
            && strInspector.matchesIgnoreCase(LAST_INSERT_ID_FUNC) != -1) {
          rewritableAsMultiValues = false;
          strInspector.incrementPosition(
              LAST_INSERT_ID_FUNC.length()); // Advance to the end of "LAST_INSERT_ID".
    ...
    if (rewritableAsMultiValues) {
      ...
      if (valuesClauseBegin != -1) {
        this.valuesClauseLength =
            (valuesClauseEnd != -1 ? valuesClauseEnd : this.queryLength) - valuesClauseBegin;
      } else {
        rewritableAsMultiValues = false;
      }
    } else {
  ...
  /**
   * Constructs a {@link QueryInfo} object with a multi-value clause for the specified batch count,
   * that stems from the specified baseQueryInfo.
   *
   * @param baseQueryInfo the {@link QueryInfo} instance that provides the query static parts used
   *     to create the new instance now augmented to accommodate the extra number of parameters
   * @param batchCount the number of batches, i.e., the number of times the VALUES clause needs to
   *     be repeated inside the new query
   */
  private QueryInfo(QueryInfo baseQueryInfo, int batchCount) {
    ...
    if (this.numberOfPlaceholders == 0) {
      ...
      int length = end - begin;
      byte[] valuesSegment =
          StringUtils.getBytes(this.baseQueryInfo.sql, begin, length, this.encoding);
      byte[] bindingSegment = StringUtils.getBytes(",", this.encoding);

      // First batch is in the query.
      ByteBuffer queryByteBuffer =
          ByteBuffer.allocate(
              this.baseQueryInfo.queryLength + (length + bindingSegment.length) * (batchCount - 1));

      // Head section: from the start to the end of the values - includes first values batch.
      queryByteBuffer.put(
          StringUtils.getBytes(
              this.baseQueryInfo.sql, 0, this.baseQueryInfo.valuesEndpoints.get(1), this.encoding));
      ...
      int begin =
          this.baseQueryInfo.valuesEndpoints.get(this.baseQueryInfo.valuesEndpoints.size() - 2);
      int end =
          this.baseQueryInfo.valuesEndpoints.get(this.baseQueryInfo.valuesEndpoints.size() - 1);
      int length = end - begin;
      byte[] valuesEndSegment =
          StringUtils.getBytes(this.baseQueryInfo.sql, begin, length, this.encoding);
      ...
      length = end - begin;
      byte[] valuesBeginSegment =
          StringUtils.getBytes(this.baseQueryInfo.sql, begin, length, this.encoding);
      ByteBuffer bindingSegmentByteBuffer =
          ByteBuffer.allocate(
      ...
      this.staticSqlParts[this.staticSqlParts.length - 1] =
          this.baseQueryInfo.staticSqlParts[this.baseQueryInfo.staticSqlParts.length - 1];
    }
  }

  /**
   * Returns the number of queries identified in the original SQL string. Different queries are
   * identified by the presence of the query delimiter character, i.e., a semicolon.
   *
  ...
  /**
   * Returns the first character of the statement from the query used to build this {@link
   * QueryInfo}.
   *
  ...
  /**
   * If this object represents a query that is re-writable as a multi-values statement and if
   * rewriting batched statements is enabled, then returns the length of the parsed VALUES clause
   * section, including the placeholder characters themselves, otherwise returns -1.
   *
   * @return the length of the parsed VALUES clause section, including the placeholder characters
   *     themselves, otherwise returns -1
   */
  ...
  /**
   * Does this query info represent a query that contains an ON DUPLICATE KEY UPDATE clause? This
   * operation does not take into consideration the multiplicity of queries in the original SQL.
   *
   * <p>Checking whether the original query contains an ON DUPLICATE KEY UPDATE clause is
   * conditional to how the connection properties `dontCheckOnDuplicateKeyUpdateInSQL` and
   * `rewriteBatchedStatements` are set, with `rewriteBatchedStatements=true` implicitly disabling
   * `dontCheckOnDuplicateKeyUpdateInSQL`.
   *
   * @return <code>true</code> if the query or any of the original queries contain an ON DUPLICATE
   *     KEY UPDATE clause.
   */
  ...
  /**
   * Returns the static sections of the parsed query, as byte arrays, split by the places where the
   * placeholders were located.
   *
   * @return the static sections of the parsed query, as byte arrays, split by the places where the
   *     placeholders were located
   */
  ...
  /**
   * Returns a {@link QueryInfo} for a multi-values INSERT/REPLACE assembled for the specified batch
   * count, without re-parsing.
   *
  ...
  public String getSqlForBatch() {
    ...
    int size =
        this.baseQueryInfo.queryLength
            + (this.batchCount - 1) * this.baseQueryInfo.valuesClauseLength
            + this.batchCount
            - 1;
  ...
  /**
   * Finds and returns the position of the first non-whitespace character from the specified SQL,
   * skipping comments and quoted text.
   *
  ...
  /**
   * Finds and returns the first non-whitespace character from the specified SQL, skipping comments
   * and quoted text.
   *
   * @param sql the query to search
   * @param noBackslashEscapes whether backslash escapes are disabled or not
  ...
  /**
   * Checks whether the given query is safe to run in a read-only session. In case of doubt it is
   * assumed to be safe. This operation does not take into consideration the multiplicity of queries
  ...
  public static boolean isReadOnlySafeQuery(String sql, boolean noBackslashEscapes) {
    ...
    if (firstStatementChar == 'A'
        && StringUtils.startsWithIgnoreCaseAndWs(sql, "ALTER", statementKeywordPos)) {
      return false;
      ...
      return false;
    } else if (firstStatementChar == 'G'
        && StringUtils.startsWithIgnoreCaseAndWs(sql, "GRANT", statementKeywordPos)) {
      return false;
      ...
      return false;
    } else if (firstStatementChar == 'L'
        && StringUtils.startsWithIgnoreCaseAndWs(sql, "LOAD", statementKeywordPos)) {
      return false;
      ...
      return false;
        ...
        && StringUtils.startsWithIgnoreCaseAndWs(sql, "TRUNCATE", statementKeywordPos)) {
      return false;
      ...
      return context == null
          || !context.equalsIgnoreCase("DELETE") && !context.equalsIgnoreCase("UPDATE");
    }
    return true; // Assume it's safe by default.
  }

  /**
   * Returns the type of return that can be expected from executing the given query. This operation
   * does not take into consideration the multiplicity of queries in the specified SQL.
   *
   * @param sql the query to check
   * @param noBackslashEscapes whether backslash escapes are disabled or not
   * @return the return type that can be expected from the given query, one of the elements of
   *     {@link QueryReturnType}.
   */
    ...
    if (firstStatementChar == 'A'
      ...
      return QueryReturnType.MAY_PRODUCE_RESULT_SET;
        ...
        && StringUtils.startsWithIgnoreCaseAndWs(
            sql, "CHECK", statementKeywordPos)) { // Also matches "CHECKSUM".
      return QueryReturnType.PRODUCES_RESULT_SET;
    } else if (firstStatementChar == 'D'
        && StringUtils.startsWithIgnoreCaseAndWs(
            sql, "DESC", statementKeywordPos)) { // Also matches "DESCRIBE".
      return QueryReturnType.PRODUCES_RESULT_SET;
      ...
      if (context == null) {
        return QueryReturnType.MAY_PRODUCE_RESULT_SET;
      } else if (context.equalsIgnoreCase("SELECT")
          || context.equalsIgnoreCase("TABLE")
          || context.equalsIgnoreCase("VALUES")) {
        return QueryReturnType.PRODUCES_RESULT_SET;
      } else {
  ...
  /**
   * Returns the context of the WITH statement. The context can be: SELECT, TABLE, VALUES, UPDATE or
   * DELETE. This operation does not take into consideration the multiplicity of queries in the
   * specified SQL.
   *
   * @param sql the query to search
   * @param noBackslashEscapes whether backslash escapes are disabled or not
  ...
  private static String getContextForWithStatement(String sql, boolean noBackslashEscapes) {
    ...
    String commentsFreeSql =
        StringUtils.stripCommentsAndHints(
            sql, OPENING_MARKERS, CLOSING_MARKERS, !noBackslashEscapes);
  ...
  /**
   * Checks whether the specified SQL contains or not an ON DUPLICATE KEY UPDATE clause. This
   * operation does not take into consideration the multiplicity of queries in the specified SQL.
   *
   * @param sql the query to search
   * @param noBackslashEscapes whether backslash escapes are disabled or not.
   * @return <code>true</code> if the query contains an ON DUPLICATE KEY UPDATE clause, <code>false
   *     </code> otherwise
   */
