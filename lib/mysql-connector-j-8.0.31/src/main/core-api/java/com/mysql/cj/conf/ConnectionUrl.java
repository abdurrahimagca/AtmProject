...
public abstract class ConnectionUrl implements DatabaseUrlContainer {
  ...
  public enum Type {
    ...
    SINGLE_CONNECTION(
        "jdbc:mysql:",
        HostsCardinality.SINGLE,
        "com.mysql.cj.conf.url.SingleConnectionUrl",
        PropertyKey.dnsSrv,
        FAILOVER_DNS_SRV_CONNECTION), //
    FAILOVER_CONNECTION(
        "jdbc:mysql:",
        HostsCardinality.MULTIPLE,
    ...
    private Type(
        String scheme,
        HostsCardinality cardinality,
        String implementingClass,
        PropertyKey dnsSrvPropertyKey,
        Type alternateDnsSrvType) {
      this.scheme = scheme;
     ...
     * Otherwise throws an {@link UnsupportedConnectionStringException}. Calling this method with
     * the argument n lower than 0 skips the hosts cardinality validation.
     *
    ...
    /**
     * Instantiates a class that implements the right type of connection URLs for the given {@link
     * ConnectionUrlParser}.
     *
     ...
     */
    public static ConnectionUrl getConnectionUrlInstance(
        ConnectionUrlParser parser, Properties info) {
      ...
      if (dnsSrvPropKey != null && type.getAlternateDnsSrvType() != null) {
        if (info != null
            && info.containsKey(
                dnsSrvPropKey
                    .getKeyName())) { // Properties map prevails over connection string options.
          if ((Boolean)
              PropertyDefinitions.getPropertyDefinition(dnsSrvPropKey)
                  .parseObject(info.getProperty(dnsSrvPropKey.getKeyName()), null)) {
            type = fromValue(type.getAlternateDnsSrvType().getScheme(), hostsCount);
          }
        } else if ((parsedProperties = parser.getProperties())
                .containsKey(dnsSrvPropKey.getKeyName())
            && (Boolean)
                PropertyDefinitions.getPropertyDefinition(dnsSrvPropKey)
                    .parseObject(parsedProperties.get(dnsSrvPropKey.getKeyName()), null)) {
          type = fromValue(type.getAlternateDnsSrvType().getScheme(), hostsCount);
        }
      }
  ...
  /**
   * Static factory method that returns either a new instance of a {@link ConnectionUrl} or a cached
   * one. Returns "null" it can't handle the connection string.
   *
   * @param connString the connection string
   * @param info the connection arguments map
   * @return an instance of a {@link ConnectionUrl} or "null" if isn't able to handle the connection
   *     string
   */
  ...
  protected void collectProperties(ConnectionUrlParser connStrParser, Properties info) {
    ...
    connStrParser.getProperties().entrySet().stream()
        .forEach(e -> this.properties.put(PropertyKey.normalizeCase(e.getKey()), e.getValue()));

    // Properties passed in override the ones from the connection string.
    if (info != null) {
      info.stringPropertyNames().stream()
          .forEach(k -> this.properties.put(PropertyKey.normalizeCase(k), info.getProperty(k)));
    }
  ...
  protected void setupPropertiesTransformer() {
    String propertiesTransformClassName =
        this.properties.get(PropertyKey.propertiesTransform.getKeyName());
    if (!isNullOrEmpty(propertiesTransformClassName)) {
      try {
        ...
        throw ExceptionFactory.createException(
            ...
            Messages.getString(
                "ConnectionString.9", new Object[] {propertiesTransformClassName, e.toString()}),
            e);
      }
    }
  }
  ...
  /**
   * Returns a map containing the properties read from the given configuration files. Multiple files
   * can be referenced using a comma as separator.
   *
  ...
  public static Properties getPropertiesFromConfigFiles(String configFiles) {
    ...
    for (String configFile : configFiles.split(",")) {
      try (InputStream configAsStream =
          ConnectionUrl.class.getResourceAsStream(
              "/com/mysql/cj/configurations/" + configFile + ".properties")) {
        ...
        throw ExceptionFactory.createException(
            InvalidConnectionAttributeException.class,
            Messages.getString("ConnectionString.11", new Object[] {configFile}),
            e);
      }
    }
    return configProps;
  ...
  /**
   * Some acceptable property values have changed in c/J 8.0 but old values remain hard-coded in
   * widely used software. So, old values must be accepted and translated to new ones.
   *
   ...
   * properties transformations to the collected properties if {@link ConnectionPropertiesTransform}
   * was declared in the connection arguments.
   *
  ...
  protected HostInfo fixHostInfo(HostInfo hi) {
    ...
    hi.getHostProperties().entrySet().stream()
        .forEach(e -> hostProps.put(PropertyKey.normalizeCase(e.getKey()), e.getValue()));
    ...
    if (port == HostInfo.NO_PORT && !isNullOrEmpty(portAsString)) {
      try {
        ...
        throw ExceptionFactory.createException(
            ...
            Messages.getString(
                "ConnectionString.7", new Object[] {hostProps.get(PropertyKey.PORT.getKeyName())}),
            e);
      }
    }
  ...
  /**
   * Subclasses should override this to perform any required pre-processing on the host information
   * properties.
   *
  ...
  /**
   * Returns the default user. Usually the one provided in the method {@link
   * DriverManager#getConnection(String, String, String)} or as connection argument.
   *
  ...
  /**
   * Returns the default password. Usually the one provided in the method {@link
   * DriverManager#getConnection(String, String, String)} or as connection argument.
   *
  ...
  protected void fixProtocolDependencies(Map<String, String> hostProps) {
    ...
    if (!isNullOrEmpty(protocol) && protocol.equalsIgnoreCase("PIPE")) {
      if (!hostProps.containsKey(PropertyKey.socketFactory.getKeyName())) {
        hostProps.put(
            PropertyKey.socketFactory.getKeyName(), "com.mysql.cj.protocol.NamedPipeSocketFactory");
      }
    }
  }
  ...
  public String getDatabase() {
    return this.properties.containsKey(PropertyKey.DBNAME.getKeyName())
        ? this.properties.get(PropertyKey.DBNAME.getKeyName())
   ...
   *
   * <p>By default returns all hosts. Subclasses should override this method in order to implement
   * support for different views, usually by splitting the global hosts into smaller sub-lists.
   *
  ...
  public HostInfo getHostOrSpawnIsolated(String hostPortPair, List<HostInfo> hostsList) {
    ...
    ConnectionUrlParser.Pair<String, Integer> hostAndPort =
        ConnectionUrlParser.parseHostPortPair(hostPortPair);
   ...
   */
  protected HostInfo buildHostInfo(
      String host, int port, String user, String password, Map<String, String> hostProps) {
    ...
    if (this.propertiesTransformer != null) {
      ...
      try {
        ...
        throw ExceptionFactory.createException(
            ...
            Messages.getString(
                "ConnectionString.8",
                new Object[] {
                  PropertyKey.PORT.getKeyName(),
                  transformedProps.getProperty(PropertyKey.PORT.getKeyName())
                }),
            e);
      }
      ...
      transformedProps.stringPropertyNames().stream()
  ...
  public Properties getConnectionArgumentsAsProperties() {
    ...
    return this.propertiesTransformer != null
        ? this.propertiesTransformer.transformProperties(props)
        : props;
  }
  ...
  public List<HostInfo> getHostsListFromDnsSrv(HostInfo srvHost) {
    ...
    try {
      ...
      throw ExceptionFactory.createException(
          Messages.getString("ConnectionString.26", new Object[] {srvServiceName}), e);
    }
    if (srvRecords == null || srvRecords.size() == 0) {
      throw ExceptionFactory.createException(
          Messages.getString("ConnectionString.26", new Object[] {srvServiceName}));
    }
        ...
        .map(
            s ->
                buildHostInfo(
                    ...
                    baseHostInfo.getHostProperties()))
  ...
  public String toString() {
    ...
    asStr.append(
        String.format(
            " :: {type: \"%s\", hosts: %s, database: \"%s\", properties: %s, propertiesTransformer:"
                + " %s}",
            this.type,
            this.hosts,
            this.originalDatabase,
