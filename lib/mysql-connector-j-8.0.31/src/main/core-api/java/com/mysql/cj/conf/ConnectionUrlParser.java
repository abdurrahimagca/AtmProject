 ...
 * Instead of using a URI instance to ensure the correct syntax of the connection string, this
 * implementation uses regular expressions which is faster but also less strict in terms of
 * validations. This actually works better because database URLs don't exactly stick to the RFC 3986
 * rules.
 *
 * <p><i>scheme://authority/path?query#fragment</i>
 *
 * <p>This results in splitting the connection string URL and processing its internal parts:
 *
 * <dl>
 *   <dt>scheme
 *   <dd>The protocol and subprotocol identification. Usually "jdbc:mysql:" or "mysqlx:".
 *   <dt>authority
 *   <dd>Contains information about the user credentials and/or the host and port information.
 *       Unlike its definition in the RFC 3986 specification, there can be multiple authority
 *       sections separated by a single comma (,) in a connection string. It is also possible to use
 *       an alternative syntax for the user and/or host identification, that also allows setting per
 *       host connection properties, in the form of
 *       "[user[:password]@]address=(key1=value)[(key2=value)]...[,address=(key3=value)[(key4=value)]...]...".
 *   <dt>path
 *   <dd>Corresponds to the database identification.
 *   <dt>query
 *   <dd>The connection properties, written as
 *       "propertyName1[=[propertyValue1]][&amp;propertyName2[=[propertyValue2]]]..."
 *   <dt>fragment
 *   <dd>The fragment section is ignored in Connector/J connection strings.
 * </dl>
 */
public class ConnectionUrlParser implements DatabaseUrlContainer {
  ...
  private ConnectionUrlParser(String connString) {
    if (connString == null) {
      throw ExceptionFactory.createException(
          WrongArgumentException.class, Messages.getString("ConnectionString.0"));
    }
  ...
  /**
   * Checks if the scheme part of given connection string matches one of the {@link Type}s supported
   * by Connector/J. Throws {@link WrongArgumentException} if connString is null.
   *
  ...
  private void parseConnectionString() {
    ...
    if (!matcher.matches()) {
      throw ExceptionFactory.createException(
          WrongArgumentException.class, Messages.getString("ConnectionString.1"));
    }
   ...
   * The placeholder _host_, can be a host name, IPv4 or IPv6. This parser doesn't check IP syntax.
   * IPv6 addresses are enclosed by square brackets ([::1]). The placeholder _any_of_the_above_?_
   * can be any of the above except for the user information part (_user_:_password_@). When the
   * symbol ":" is not used, it means an null/empty password or a default (HostInfo.NO_PORT) port,
   * respectively. When the symbol "@" is not used, it means that the authority part doesn't contain
   * user information (depending on the scheme type can still be provided via key=value pairs).
   *
  ...
  private void parseAuthoritySegment(String authSegment) {
    ...
    throw ExceptionFactory.createException(
        ...
        Messages.getString("ConnectionString.2", new Object[] {authSegment}));
  }
   ...
   * @return the {@link HostInfo} instance containing the parsed information or <code>null</code> if
   *     the host part is not empty
   */
  ...
  /**
   * Parses the host information resorting to a URI object. This process handles most single-host
   ...
   * @param hostInfo the string containing the host information part
   * @return the {@link HostInfo} instance containing the parsed information or <code>null</code> if
   *     unable to parse the host information
   */
  private HostInfo buildHostInfoResortingToUriParser(
      String user, String password, String hostInfo) {
    String host = null;
   ...
   * @param hostInfo the string containing the host information part
   * @return a list with all {@link HostInfo} instances containing the parsed information or <code>
   *     null</code> if unable to parse the host information
   */
  private List<HostInfo> buildHostInfoResortingToSubHostsListParser(
      String user, String password, String hostInfo) {
      ...
      for (String h : hostsList) {
        ...
        if ((hi = buildHostInfoForEmptyHost(user, password, h)) != null) {
          hostInfoList.add(hi);
            ...
            || (maybeIPv6
                && (hi = buildHostInfoResortingToUriParser(user, password, "[" + h + "]"))
                    != null)) {
          hostInfoList.add(hi);
        } else if ((hi = buildHostInfoResortingToKeyValueSyntaxParser(user, password, h)) != null) {
          hostInfoList.add(hi);
        } else if ((hi = buildHostInfoResortingToAddressEqualsSyntaxParser(user, password, h))
            != null) {
          hostInfoList.add(hi);
   ...
   *     unable to parse the host information
   */
  private HostInfo buildHostInfoResortingToKeyValueSyntaxParser(
      String user, String password, String hostInfo) {
    if (!hostInfo.startsWith(KEY_VALUE_HOST_INFO_OPENING_MARKER)
        || !hostInfo.endsWith(KEY_VALUE_HOST_INFO_CLOSING_MARKER)) {
      // This pattern won't work.
      return null;
    }
    hostInfo =
        hostInfo.substring(
            KEY_VALUE_HOST_INFO_OPENING_MARKER.length(),
            hostInfo.length() - KEY_VALUE_HOST_INFO_CLOSING_MARKER.length());
    return new HostInfo(
        this,
        null,
        HostInfo.NO_PORT,
        user,
        password,
        processKeyValuePattern(KEY_VALUE_HOST_PTRN, hostInfo));
  }
   ...
   *     unable to parse the host information
   */
  private HostInfo buildHostInfoResortingToAddressEqualsSyntaxParser(
      String user, String password, String hostInfo) {
    ...
    return new HostInfo(
        ...
        user,
        password,
        processKeyValuePattern(ADDRESS_EQUALS_HOST_PTRN, hostInfo));
  }
   ...
   *     unable to parse the host information
   */
  private HostInfo buildHostInfoResortingToGenericSyntaxParser(
      String user, String password, String hostInfo) {
   ...
   *
   * @param authSegment the string containing the authority segment, i.e., the user and host
   *     information parts
   * @return a {@link Pair} containing the user information in the left side and the host
   ...
   * @return a {@link Pair} containing the user and password information or null if the user
   *     information can't be parsed
   */
   ...
   * @return a {@link Pair} containing the host and port information or null if the host information
   *     can't be parsed
   */
    ...
    if (matcher.matches()) {
      ...
      if (!isNullOrEmpty(portAsString)) {
        try {
          ...
          throw ExceptionFactory.createException(
              WrongArgumentException.class,
              Messages.getString("ConnectionString.3", new Object[] {hostInfo}),
              e);
        }
      }
  ...
  /**
   * Parses the connection properties section and stores the extracted key/value pairs into a local
   * map.
   */
  ...
  /**
   * Takes a two-matching-groups (respectively named "key" and "value") pattern which is
   * successively tested against the given string and produces a key/value map with the matched
   * values. The given pattern must ensure that there are no leftovers between successive tests,
   * i.e., the end of the previous match must coincide with the beginning of the next.
   *
  ...
  public String toString() {
    ...
    asStr.append(
        String.format(
            " :: {scheme: \"%s\", authority: \"%s\", path: \"%s\", query: \"%s\", parsedHosts: %s,"
                + " parsedProperties: %s}",
            this.scheme,
            this.authority,
            this.path,
