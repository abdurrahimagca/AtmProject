...
public class HostInfo implements DatabaseUrlContainer {
  ...
  /**
   * Constructs a {@link HostInfo} instance initialized with the provided host, port, user, password
  ...
  public Properties exposeAsProperties() {
    ...
    this.hostProperties.entrySet().stream()
        .forEach(e -> props.setProperty(e.getKey(), e.getValue() == null ? "" : e.getValue()));
   ...
   * @return <code>true</code> if both objects have equal host and port pairs, <code>false</code>
   *     otherwise.
   */
  public boolean equalHostPortPair(HostInfo hi) {
    return (getHost() != null && getHost().equals(hi.getHost())
            || getHost() == null && hi.getHost() == null)
        && getPort() == hi.getPort();
  }
  ...
  public String toString() {
    ...
    asStr.append(
        String.format(
            ...
            this.host, this.port, this.hostProperties));
