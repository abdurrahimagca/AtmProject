...
public class ExceptionFactory {
  ...
  public static CJException createException(
      String message, Throwable cause, ExceptionInterceptor interceptor) {
   ...
   */
  public static <T extends CJException> T createException(
      Class<T> clazz, String message, Throwable cause, ExceptionInterceptor interceptor) {
  ...
  public static CJCommunicationsException createCommunicationsException(
      ...
      Throwable cause,
      ExceptionInterceptor interceptor) {
    CJCommunicationsException sqlEx =
        createException(CJCommunicationsException.class, null, cause, interceptor);
  ...
  public static String createLinkFailureMessageBasedOnHeuristics(
    ...
    long lastPacketReceivedTimeMs =
        packetReceivedTimeHolder == null
            ? 0L
            : packetReceivedTimeHolder.getLastPacketReceivedTime();
    ...
    if (propertySet != null) {
      isInteractiveClient =
          propertySet.getBooleanProperty(PropertyKey.interactiveClient).getValue();
      ...
      if (serverSession != null) {
        serverTimeoutSecondsStr =
            isInteractiveClient
                ? serverSession.getServerVariable("interactive_timeout")
    ...
    if (serverTimeoutSeconds != 0) {
      if (timeSinceLastPacketSeconds > serverTimeoutSeconds) {
        ...
        timeoutMessageBuf.append(
            Messages.getString(
                isInteractiveClient ? "CommunicationsException.4" : "CommunicationsException.3"));
      }
    ...
    if (dueToTimeout == DUE_TO_TIMEOUT_TRUE || dueToTimeout == DUE_TO_TIMEOUT_MAYBE) {
      ...
      //
      // Attempt to determine the reason for the underlying exception (we can only make a best-guess
      // here)
      //
      if (underlyingException instanceof BindException) {
        String localSocketAddress =
            propertySet.getStringProperty(PropertyKey.localSocketAddress).getValue();
        exceptionMessageBuf.append(
            localSocketAddress != null && !Util.interfaceExists(localSocketAddress)
    ...
    if (exceptionMessageBuf.length() == 0) {
      ...
      if (propertySet.getBooleanProperty(PropertyKey.maintainTimeStats).getValue()
          && !propertySet.getBooleanProperty(PropertyKey.paranoid).getValue()) {
        exceptionMessageBuf.append("\n\n");
        exceptionMessageBuf.append(
            lastPacketReceivedTimeMs != 0
                ? Messages.getString(
