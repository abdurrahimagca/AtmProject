...
public interface ResultsetRows extends RowList, ProtocolEntity {
   ...
   *
   * <p>This means that move back and move forward won't work because we do not hold on to the
   * records.
   *
  ...
  /**
   * Sometimes the driver doesn't have metadata until after the statement has the result set in-hand
   * (because it's cached), so it can call this to set it after the fact.
   *
