...
public interface ServerSessionStateController {
   ...
   *
   * <p>Please note that the driver could issue some queries internally. With that there is no
   * guarantee that all session changes are reflected in the {@link ServerSessionStateChanges}
   * object after the recent user's query. If this is an issue, a {@link
   * SessionStateChangesListener} can be added via {@link
   * #addSessionStateChangesListener(SessionStateChangesListener)} to catch all session changes.
   *
  ...
  /**
   * Add the {@link SessionStateChangesListener} that will process {@link ServerSessionStateChanges}
   * on it's arrival.
   *
   ...
   *
   * <p>Driver is getting these changes when connection property trackSessionState=true and server
   ...
   *
   * <p>The server messages about session changes are parsed according to their known types:
   *
   * <ul>
   *   <li>{@link #SESSION_TRACK_SYSTEM_VARIABLES} - two values, the system variable name and it's
   *       new value;
   *   <li>{@link #SESSION_TRACK_SCHEMA} - single value, the new schema name;
   *   <li>{@link #SESSION_TRACK_STATE_CHANGE} - single value, "1" or "0";
   *   <li>{@link #SESSION_TRACK_GTIDS} - single value, list of GTIDs as reported by server;
   *   <li>{@link #SESSION_TRACK_TRANSACTION_CHARACTERISTICS} - single value, transaction
   *       characteristics statement;
   *   <li>{@link #SESSION_TRACK_TRANSACTION_STATE} - single value, transaction state record.
   * </ul>
   *
   * For the unknown change type the raw payload is written into the single value.
   *
   * <p>See more details in the <a
   * href="https://dev.mysql.com/doc/refman/8.0/en/session-state-tracking.html">server
   * documentation</a>.
   */
