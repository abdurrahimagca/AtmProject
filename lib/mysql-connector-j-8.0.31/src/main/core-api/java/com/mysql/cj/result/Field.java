...
public class Field implements ProtocolEntity {
  ...
  public Field(
    ...
    // ucs2, utf16, and utf32 cannot be used as a client character set, but if it was received from
    // server under some circumstances we can parse them as utf16
  ...
  /**
   * Used by prepared statements to re-use result set data conversion methods when generating bound
   * parameter retrieval instance for statement interceptors.
   *
  ...
  public Field(
    ...
    switch (mysqlType) {
      ...
      case JSON:
        ...
        // ucs2, utf16, and utf32 cannot be used as a client character set, but if it was received
        // from server under some circumstances we can parse them as utf16
        this.encoding = "UnicodeBig".equals(encoding) ? "UTF-16" : encoding;
  ...
  public String getFullName() {
    if (this.fullName == null) {
      StringBuilder fullNameBuf =
          new StringBuilder(this.tableName.length() + 1 + this.columnName.length());
