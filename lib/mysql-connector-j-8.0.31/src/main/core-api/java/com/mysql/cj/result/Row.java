...
public interface Row extends ProtocolEntity {
   ...
   * Row constructor or {@link #setMetadata(ColumnDefinition)} call before calling this method to
   * allow correct columnIndex boundaries check and data type recognition.
   *
  ...
  default byte[] getBytes(int columnIndex) {
    // TODO check that "if the value is SQL NULL, the value returned is null" is correctly
    // implemented
    throw ExceptionFactory.createException(
        CJOperationNotSupportedException.class,
        Messages.getString("OperationNotSupportedException.0"));
