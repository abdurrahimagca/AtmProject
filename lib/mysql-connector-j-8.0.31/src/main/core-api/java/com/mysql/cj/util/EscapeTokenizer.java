...
public class EscapeTokenizer {
    ...
    for (; this.pos < this.sourceLength; this.pos++) {
      ...
      if (!this.inQuotes && !backslashEscape) {
        ...
        if (c == CHR_COMMENT) {
          ...
          if ((this.pos + 1 < this.sourceLength)
              && (this.source.charAt(this.pos + 1) == CHR_COMMENT)) {
  ...
  /**
   * Returns true if a variable reference was found. Note that this information isn't accurate until
   * finishing to process all tokens from source String. It also can't be used as per token basis.
   *
