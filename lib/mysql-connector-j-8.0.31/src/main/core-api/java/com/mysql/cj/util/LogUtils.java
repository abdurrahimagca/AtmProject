...
public class LogUtils {
  ...
  public static String findCallingClassAndMethod(Throwable t) {
    ...
    int endInternalMethods =
        Math.max(
            Math.max(
                stackTraceAsString.lastIndexOf("com.mysql.cj"),
                stackTraceAsString.lastIndexOf("com.mysql.cj.core")),
    ...
    if (endInternalMethods != -1) {
      ...
      if (endOfLine != -1) {
        int nextEndOfLine =
            stackTraceAsString.indexOf(LINE_SEPARATOR, endOfLine + LINE_SEPARATOR_LENGTH);
