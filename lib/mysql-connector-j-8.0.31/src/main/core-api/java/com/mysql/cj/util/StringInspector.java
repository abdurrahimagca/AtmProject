 ...
 *
 * <p>Provides string searching and manipulation operations such as finding sub-strings, matching
 * sub-strings or building a comments-free version of a string.
 *
 * <p>This object keeps internal state that affect the operations, e.g., executing an indexOf
 * operation after another causes the second to start the search form where the previous one
 * stopped.
 */
public class StringInspector {
  ...
  /**
   * This object provides string searching and manipulation operations such as finding sub-strings,
   * matching sub-strings or building a comments-free version of a string.
   *
   ...
   * @param closingMarkers the characters that can delimit the end of a of a quoted text block. The
   *     length of this parameter must match the length of th previous one
   * @param overridingMarkers the subset of <code>openingMarkers</code> that override the remaining
   *     markers, e.g., if <code>openingMarkers = "'("</code>, <code>closingMarkers = "')"</code>
   *     and <code>overridingMarkers = "'"</code> then the block between the outer parenthesis in
   *     <code>"start
   *            ('max('); end"</code> is strictly consumed, otherwise the suffix <code>"'); end"
   *     </code> would end up being consumed too in the process of handling the nested parenthesis.
   * @param searchMode a <code>Set</code>, ideally an <code>EnumSet</code>, containing the default
   *     flags from the enum {@link SearchMode} that determine the behavior of the search and
   *     manipulation operations. Note that some operation may override the search mode according to
   *     their needs, but the default one is not touched.
   */
  ...
  public StringInspector(
    ...
    if (this.defaultSearchMode.contains(SearchMode.SKIP_BETWEEN_MARKERS)) {
      if (this.openingMarkers == null
          || this.closingMarkers == null
          || this.openingMarkers.length() != this.closingMarkers.length()) {
        throw new IllegalArgumentException(
            Messages.getString(
                "StringInspector.2", new String[] {this.openingMarkers, this.closingMarkers}));
      }
      if (this.overridingMarkers == null) {
        throw new IllegalArgumentException(
            Messages.getString(
                "StringInspector.3", new String[] {this.overridingMarkers, this.openingMarkers}));
      }
  ...
  /**
   * Sets the position from where the source string will be processed from now on, taking into
   * consideration the "escaped" status of current character, if the mode {@link
  ...
  /**
   * Marks the current object's state. A subsequent call to the {@link #reset()} method restores the
   * marked state.
   */
  public void mark() {
  ...
  /**
   * Increments the current position index, by one, taking into consideration the "escaped" status
   * of current character, if the mode {@link SearchMode#ALLOW_BACKSLASH_ESCAPE} is present in the
   * default search mode.
   *
  ...
  /**
   * Increments the current position index, by be given number, taking into consideration the
   * "escaped" status of current character, if the mode {@link SearchMode#ALLOW_BACKSLASH_ESCAPE} is
   ...
   * Increments the current position index, by be given number, taking into consideration the
   * "escaped" status of current character, if the mode {@link SearchMode#ALLOW_BACKSLASH_ESCAPE} is
   * present in the specified search mode.
   *
   * @param by the number of positions to increment
   * @param searchMode the search mode to use in this operation
   * @return the new current position
  ...
  /**
   * Checks if the current character is escaped by analyzing previous characters, as long as the
   * search mode {@link SearchMode#ALLOW_BACKSLASH_ESCAPE} is present in the default search mode.
   */
  ...
  /**
   * Returns the position of the next valid character. This method does not increment the current
   * position automatically, i.e., if already positioned in a valid character then repeated calls
   * return always the same index. If the character in the current position matches one of the
   * prefixes that determine a skipping block, then the position marker advances to the first
   * character after the block to skip.
   *
   * @return the position of the next valid character, or the current position if already on a valid
   *     character
   */
  ...
  /**
   * Returns the position of the next valid character using the given search mode instead of the
   * default one. This method does not increment the current position automatically, i.e., if
   * already positioned in a valid character then repeated calls return always the same index. If
   * the character in the current position matches one of the prefixes that determine a skipping
   * block, then the position marker advances to the first character after the block to skip.
   *
   * @param searchMode the search mode to use in this operation
   * @return the position of the next valid character, or the current position if already on a valid
   *     character
  ...
  private int indexOfNextChar(Set<SearchMode> searchMode) {
    ...
    for (; this.pos < this.stopAt; this.pos++) {
      ...
      boolean checkSkipConditions =
          !searchMode.contains(SearchMode.ALLOW_BACKSLASH_ESCAPE) || !this.escaped;

      if (checkSkipConditions
          && searchMode.contains(SearchMode.SKIP_BETWEEN_MARKERS)
          && this.openingMarkers.indexOf(c0) != -1) {
        ...
        if (this.pos >= this.stopAt) {
          ...
          && searchMode.contains(SearchMode.SKIP_BLOCK_COMMENTS)
          && c0 == '/'
          && c1 == '*'
          && c2 != '!'
          && c2 != '+') {
        // Comments block found, skip until end of block ("*/") (backslash escape doesn't work in
        // comments).
        ...
        c2 = this.pos + 2 < this.srcLen ? this.source.charAt(this.pos + 2) : Character.MIN_VALUE;
          ...
          && ((c0 == '-'
                  ...
                  && (Character.isWhitespace(c2)
                      || (dashDashCommentImmediateEnd = c2 == ';')
        ...
        if (dashDashCommentImmediateEnd) {
          ...
          if (this.pos >= this.stopAt) {
            ...
            c2 =
                this.pos + 2 < this.srcLen ? this.source.charAt(this.pos + 2) : Character.MIN_VALUE;
          }
        }

      } else if (checkSkipConditions
          && searchMode.contains(SearchMode.SKIP_HINT_BLOCKS)
          && c0 == '/'
          && c1 == '*'
          && c2 == '+') {
        // Hints block found, skip until end of block ("*/") (backslash escape doesn't work in
        // hints).
        this.pos++; // Move to next char ('*').
        this.pos++; // Move to next char ('+').
        while (++this.pos < this.stopAt
        ...
        c2 = this.pos + 2 < this.srcLen ? this.source.charAt(this.pos + 2) : Character.MIN_VALUE;
          ...
          && searchMode.contains(SearchMode.SKIP_MYSQL_MARKERS)
          && c0 == '/'
          && c1 == '*'
          && c2 == '!') {
        ...
        if (c2 == '!') {
          ...
          for (; i < NON_COMMENTS_MYSQL_VERSION_REF_LENGTH; i++) {
            if (this.pos + 1 + i >= this.srcLen
                || !Character.isDigit(this.source.charAt(this.pos + 1 + i))) {
              break;
            }
          }
          if (i == NON_COMMENTS_MYSQL_VERSION_REF_LENGTH) {
            ...
            if (this.pos >= this.stopAt) {
              this.pos =
                  this.stopAt
                      - 1; // Reached stop position. Correct position will be set by outer loop.
            }
          }
        }
      ...
      // Reaching here means that the position has incremented thus an 'escaped' status no longer
      // holds.
      this.escaped = false;
    }
  ...
  /**
   * Returns the position of the next closing marker corresponding to the opening marker in the
   * current position. If the current position is not an opening marker, then -1 is returned
   * instead.
   *
   * @param searchMode the search mode to use in this operation
   * @return the position of the next closing marker corresponding to the opening marker in the
   *     current position
   */
  private int indexOfClosingMarker(Set<SearchMode> searchMode) {
    ...
    while (++this.pos < this.stopAt
        && ((c0 = this.source.charAt(this.pos)) != closingMarker || nestedMarkersCount != 0)) {
      if (!outerIsAnOverridingMarker && this.overridingMarkers.indexOf(c0) != -1) {
        // There is an overriding marker that needs to be consumed before returning to the previous
        // marker.
        int overridingMarkerIndex =
            this.openingMarkers.indexOf(
                c0); // OverridingMarkers must be a sub-list of openingMarkers.
  ...
  /**
   * Returns the position of the next alphanumeric character regardless the default search mode
   * originally specified. This method does not increment the current position automatically, i.e.,
   * if already positioned in a valid character then repeated calls return always the same index. If
   * the character in the current position matches one of the prefixes that determine a skipping
   * block, then the position marker advances to the first alphanumeric character after the block to
   * skip.
   *
   * @return the position of the next valid character, or the current position if already on a valid
   *     character
  ...
  /**
   * Returns the position of the next non-whitespace character regardless the default search mode
   * originally specified. This method does not increment the current position automatically, i.e.,
   * if already positioned in a valid character then repeated calls return always the same index. If
   * the character in the current position matches one of the prefixes that determine a skipping
   * block, then the position marker advances to the first non-whitespace character after the block
   * to skip.
   *
   * @return the position of the next valid character, or the current position if already on a valid
   *     character
  ...
  /**
   * Returns the position of the next whitespace character regardless the default search mode
   * originally specified. This method does not increment the current position automatically, i.e.,
   * if already positioned in a valid character then repeated calls return always the same index. If
   * the character in the current position matches one of the prefixes that determine a skipping
   * block, then the position marker advances to the first whitespace character after the block to
   * skip.
   *
   * @return the position of the next valid character, or the current position if already on a valid
   *     character
  ...
  /**
   * Finds the position of the given string within the source string, ignoring case, with the option
   * to skip text delimited by the specified markers or inside comment blocks.
   *
   * @param searchFor the sub-string to search for
   * @return the position where the sub-string is found, starting from the current position, or -1
   *     if not found
   */
  ...
  public int indexOfIgnoreCase(String searchFor, Set<SearchMode> searchMode) {
    ...
    if (Character.isWhitespace(firstCharOfSearchForLc)
        && this.defaultSearchMode.contains(SearchMode.SKIP_WHITE_SPACE)) {
    ...
    while (this.pos < localStopAt) {
      ...
      if (StringUtils.isCharEqualIgnoreCase(
              getChar(), firstCharOfSearchForUc, firstCharOfSearchForLc)
  ...
  /**
   * Finds the position of the of the first of a consecutive sequence of strings within the source
   * string, ignoring case, with the option to skip text delimited by the specified markers or
   * inside comment blocks.
   *
   * <p>Independently of the <code>searchMode</code> specified, when searching for the second and
   * following sub-strings {@link SearchMode#SKIP_WHITE_SPACE} will be added and {@link
   * SearchMode#SKIP_BETWEEN_MARKERS} removed.
   *
   * @param searchFor the sequence of sub-strings to search
   * @return the position where the first sub-string is found, starting from the current position,
   *     or -1 if not found
   */
  public int indexOfIgnoreCase(String... searchFor) {
    if (searchFor == null) {
    ...
    searchForLength +=
        searchForWordsCount > 0 ? searchForWordsCount - 1 : 0; // Count gaps between words.
    int localStopAt = this.srcLen - searchForLength + 1;
    if (localStopAt > this.stopAt) {
      localStopAt = this.stopAt;
    }
    ...
    if (Character.isWhitespace(searchFor[0].charAt(0))
        && this.defaultSearchMode.contains(SearchMode.SKIP_WHITE_SPACE)) {
    ...
    // searchMode used to search 2nd and following words cannot contain
    // SearchMode.SKIP_BETWEEN_MARKERS and must contain SearchMode.SKIP_WHITE_SPACE.
    ...
    while (this.pos < localStopAt) {
      ...
      while (++wc < searchForWordsCount && match) {
        if (indexOfNextChar(searchMode2) == -1
            || startingPositionForNextWord == this.pos
            || !StringUtils.regionMatchesIgnoreCase(this.source, this.pos, searchFor[wc])) {
          // Either there are no more chars to search, there is no gap between words or match
          // failed.
          match = false;
        } else {
  ...
  /**
   * Checks if the given string matches the source string counting from the current position,
   * ignoring case, with the option to skip text delimited by the specified markers or inside
   * comment blocks.
   *
  ...
  /**
   * Checks if the given consecutive sequence of strings matches the source string counting from the
   * current position, ignoring case, with the option to skip text delimited by the specified
   * markers or inside comment blocks.
   *
   * <p>Independently of the <code>searchMode</code> specified, when matching the second and
   * following sub-strings {@link SearchMode#SKIP_WHITE_SPACE} will be added and {@link
   * SearchMode#SKIP_BETWEEN_MARKERS} removed.
   *
  ...
  public int matchesIgnoreCase(String... toMatch) {
    ...
    // searchMode used to match 2nd and following words cannot contain
    // SearchMode.SKIP_BETWEEN_MARKERS and must contain SearchMode.SKIP_WHITE_SPACE.
    Set<SearchMode> searchMode2 = EnumSet.copyOf(this.defaultSearchMode);
    searchMode2.add(SearchMode.SKIP_WHITE_SPACE);
    searchMode2.remove(SearchMode.SKIP_BETWEEN_MARKERS);
  ...
  public String stripCommentsAndHints() {
    ...
    Set<SearchMode> searchMode =
        EnumSet.of(
            SearchMode.SKIP_BLOCK_COMMENTS,
            SearchMode.SKIP_LINE_COMMENTS,
            SearchMode.SKIP_HINT_BLOCKS);
    if (this.defaultSearchMode.contains(SearchMode.ALLOW_BACKSLASH_ESCAPE)) {
    ...
    while (this.pos < this.stopAt) {
      ...
      if (!this.escaped && this.openingMarkers.indexOf(getChar()) != -1) {
        // Characters between markers must be taken all at once otherwise indexOfNextChar() would
        // skip them if they contain comments-like text.
  ...
  /**
   * Splits the source string by the given delimiter. Consecutive delimiters result in empty string
   * parts.
   *
