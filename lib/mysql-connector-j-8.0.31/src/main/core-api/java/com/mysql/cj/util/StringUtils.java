...
/**
 * Various utility methods for converting to/from byte arrays in the platform encoding and several
...
public class StringUtils {
  ...
  private static final String VALID_ID_CHARS =
      "abcdefghijklmnopqrstuvwxyzABCDEFGHIGKLMNOPQRSTUVWXYZ0123456789$_#@";
  ...
  public static String dumpAsHex(byte[] byteBuffer, int length) {
    ...
    for (int p = 0, l = 0;
        p < length;
        l = 0) { // p: position in buffer (1..length); l: position in line (1..8)
  ...
  /**
   * Returns the byte[] representation of the given string properly wrapped between the given char
   * delimiters using the given encoding.
   *
   * @param s source string
  ...
  public static int indexOfIgnoreCase(int startingPosition, String searchIn, String searchFor) {
    ...
    for (int i = startingPosition; i <= stopSearchingAt; i++) {
      if (isCharAtPosNotEqualIgnoreCase(
          searchIn, i, firstCharOfSearchForUc, firstCharOfSearchForLc)) {
        // find the first occurrence of the first character of searchFor in searchIn
        while (++i <= stopSearchingAt
            && (isCharAtPosNotEqualIgnoreCase(
                searchIn, i, firstCharOfSearchForUc, firstCharOfSearchForLc))) {}
      }
   ...
   *
   * <p>Independently of the <code>searchMode</code> provided, when searching for the second and
   * following strings <code>SearchMode.SKIP_WHITE_SPACE</code> will be added and <code>
   * SearchMode.SKIP_BETWEEN_MARKERS</code> removed.
   *
   * @param startingPosition the position to start the search from
   * @param searchIn the string to search in
   ...
   *     from the enum <code>StringUtils.SearchMode</code> that determine the behavior of the search
   * @return the position where <code>searchFor</code> is found within <code>searchIn</code>
   *     starting from <code>startingPosition</code>.
   */
  public static int indexOfIgnoreCase(
    ...
    StringInspector strInspector =
        new StringInspector(
            searchIn, startingPosition, openingMarkers, closingMarkers, "", searchMode);
  ...
  public static int indexOfIgnoreCase(
    ...
    return indexOfIgnoreCase(
        startingPosition, searchIn, searchFor, openingMarkers, closingMarkers, "", searchMode);
  }
   ...
   *     markers, e.g., if <code>openingMarkers = "'("</code> and <code>overridingMarkers = "'"
   *     </code> then the block between the outer parenthesis in <code>"start ('max('); end"</code>
   *     is strictly consumed, otherwise the suffix <code>" end"</code> would end up being consumed
   *     too in the process of handling the nested parenthesis.
   * @param searchMode a <code>Set</code>, ideally an <code>EnumSet</code>, containing the flags
   *     from the enum <code>StringUtils.SearchMode</code> that determine the behavior of the search
   * @return the position where <code>searchFor</code> is found within <code>searchIn</code>
   *     starting from <code>startingPosition</code>.
  ...
  /**
   * Finds the position of the next alphanumeric character within a string, with the option to skip
   * text delimited by given markers or within comments.
   *
   * @param startingPosition the position to start the search from
   * @param searchIn the string to search in
   * @param openingMarkers characters that delimit the beginning of a text block to skip
   ...
   * @param searchMode a <code>Set</code>, ideally an <code>EnumSet</code>, containing the flags
   *     from the enum <code>StringUtils.SearchMode</code> that determine the behavior of the search
   * @return the position where the next non-whitespace character is found within <code>searchIn
   *     </code> starting from <code>startingPosition</code>.
   */
  public static int indexOfNextAlphanumericChar(
      int startingPosition,
      String searchIn,
      String openingMarkers,
      String closingMarkers,
  ...
  /**
   * Finds the position of the next non-whitespace character within a string, with the option to
   * skip text delimited by given markers or within comments.
   *
   * @param startingPosition the position to start the search from
   * @param searchIn the string to search in
   * @param openingMarkers characters that delimit the beginning of a text block to skip
  ...
  /**
   * Finds the position of the next whitespace character within a string, with the option to skip
   * text delimited by given markers or within comments.
   *
   * @param startingPosition the position to start the search from
   * @param searchIn the string to search in
   ...
   * @param searchMode a <code>Set</code>, ideally an <code>EnumSet</code>, containing the flags
   *     from the enum <code>StringUtils.SearchMode</code> that determine the behavior of the search
   * @return the position where the next whitespace character is found within <code>searchIn</code>
   *     starting from <code>startingPosition</code>.
   */
  public static int indexOfNextWsChar(
      int startingPosition,
      String searchIn,
      String openingMarkers,
      String closingMarkers,
  ...
  private static boolean isCharAtPosNotEqualIgnoreCase(
  ...
  protected static boolean isCharEqualIgnoreCase(
      char charToCompare, char compareToCharUC, char compareToCharLC) {
  ...
  /**
   * Splits stringToSplit into a list, using the given delimiter and skipping all between the given
   * markers.
   *
   * @param stringToSplit the string to split
   * @param delimiter the string to split on
   ...
   * @param trim should the split strings be whitespace trimmed?
   * @param searchMode a <code>Set</code>, ideally an <code>EnumSet</code>, containing the flags
   *     from the enum <code>StringUtils.SearchMode</code> that determine the behaviour of the
   *     search
   * @return the list of strings, split by delimiter
   * @throws IllegalArgumentException if an error occurs
   */
  ...
  public static List<String> split(
    ...
    StringInspector strInspector =
        new StringInspector(
            stringToSplit, openingMarkers, closingMarkers, overridingMarkers, searchMode);
   ...
   * case and starting at 'startAt'. Shorthand for a String.regionMatch(...)
   *
   * @param searchIn the string to search in
  ...
  /**
   * Determines whether or not the string 'searchIn' starts with the string 'searchFor',
   * dis-regarding case. Shorthand for a String.regionMatch(...)
   *
   * @param searchIn the string to search in
   * @param searchFor the string to search for
   * @return whether searchIn starts with searchFor, ignoring case
  ...
  /**
   * Determines whether or not the string 'searchIn' starts with the string 'searchFor',
   * disregarding case,leading whitespace and non-alphanumeric characters.
   *
   * @param searchIn the string to search in
   * @param searchFor the string to search for
  ...
  /**
   * Determines whether or not the string 'searchIn' starts with one of the strings in 'searchFor',
   * disregarding case and leading whitespace
   *
   * @param searchIn the string to search in
  ...
  /**
   * Determines whether or not the string 'searchIn' ends with the string 'searchFor', dis-regarding
   ...
   *
   * @param searchIn the string to search in
   * @param searchFor the string to search for, using the 'standard' SQL wildcard chars of '%' and
   *     '_'
  ...
  /**
   * Compares searchIn against searchForWildcard with wildcards (heavily borrowed from
   * strings/ctype-simple.c in the server sources)
   *
   * <p>This method does a single passage matching for normal characters and WILDCARD_ONE (_), and
   * recursive matching for WILDCARD_MANY (%) which may be repeated for as many anchor chars are
   * found.
   *
   * @param searchIn the string to search in
   * @param searchFor the string to search for, using the 'standard' SQL wildcard chars of '%' and
   *     '_'
   * @return WILD_COMPARE_MATCH if matched, WILD_COMPARE_NO_MATCH if not matched,
   *     WILD_COMPARE_CONTINUE_WITH_WILD if not matched yet, but it may in one of following
   *     recursion rounds
   */
  private static int wildCompareInternal(String searchIn, String searchFor) {
    ...
    while (searchForPos != searchForEnd) {
      while ((searchFor.charAt(searchForPos) != WILDCARD_MANY)
          && (searchFor.charAt(searchForPos) != WILDCARD_ONE)) {
        if ((searchFor.charAt(searchForPos) == WILDCARD_ESCAPE)
            && ((searchForPos + 1) != searchForEnd)) {
          searchForPos++;
        }

        if ((searchInPos == searchInEnd)
            || (Character.toUpperCase(searchFor.charAt(searchForPos++))
                != Character.toUpperCase(searchIn.charAt(searchInPos++)))) {
        ...
        if (searchForPos == searchForEnd) {
          return ((searchInPos != searchInEnd)
              ...
              : WILD_COMPARE_MATCH); /* Match if both are at end */
        }
      ...
      if (searchFor.charAt(searchForPos) == WILDCARD_MANY) {
        ...
        char cmp;
        if (((cmp = searchFor.charAt(searchForPos)) == WILDCARD_ESCAPE)
            && ((searchForPos + 1) != searchForEnd)) {
        ...
        do {
          ...
          int tmp =
              wildCompareInternal(
                  searchIn.substring(searchInPos), searchFor.substring(searchForPos));
          if (tmp <= 0) {
            return tmp;
          }
   ...
   */
  public static String stripCommentsAndHints(
      final String source,
      final String openingMarkers,
      final String closingMarkers,
  ...
  /**
   * Next two functions are to help DBMD check if the given string is in form of database.name and
   * return it as "database";"name" with comments removed. If string is NULL or wildcard (%),
   * returns null and exits.
   *
   * <p>First, we sanitize...
   *
  ...
  /**
   * Splits an entity identifier into its parts (database and entity name) and returns a list
   * containing the two elements. If the identifier doesn't contain the database part then the
   * argument <code>db</code> is used in its place and <code>source</code> corresponds to the full
   * entity name. If argument <code>source</code> is NULL or wildcard (%), returns an empty list.
   *
   ...
   * @return the input string with all comment-delimited data removed
   */
  public static List<String> splitDBdotName(
      String source, String db, String quoteId, boolean isNoBslashEscSet) {
    ...
    if (" ".equals(quoteId)) {
      ...
      dotIndex =
          indexOfIgnoreCase(
              ...
              quoteId,
              quoteId,
              isNoBslashEscSet ? SearchMode.__MRK_WS : SearchMode.__BSE_MRK_WS);
    }
  ...
  public static String getFullyQualifiedName(
    ...
    StringBuilder fullyQualifiedName =
        new StringBuilder(StringUtils.quoteIdentifier(db == null ? "" : db, quoteId, isPedantic));
   ...
   *
   * @param quoteChar ` or "
   * @param identifier in pedantic mode (connection property pedantic=true) identifier is treated as
   *     unquoted (as it is stored in the database) even if it starts and ends with quoteChar; in
   *     non-pedantic mode if identifier starts and ends with quoteChar method treats it as already
   *     quoted and doesn't modify.
   * @param isPedantic are we in pedantic mode
   ...
   *       <li>ab`c {@code ->} `ab``c`
   *       <li>ab"c {@code ->} `ab"c`
   *       <li>`ab``c` {@code ->} `ab``c` in non-pedantic mode or ```ab````c``` in pedantic mode
   *     </ul>
   *     With quoteChar="\"":<br>
   *     <ul>
   *       <li>null {@code ->} null
  ...
  public static String quoteIdentifier(String identifier, String quoteChar, boolean isPedantic) {
    ...
    // Check if the identifier is correctly quoted and if quotes within are correctly escaped. If
    // not, quote and escape it.
    if (!isPedantic && identifier.startsWith(quoteChar) && identifier.endsWith(quoteChar)) {
      ...
      String identifierQuoteTrimmed =
          identifier.substring(quoteCharLength, identifier.length() - quoteCharLength);
      ...
      while (quoteCharPos >= 0) {
        ...
        int quoteCharNextPosition =
            identifierQuoteTrimmed.indexOf(quoteChar, quoteCharNextExpectedPos);

        if (quoteCharNextPosition == quoteCharNextExpectedPos) {
          quoteCharPos =
              identifierQuoteTrimmed.indexOf(quoteChar, quoteCharNextPosition + quoteCharLength);
        } else {
   ...
   *
   * @param identifier in pedantic mode (connection property pedantic=true) identifier is treated as
   *     unquoted (as it is stored in the database) even if it starts and ends with "`"; in
   *     non-pedantic mode if identifier starts and ends with "`" method treats it as already quoted
   *     and doesn't modify.
   * @param isPedantic are we in pedantic mode
   * @return
   *     <ul>
  ...
  /**
   * Trims the identifier, removes quote chars from first and last positions and replaces double
   * occurrences of quote char from entire identifier, i.e. converts quoted identifier into the form
  ...
  public static String unQuoteIdentifier(String identifier, String quoteChar) {
    ...
    // Check if the identifier is really quoted or if it simply contains quote chars in it (assuming
    // that the value is a valid identifier).
  ...
  /**
   * Returns the byte[] representation of subset of the given char[] using the default/platform
   * encoding.
   *
   * @param value chars
   * @return bytes
  ...
  public static boolean canHandleAsServerPreparedStatementNoCache(
      String sql,
    ...
    if (allowMultiQueries) {
      ...
      canHandleAsStatement = false;
        ...
        && serverVersion.meetsMinimum(ServerVersion.parseVersion("5.7.2"))) {
      canHandleAsStatement = false;
  ...
  /**
   * Checks is the CharSequence contains digits only. No leading sign and thousands or decimal
   * separators are allowed.
   *
   * @param cs The CharSequence to check.
   * @return {@code true} if the CharSequence not empty and contains only digits, {@code false}
   *     otherwise.
   */
  ...
  /**
   * Constructs a String containing all the elements in the String array bounded and joined by the
   * provided concatenation elements. The last element uses a different delimiter.
   *
  ...
  public static String stringArrayToString(
      String[] elems, String prefix, String midDelimiter, String lastDelimiter, String suffix) {
    StringBuilder valuesString = new StringBuilder();
    if (elems.length > 1) {
      valuesString.append(
          Arrays.stream(elems)
              .limit(elems.length - 1)
              .collect(Collectors.joining(midDelimiter, prefix, lastDelimiter)));
    } else {
  ...
  /**
   * Joins all elements of the given list using serial comma (Oxford comma) rules. E.g.: - "A" - "A
   * and B" - "A, B, and C"
   *
  ...
  public static String joinWithSerialComma(List<?> elements) {
    ...
    return elements.subList(0, elements.size() - 1).stream()
            .map(Object::toString)
  ...
  public static StringBuilder escapeString(
      StringBuilder buf,
      String x,
    ...
    //
    // Note: buf.append(char) is _faster_ than appending in blocks, because the block append
    // requires a System.arraycopy().... go figure...
    //
