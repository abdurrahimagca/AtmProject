...
public class TimeUtil {
  ...
  public static final Pattern DATE_LITERAL_WITH_DELIMITERS =
      Pattern.compile(
          "(\\d{4}|\\d{2})[\\p{Punct}&&[^:]](([0])?[1-9]|[1][0-2])[\\p{Punct}&&[^:]](([0])?[1-9]|[1-2]\\d|[3][0-1])");
  public static final Pattern DATE_LITERAL_NO_DELIMITERS =
      Pattern.compile("(\\d{4}|\\d{2})([0][1-9]|[1][0-2])([0][1-9]|[1-2]\\d|[3][0-1])");

  public static final Pattern TIME_LITERAL_WITH_DELIMITERS =
      Pattern.compile("(([0-1])?\\d|[2][0-3]):([0-5])?\\d(:([0-5])?\\d(\\.\\d{1,9})?)?");
  public static final Pattern TIME_LITERAL_SHORT6 =
      Pattern.compile("([0-1]\\d|[2][0-3])([0-5]\\d){2}(\\.\\d{1,9})?");
  ...
  public static final Pattern DATETIME_LITERAL_WITH_DELIMITERS =
      Pattern.compile(
          "(\\d{4}|\\d{2})\\p{Punct}(([0])?[1-9]|[1][0-2])\\p{Punct}(([0])?[1-9]|[1-2]\\d|[3][0-1])["
              + " T](([0-1])?\\d|[2][0-3])\\p{Punct}([0-5])?\\d(\\p{Punct}([0-5])?\\d(\\.\\d{1,9})?)?");
  public static final Pattern DATETIME_LITERAL_SHORT14 =
      Pattern.compile(
          "\\d{4}([0][1-9]|[1][0-2])([0][1-9]|[1-2]\\d|[3][0-1])([0-1]\\d|[2][0-3])([0-5]\\d){2}(\\.\\d{1,9}){0,1}");
  public static final Pattern DATETIME_LITERAL_SHORT12 =
      Pattern.compile(
          "\\d{2}([0][1-9]|[1][0-2])([0][1-9]|[1-2]\\d|[3][0-1])([0-1]\\d|[2][0-3])([0-5]\\d){2}(\\.\\d{1,9}){0,1}");

  public static final Pattern DURATION_LITERAL_WITH_DAYS =
      Pattern.compile(
          "(-)?(([0-2])?\\d|[3][0-4])"
              + " (([0-1])?\\d|[2][0-3])(:([0-5])?\\d(:([0-5])?\\d(\\.\\d{1,9})?)?)?");
  public static final Pattern DURATION_LITERAL_NO_DAYS =
      Pattern.compile("(-)?\\d{1,3}:([0-5])?\\d(:([0-5])?\\d(\\.\\d{1,9})?)?");

  // Mappings from TimeZone identifications (prefixed by type: Windows, TZ name, MetaZone, TZ alias,
  // ...), to standard TimeZone Ids
   ...
   */
  public static String getCanonicalTimeZone(
      String timezoneStr, ExceptionInterceptor exceptionInterceptor) {
    if (timezoneStr == null) {
      return null;
    }
   ...
   * @param serverRoundFracSecs Flag indicating whether rounding or truncation occurs on server when
   ...
   * @return A new Timestamp object cloned from the original one and then rounded or truncated
  ...
  public static Timestamp adjustNanosPrecision(Timestamp ts, int fsp, boolean serverRoundFracSecs) {
    if (fsp < 0 || fsp > 6) {
      throw ExceptionFactory.createException(
          WrongArgumentException.class, "fsp value must be in 0 to 6 range.");
    }
    Timestamp res = (Timestamp) ts.clone();
    double tail = Math.pow(10, 9 - fsp);
    int nanos =
        serverRoundFracSecs
            ? (int) Math.round(res.getNanos() / tail) * (int) tail
            : (int) (res.getNanos() / tail) * (int) tail;
  ...
  /**
   * Return a new LocalDateTime object which value is adjusted according to known DATE, DATETIME or
   * TIMESTAMP field precision.
   *
   ...
   *     truncation. The proper value should be detected by analyzing sql_mode server variable for
   *     TIME_TRUNCATE_FRACTIONAL presence.
   * @return A new LocalDateTime object cloned from the original one and then rounded or truncated
   *     according to required fsp value
   */
  public static LocalDateTime adjustNanosPrecision(
      LocalDateTime x, int fsp, boolean serverRoundFracSecs) {
    if (fsp < 0 || fsp > 6) {
      throw ExceptionFactory.createException(
          WrongArgumentException.class, "fsp value must be in 0 to 6 range.");
    }
    ...
    int adjustedNano =
        serverRoundFracSecs
            ? (int) Math.round(originalNano / tail) * (int) tail
            : (int) (originalNano / tail) * (int) tail;
   ...
   * @param nanos fractional seconds value
   * @param fsp required fractional part length
   * @param truncateTrailingZeros whether to remove trailing zero characters in a fractional part
   *     after formatting
   * @return fractional seconds part as a string
   */
  public static String formatNanos(int nanos, int fsp, boolean truncateTrailingZeros) {
    if (nanos < 0 || nanos > 999999999) {
      throw ExceptionFactory.createException(
          ...
          "nanos value must be in 0 to 999999999 range but was " + nanos);
    }
    if (fsp < 0 || fsp > 6) {
      throw ExceptionFactory.createException(
          WrongArgumentException.class, "fsp value must be in 0 to 6 range but was " + fsp);
    }
  ...
  private static void loadTimeZoneMappings(ExceptionInterceptor exceptionInterceptor) {
    ...
    try {
      ...
      throw ExceptionFactory.createException(
          Messages.getString("TimeUtil.LoadTimeZoneMappingError"), exceptionInterceptor);
    }
   ...
   *
   * <p>Note: The SimpleDateFormat object returned by this method contains a default Calendar with
   * an altered TimeZone. It's safe to cache it between this method calls because the Calendar
   * object itself is not altered.
   *
  ...
  public static SimpleDateFormat getSimpleDateFormat(
      SimpleDateFormat cachedSimpleDateFormat, String pattern, TimeZone tz) {
    SimpleDateFormat sdf =
        cachedSimpleDateFormat != null && cachedSimpleDateFormat.toPattern().equals(pattern)
   ...
   *
   * <p>Note: Don't cache the SimpleDateFormat object returned by this method. Other methods could
   * rely on assumption that the cached SimpleDateFormat has a default Calendar and that it is safe
   * to change only it's time zone (see {@link #getSimpleDateFormat(SimpleDateFormat, String,
   * TimeZone)}.
   *
   * @param pattern format pattern
  ...
  public static Object parseToDateTimeObject(String s, MysqlType targetMysqlType) {
    if (DATE_LITERAL_WITH_DELIMITERS.matcher(s).matches()) {
    ...
    } else if (DATE_LITERAL_NO_DELIMITERS.matcher(s).matches()
        && !(targetMysqlType == MysqlType.TIME && TIME_LITERAL_SHORT6.matcher(s).matches())) {
      return s.length() == 8
          ? LocalDate.parse(s, DateTimeFormatter.BASIC_ISO_DATE)
          : LocalDate.parse(s, DateTimeFormatter.ofPattern("yyMMdd"));
      ...
      return LocalDateTime.parse(
          s,
          new DateTimeFormatterBuilder()
              .appendPattern("yyMMddHHmmss")
              .appendFraction(ChronoField.NANO_OF_SECOND, 0, 9, true)
              .toFormatter());

    } else if (DATETIME_LITERAL_WITH_DELIMITERS.matcher(s).matches()) {
      return LocalDateTime.parse(
          getCanonicalDateTime(s),
          new DateTimeFormatterBuilder()
              .appendPattern("yyyy-MM-dd HH:mm:ss")
              .appendFraction(ChronoField.NANO_OF_SECOND, 0, 9, true)
              .toFormatter());

    } else if (DURATION_LITERAL_WITH_DAYS.matcher(s).matches()
        || DURATION_LITERAL_NO_DAYS.matcher(s).matches()) {
    ...
    throw ExceptionFactory.createException(
        WrongArgumentException.class, "There is no known date-time pattern for '" + s + "' value");
  }
