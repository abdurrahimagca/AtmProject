...
public class CancelQueryTaskImpl extends TimerTask implements CancelQueryTask {
  ...
  public CancelQueryTaskImpl(Query cancellee) {
    ...
    this.queryTimeoutKillsConnection =
        session
            .getPropertySet()
            .getBooleanProperty(PropertyKey.queryTimeoutKillsConnection)
            .getValue();
  }
          ...
          public void run() {
            ...
            try {
              if (CancelQueryTaskImpl.this.queryTimeoutKillsConnection) {
                ...
                session.invokeCleanupListeners(
                ...
                synchronized (localQueryToCancel.getCancelTimeoutMutex()) {
                  ...
                  try {
                    ...
                    newSession
                        .getProtocol()
                        .sendCommand(
                            new NativeMessageBuilder(
                                    newSession.getServerSession().supportsQueryAttributes())
                                .buildComQuery(
                                    newSession.getSharedSendPacket(), "KILL QUERY " + origConnId),
                            false,
                            0);
                  } finally {
                    try {
              ...
              // We can't easily synchronize this, because then one thread can't cancel() a running
              // query.
              // Ignore, we shouldn't re-throw this, because the connection's already closed, so the
              // statement has been timed out.
            } catch (Throwable t) {
