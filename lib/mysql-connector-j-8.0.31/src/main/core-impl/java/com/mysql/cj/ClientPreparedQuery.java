...
public class ClientPreparedQuery extends AbstractQuery implements PreparedQuery {
  ...
  public ClientPreparedQuery(NativeSession sess) {
    super(sess);
    this.autoClosePStmtStreams =
        this.session.getPropertySet().getBooleanProperty(PropertyKey.autoClosePStmtStreams);
    this.useStreamLengthsInPrepStmts =
        this.session.getPropertySet().getBooleanProperty(PropertyKey.useStreamLengthsInPrepStmts);
  }
  ...
  public void checkNullOrEmptyQuery(String sql) {
    if (sql == null) {
      throw ExceptionFactory.createException(
          ...
          Messages.getString("PreparedQuery.0"),
    ...
    if (sql.length() == 0) {
      throw ExceptionFactory.createException(
          WrongArgumentException.class,
          Messages.getString("PreparedQuery.1"),
          this.session.getExceptionInterceptor());
    }
  }
    ...
    for (int i = 0; i < this.parameterCount; ++i) {
      buf.append(
          this.charEncoding != null
              ? StringUtils.toString(staticSqlStrings[i], this.charEncoding)
      ...
      val =
          this.batchCommandIndex == -1
              ? (this.queryBindings == null
                  ? null
                  : this.queryBindings.getBindValues()[i].getString())
  ...
  /**
   * Computes the maximum parameter set size, and entire batch size given the number of arguments in
   * the batch.
   *
  ...
  protected long[] computeMaxParameterSetSizeAndBatchSize(int numBatchedArgs) {
    ...
    if (this.session.getServerSession().supportsQueryAttributes()) {
      ...
      sizeOfEntireBatch +=
          (this.queryAttributesBindings.getCount() + 7) / 8 /* null_bitmap */
    ...
    for (int i = 0; i < numBatchedArgs; i++) {
      ...
      // This is a little naive, because the ?s will be replaced but it gives us some padding, and
      // is less housekeeping to ignore them. We're looking
      // for a "fuzzy" value here anyway
      //
      sizeOfParameterSet +=
          this.queryInfo.getValuesClauseLength() != -1
              ? this.queryInfo.getValuesClauseLength() + 1
              : this.originalSql.length() + 1;
