...
public class NativeCharsetSettings extends CharsetMapping implements CharsetSettings {
  ...
  private String errorMessageEncoding =
      "Cp1252"; // to begin with, changes after we talk to the server
  ...
  private static final Map<String, Map<Integer, String>> customCollationIndexToCollationNameByUrl =
      new HashMap<>();
  private static final Map<String, Map<String, Integer>> customCollationNameToCollationIndexByUrl =
      new HashMap<>();
   ...
   * URLs.
   */
  private static final Map<String, Map<Integer, String>> customCollationIndexToCharsetNameByUrl =
      new HashMap<>();

  /** Actual mysql charset name to mblen map of user defined charsets for given server URLs. */
  private static final Map<String, Map<String, Integer>> customCharsetNameToMblenByUrl =
      new HashMap<>();

  private static final Map<String, Map<String, String>> customCharsetNameToJavaEncodingByUrl =
      new HashMap<>();
  private static final Map<String, Map<String, Integer>> customCharsetNameToCollationIndexByUrl =
      new HashMap<>();
  private static final Map<String, Map<String, String>> customJavaEncodingUcToCharsetNameByUrl =
      new HashMap<>();
  ...
  public NativeCharsetSettings(NativeSession sess) {
    ...
    this.characterSetResults =
        this.session.getPropertySet().getProperty(PropertyKey.characterSetResults);
    this.passwordCharacterEncoding =
        this.session.getPropertySet().getStringProperty(PropertyKey.passwordCharacterEncoding);
    this.connectionCollation =
        this.session.getPropertySet().getStringProperty(PropertyKey.connectionCollation);
    this.cacheServerConfiguration =
        sess.getPropertySet().getBooleanProperty(PropertyKey.cacheServerConfiguration);
   ...
   * @param replaceImpermissibleEncodings The character_set_client system variable cannot be set to
   *     ucs2, utf16, utf16le, utf32 charsets. If "true" the corresponding connection property value
   *     will be replaced with "UTF-8"
   */
  private void tryAndFixEncoding(
      RuntimeProperty<String> encodingProperty, boolean replaceImpermissibleEncodings) {
        ...
        try {
          ...
          if (newEncoding == null) {
            throw ExceptionFactory.createException(
                ...
                Messages.getString("StringUtils.0", new Object[] {oldEncoding}),
  ...
  public int configurePreHandshake(boolean reset) {
    ...
    if (encoding == null) {
      ...
      if ((connectionColl == null
              || (this.sessionCollationIndex =
                      getStaticCollationIndexForCollationName(connectionColl))
                  == null)
          && (encoding = this.characterEncoding.getValue()) == null) {
        // If none of "passwordCharacterEncoding", "connectionCollation" or "characterEncoding" is
        // specified then use UTF-8.
        // It would be better to use the server default collation here, to avoid unnecessary SET
        // NAMES queries after the handshake if server
        // default charset if not utf8, but we can not do it until server Bug#32729185 is fixed.
        // Server cuts collation index to lower byte and, for example,
        // if the server is started with character-set-server=utf8mb4 and
        // collation-server=utf8mb4_is_0900_ai_ci (collation index 257) the Protocol::HandshakeV10
        // will contain character_set=1, "big5_chinese_ci". This is true not only for MySQL 8.0,
        // where built-in collations with indexes > 255 were first introduced,
        // but also other server series would be affected when configured with custom collations,
        // for which the reserved collation id range is >= 1024.
        this.sessionCollationIndex = MYSQL_COLLATION_INDEX_utf8mb4_0900_ai_ci;
      }
    }

    if (this.sessionCollationIndex == null) {
      if ((this.sessionCollationIndex =
              getStaticCollationIndexForJavaEncoding(encoding, capabilities.getServerVersion()))
          == 0) {
        throw ExceptionFactory.createException(
            WrongArgumentException.class,
            Messages.getString("StringUtils.0", new Object[] {encoding}));
      }
    }
    ...
    this.serverSession
        ...
        .put(
            ...
            getStaticCollationNameForCollationIndex(this.sessionCollationIndex));
  ...
  public void configurePostHandshake(boolean dontCheckServerMatch) {
    ...
    try {
      ...
      if (requiredCollation != null
          && (requiredCollationIndex = getCollationIndexForCollationName(requiredCollation))
              != null) {
        ...
        requiredEncoding =
            getJavaEncodingForCollationIndex(requiredCollationIndex, requiredEncoding);
        this.sessionCollationIndex = requiredCollationIndex;
      }

      if (requiredEncoding
          != null) { // If either connectionCollation or characterEncoding is defined.
        if (sessionCollationClause.length() == 0) { // If no connectionCollation is defined.
          sessionCharsetName =
              getMysqlCharsetForJavaEncoding(
                  requiredEncoding.toUpperCase(Locale.ENGLISH),
        ...
        if (!StringUtils.isNullOrEmpty(passwordEncoding)) {
          ...
          sessionCollationClause =
              " COLLATE " + getCollationNameForCollationIndex(this.sessionCollationIndex);
        }

        if (((requiredEncoding =
                getJavaEncodingForCollationIndex(this.sessionCollationIndex, requiredEncoding))
            == null)) {
          // if there is no mapping for default collation index leave characterEncoding as specified
          // by user
          throw ExceptionFactory.createException(
              Messages.getString(
                  "Connection.5", new Object[] {this.sessionCollationIndex.toString()}),
              this.session.getExceptionInterceptor());
        }
      ...
      throw ExceptionFactory.createException(
          Messages.getString("Connection.6", new Object[] {this.sessionCollationIndex}),
          this.session.getExceptionInterceptor());
    }
    ...
    if (sessionCharsetName != null) {
      ...
      boolean isCollationDifferent =
          sessionCollationClause.length() > 0
              && !requiredCollation.equalsIgnoreCase(
                  this.serverSession.getServerVariable(COLLATION_CONNECTION));
      if (dontCheckServerMatch || isCharsetDifferent || isCollationDifferent) {
        this.session
            ...
            .sendCommand(
                getCommandBuilder()
                    .buildComQuery(
                        null, "SET NAMES " + sessionCharsetName + sessionCollationClause),
                false,
                0);
        ...
        if (sessionCollationClause.length() > 0) {
          ...
          this.serverSession
              .getServerVariables()
              .put(COLLATION_CONNECTION, getCollationNameForCollationIndex(idx));
        }
      }
    }
    ...
    if (StringUtils.isNullOrEmpty(characterSetResultsValue)
        || "null".equalsIgnoreCase(characterSetResultsValue)) {
      if (!StringUtils.isNullOrEmpty(sessionResultsCharset)
          && !"NULL".equalsIgnoreCase(sessionResultsCharset)) {
        this.session
            .getProtocol()
            .sendCommand(
                getCommandBuilder().buildComQuery(null, "SET character_set_results = NULL"),
                false,
                0);
      ...
      String defaultMetadataCharsetMysql =
          this.serverSession.getServerVariable("character_set_system");
      this.metadataEncoding =
          defaultMetadataCharsetMysql != null
              ? getJavaEncodingForMysqlCharset(defaultMetadataCharsetMysql)
              : "UTF-8";
      ...
      String resultsCharsetName =
          getMysqlCharsetForJavaEncoding(
              characterSetResultsValue.toUpperCase(Locale.ENGLISH),
              this.serverSession.getServerVersion());

      if (resultsCharsetName == null) {
        throw ExceptionFactory.createException(
            WrongArgumentException.class,
            Messages.getString("Connection.7", new Object[] {characterSetResultsValue}),
            this.session.getExceptionInterceptor());
      }
      ...
      if (!resultsCharsetName.equalsIgnoreCase(sessionResultsCharset)) {
        this.session
            ...
            .sendCommand(
                getCommandBuilder()
                    .buildComQuery(null, "SET character_set_results = " + resultsCharsetName),
                false,
                0);
    ...
    this.metadataCollationIndex =
        getCollationIndexForJavaEncoding(
            this.metadataEncoding, this.serverSession.getServerVersion());

    checkForCharsetMismatch();

    /**
     * Check if we need a CharsetEncoder for escaping codepoints that are transformed to backslash
     * (0x5c) in the connection encoding.
     */
    try {
  ...
  private boolean characterSetNamesMatches(String mysqlEncodingName) {
    ...
    if (mysqlEncodingName != null) {
      // set names is equivalent to character_set_client ..._results and ..._connection, but we set
      // _results later, so don't check it here.
      res =
          (mysqlEncodingName.equalsIgnoreCase(
                  this.serverSession.getServerVariable(CHARACTER_SET_CLIENT))
              && mysqlEncodingName.equalsIgnoreCase(
                  this.serverSession.getServerVariable(CHARACTER_SET_CONNECTION)));
      // if still doesn't match then checking aliases
      List<String> aliases;
      if (!res
          && (aliases = CharsetMapping.getStaticMysqlCharsetAliasesByName(mysqlEncodingName))
              != null) {
        for (String alias : aliases) {
          if (res =
              (alias.equalsIgnoreCase(this.serverSession.getServerVariable(CHARACTER_SET_CLIENT))
  ...
  /**
   * Builds the map needed for 4.1.0 and newer servers that maps field-level charset/collation info
   * to a java character encoding name.
   */
  private void buildCollationMapping() {
    ...
    if (this.cacheServerConfiguration.getValue()) {
      synchronized (customCollationIndexToCharsetNameByUrl) {
        customCollationIndexToCollationName =
            customCollationIndexToCollationNameByUrl.get(databaseURL);
        customCollationNameToCollationIndex =
            customCollationNameToCollationIndexByUrl.get(databaseURL);
    ...
    if (customCollationIndexToCharsetName == null
        && this.session
            .getPropertySet()
            .getBooleanProperty(PropertyKey.detectCustomCollations)
            .getValue()) {
      ...
      String customCharsetMapping =
          this.session
              .getPropertySet()
              .getStringProperty(PropertyKey.customCharsetMapping)
              .getValue();
      ...
      try {
            ...
            this.session
                ...
                .sendCommand(
                    getCommandBuilder()
                        .buildComQuery(
                            null,
                            "SELECT c.COLLATION_NAME, c.CHARACTER_SET_NAME, c.ID, cs.MAXLEN,"
                                + " c.IS_DEFAULT='Yes' from INFORMATION_SCHEMA.COLLATIONS AS c LEFT"
                                + " JOIN INFORMATION_SCHEMA.CHARACTER_SETS AS cs ON"
                                + " cs.CHARACTER_SET_NAME=c.CHARACTER_SET_NAME"),
                    false,
                    0);
        Resultset rs =
            this.session
        ...
        while ((r = rs.getRows().next()) != null) {
          ...
          if (collationIndex >= MAP_SIZE //
            ...
            if (isDefault
                || !customCharsetNameToCollationIndex.containsKey(charsetName)
                    && CharsetMapping.getStaticCollationIndexForMysqlCharsetName(charsetName)
                        == 0) {
        ...
        throw ExceptionFactory.createException(
            e.getMessage(), e, this.session.getExceptionInterceptor());
      }

      if (this.cacheServerConfiguration.getValue()) {
        synchronized (customCollationIndexToCharsetNameByUrl) {
          customCollationIndexToCollationNameByUrl.put(
              databaseURL, Collections.unmodifiableMap(customCollationIndexToCollationName));
          customCollationNameToCollationIndexByUrl.put(
              databaseURL, Collections.unmodifiableMap(customCollationNameToCollationIndex));
          customCollationIndexToCharsetNameByUrl.put(
              databaseURL, Collections.unmodifiableMap(customCollationIndexToCharsetName));
          customCharsetNameToMblenByUrl.put(
              databaseURL, Collections.unmodifiableMap(customCharsetNameToMblen));
          customCharsetNameToJavaEncodingByUrl.put(
              databaseURL, Collections.unmodifiableMap(customCharsetNameToJavaEncoding));
          customJavaEncodingUcToCharsetNameByUrl.put(
              databaseURL, Collections.unmodifiableMap(customJavaEncodingUcToCharsetName));
          customCharsetNameToCollationIndexByUrl.put(
              databaseURL, Collections.unmodifiableMap(customCharsetNameToCollationIndex));
          customMultibyteEncodingsByUrl.put(
              databaseURL, Collections.unmodifiableSet(customMultibyteEncodings));
        }
      }
    }
  ...
  public Integer getCollationIndexForCollationName(String collationName) {
    ...
    if (this.collationNameToCollationIndex == null
        || (collationIndex = this.collationNameToCollationIndex.get(collationName)) == null) {
      collationIndex = getStaticCollationIndexForCollationName(collationName);
    }
  ...
  public String getCollationNameForCollationIndex(Integer collationIndex) {
    ...
    if (collationIndex != null
        && (this.collationIndexToCollationName == null
  ...
  public String getMysqlCharsetNameForCollationIndex(Integer collationIndex) {
    String charset = null;
    if (this.collationIndexToCharsetName == null
        || (charset = this.collationIndexToCharsetName.get(collationIndex)) == null) {
  ...
  public String getJavaEncodingForCollationIndex(
      Integer collationIndex, String fallBackJavaEncoding) {
  ...
  public int getCollationIndexForJavaEncoding(String javaEncoding, ServerVersion version) {
    return getCollationIndexForMysqlCharsetName(
        getMysqlCharsetForJavaEncoding(javaEncoding, version));
  }

  public int getCollationIndexForMysqlCharsetName(String charsetName) {
    Integer index = null;
    if (this.charsetNameToCollationIndex == null
        || (index = this.charsetNameToCollationIndex.get(charsetName)) == null) {
  ...
  public String getJavaEncodingForMysqlCharset(String mysqlCharsetName) {
    String encoding = null;
    if (this.charsetNameToJavaEncoding == null
        || (encoding = this.charsetNameToJavaEncoding.get(mysqlCharsetName)) == null) {
  ...
  public String getMysqlCharsetForJavaEncoding(String javaEncoding, ServerVersion version) {
    ...
    if (this.javaEncodingUcToCharsetName == null
        || (charset =
                this.javaEncodingUcToCharsetName.get(javaEncoding.toUpperCase(Locale.ENGLISH)))
            == null) {
  ...
  public boolean isImpermissibleCollation(int collationIndex) {
    ...
    if (this.collationIndexToCharsetName != null
        && (charsetName = this.collationIndexToCharsetName.get(collationIndex)) != null) {
  ...
  public boolean isMultibyteCharset(String javaEncodingName) {
    if (this.multibyteEncodings != null
        && this.multibyteEncodings.contains(javaEncodingName.toUpperCase(Locale.ENGLISH))) {
      return true;
    }
    ...
    if ((charset = getMysqlCharsetNameForCollationIndex(charsetIndex)) == null) {
      ...
      charset =
          getStaticMysqlCharsetForJavaEncoding(
              javaCharsetName, this.serverSession.getServerVersion());
    }
    Integer mblen = null;
