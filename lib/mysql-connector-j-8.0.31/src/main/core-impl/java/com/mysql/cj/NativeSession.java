...
public class NativeSession extends CoreSession implements Serializable {
  ...
  private CopyOnWriteArrayList<WeakReference<SessionEventListener>> listeners =
  ...
  public void connect(
    ...
    if (this.protocol == null) {
      this.protocol =
          NativeProtocol.getInstance(
              this, socketConnection, this.propertySet, this.log, transactionManager);
    } else {
    ...
    this.commandBuilder =
        new NativeMessageBuilder(this.getServerSession().supportsQueryAttributes());
  }
  ...
  // TODO: we should examine the call flow here, we shouldn't have to know about the socket
  // connection but this should be address in a wider scope.
  @Override
  public void forceClose() {
    if (this.protocol != null) {
      // checking this.protocol != null isn't enough if connection is used concurrently (the usual
      // situation
      // with application servers which have additional thread management), this.protocol can become
      // null
      // at any moment after this check, causing a race condition and NPEs on next calls;
      // but we may ignore them because at this stage null this.protocol means that we successfully
      // closed all resources by other thread.
      try {
      ...
      // this.protocol = null; // TODO actually we shouldn't remove protocol instance because some
      // it's methods can be called after closing socket
    }
    synchronized (this) {
      if (this.cancelTimer != null) {
        this.cancelTimer.cancel();
  ...
  public void enableMultiQueries() {
    this.protocol.sendCommand(
        this.commandBuilder.buildComSetOption(
            ((NativeProtocol) this.protocol).getSharedSendPacket(), 0),
        false,
        0);
  ...
  public void disableMultiQueries() {
    this.protocol.sendCommand(
        this.commandBuilder.buildComSetOption(
            ((NativeProtocol) this.protocol).getSharedSendPacket(), 1),
        false,
        0);
    ((NativeServerSession) getServerSession()).preserveOldTransactionState();
  }
  ...
  public boolean isSetNeededForAutoCommitMode(boolean autoCommitFlag) {
    ...
    return ((NativeServerSession) this.protocol.getServerSession())
        .isSetNeededForAutoCommitMode(autoCommitFlag, false);
  }
  ...
  public void shutdownServer() {
    if (versionMeetsMinimum(5, 7, 9)) {
      this.protocol.sendCommand(
          this.commandBuilder.buildComQuery(getSharedSendPacket(), "SHUTDOWN"), false, 0);
    } else {
      this.protocol.sendCommand(
          this.commandBuilder.buildComShutdown(getSharedSendPacket()), false, 0);
    }
  }

  public void setSocketTimeout(int milliseconds) {
    getPropertySet()
        .getProperty(PropertyKey.socketTimeout)
        .setValue(milliseconds); // for re-connects
  ...
  public <T extends Resultset> T invokeQueryInterceptorsPre(
    ...
    return ((NativeProtocol) this.protocol)
        .invokeQueryInterceptorsPre(sql, interceptedQuery, forceExecute);
  }
  ...
  public <T extends Resultset> T invokeQueryInterceptorsPost(
    ...
    return ((NativeProtocol) this.protocol)
        .invokeQueryInterceptorsPost(sql, interceptedQuery, originalResultSet, forceExecute);
  }
    ...
    synchronized (syncMutex) {
      ...
      try {
        Class<?> factoryClass =
            Class.forName(
                getPropertySet()
                    .getStringProperty(PropertyKey.serverConfigCacheFactory)
                    .getStringValue());

        @SuppressWarnings("unchecked")
        CacheAdapterFactory<String, Map<String, String>> cacheFactory =
            ((CacheAdapterFactory<String, Map<String, String>>) factoryClass.newInstance());

        this.serverConfigCache =
            cacheFactory.getInstance(
                syncMutex, this.hostInfo.getDatabaseUrl(), Integer.MAX_VALUE, Integer.MAX_VALUE);
            ...
            new ExceptionInterceptor() {
              ...
              public Exception interceptException(Exception sqlEx) {
                if (sqlEx instanceof SQLException
                    && ((SQLException) sqlEx).getSQLState() != null
                    && ((SQLException) sqlEx).getSQLState().startsWith("08")) {
                  NativeSession.this.serverConfigCache.invalidate(
                      NativeSession.this.hostInfo.getDatabaseUrl());
                }
                return null;
              }
            };
        ...
        throw ExceptionFactory.createException(
            Messages.getString(
                ...
                new Object[] {
                  getPropertySet().getStringProperty(PropertyKey.queryInfoCacheFactory).getValue(),
  ...
  public void loadServerVariables(Object syncMutex, String version) {
    ...
    if (this.cacheServerConfiguration.getValue()) {
      ...
      Map<String, String> cachedVariableMap =
          this.serverConfigCache.get(this.hostInfo.getDatabaseUrl());

      if (cachedVariableMap != null) {
        String cachedServerVersion = cachedVariableMap.get(SERVER_VERSION_STRING_VAR_NAME);

        if (cachedServerVersion != null
            && getServerSession().getServerVersion() != null
    ...
    try {
      ...
      String versionComment =
          (this.propertySet.getBooleanProperty(PropertyKey.paranoid).getValue() || version == null)
              ? ""
              : "/* " + version + " */";
      ...
      if (versionMeetsMinimum(5, 1, 0)) {
        ...
        NativePacketPayload resultPacket =
            (NativePacketPayload)
                this.protocol.sendCommand(
                    this.commandBuilder.buildComQuery(null, versionComment + "SHOW VARIABLES"),
                    false,
                    0);
        Resultset rs =
            ((NativeProtocol) this.protocol)
        ...
        while ((r = rs.getRows().next()) != null) {
          this.protocol
              .getServerSession()
              .getServerVariables()
              .put(r.getValue(0, vf), r.getValue(1, vf));
        }
      }
    } catch (IOException e) {
    ...
    if (this.cacheServerConfiguration.getValue()) {
      this.protocol
          .getServerSession()
          .getServerVariables()
          .put(SERVER_VERSION_STRING_VAR_NAME, getServerSession().getServerVersion().toString());
      Map<String, String> localVariableMap = new HashMap<>();
      localVariableMap.putAll(this.protocol.getServerSession().getServerVariables());
      this.serverConfigCache.put(
          this.hostInfo.getDatabaseUrl(), Collections.unmodifiableMap(localVariableMap));
    }
  }

  public void setSessionVariables() {
    String sessionVariables =
        getPropertySet().getStringProperty(PropertyKey.sessionVariables).getValue();
    if (sessionVariables != null) {
      ...
      if (!variablesToSet.isEmpty()) {
        ...
        this.protocol.sendCommand(
            this.commandBuilder.buildComQuery(null, query.toString()), false, 0);
      }
    }
  }
  ...
  public String getProcessHost() {
    try {
      ...
      if (processHost == null) {
        // http://bugs.mysql.com/bug.php?id=44167 - connection ids on the wire wrap at 4 bytes even
        // though they're 64-bit numbers
        this.log.logWarn(
            String.format(
                "Connection id %d not found in \"SHOW PROCESSLIST\", assuming 32-bit overflow,"
                    + " using SELECT CONNECTION_ID() instead",
                threadId));
        ...
        if ((r = rs.getRows().next()) != null) {
          ...
          this.log.logError(
              "No rows returned for statement \"SELECT CONNECTION_ID()\", local connection check"
      ...
      if (processHost == null) {
        this.log.logWarn(
            String.format(
                "Cannot find process listing for connection %d in SHOW PROCESSLIST output, unable"
                    + " to determine if locally connected",
                threadId));
      }
      return processHost;
    } catch (IOException e) {
  ...
  private String findProcessHost(long threadId) {
    try {
      ...
      NativePacketPayload resultPacket =
          versionMeetsMinimum(5, 6, 0) // performance_schema.threads in MySQL 5.5 does not contain
                  // PROCESSLIST_HOST column
                  && ps != null
                  && ("1".contentEquals(ps) || "ON".contentEquals(ps))
              ? (NativePacketPayload)
                  this.protocol.sendCommand(
                      this.commandBuilder.buildComQuery(
                          null,
                          "select PROCESSLIST_ID, PROCESSLIST_USER, PROCESSLIST_HOST from"
              ...
              : (NativePacketPayload)
                  this.protocol.sendCommand(
                      this.commandBuilder.buildComQuery(null, "SHOW PROCESSLIST"), false, 0);
  ...
  public String queryServerVariable(String varName) {
    try {
      ...
      NativePacketPayload resultPacket =
          (NativePacketPayload)
              this.protocol.sendCommand(
                  this.commandBuilder.buildComQuery(null, "SELECT " + varName), false, 0);
      Resultset rs =
          ((NativeProtocol) this.protocol)
              .readAllResults(
                  -1,
  ...
  public <T extends Resultset> T execSQL(
    ...
    if (this.autoReconnect.getValue()
        && (getServerSession().isAutoCommit() || this.autoReconnectForPools.getValue())
        && this.needsPing
        && !isBatch) {
      try {
        ping(false, 0);
      ...
      return packet == null
          ? ((NativeProtocol) this.protocol)
              .sendQueryString(
                  callingQuery,
                  query,
                  this.characterEncoding.getValue(),
                  maxRows,
                  streamResults,
                  cachedMetadata,
                  resultSetFactory)
          : ((NativeProtocol) this.protocol)
              .sendQueryPacket(
                  callingQuery, packet, maxRows, streamResults, cachedMetadata, resultSetFactory);

    } catch (CJException sqlE) {
      if (getPropertySet().getBooleanProperty(PropertyKey.dumpQueriesOnException).getValue()) {
        String extractedSql =
            NativePacketPayload.extractSqlFromPacket(
                query,
                packet,
  ...
  public long getIdleFor() {
    return this.lastQueryFinishedTime == 0
        ? 0
        : System.currentTimeMillis() - this.lastQueryFinishedTime;
  }
  ...
  public void ping(boolean checkForClosedConnection, int timeoutMillis) {
    ...
    long pingMillisLifetime =
        getPropertySet()
            .getIntegerProperty(PropertyKey.selfDestructOnPingSecondsLifetime)
            .getValue();
    int pingMaxOperations =
        getPropertySet().getIntegerProperty(PropertyKey.selfDestructOnPingMaxOperations).getValue();

    if ((pingMillisLifetime > 0
            && (System.currentTimeMillis() - this.connectionCreationTimeMillis)
                > pingMillisLifetime)
      ...
      throw ExceptionFactory.createException(
          Messages.getString("Connection.exceededConnectionLifetime"),
    ...
    this.protocol.sendCommand(
        this.commandBuilder.buildComPing(null),
        false,
        timeoutMillis); // it isn't safe to use a shared packet here
  }
