...
public class DefaultPropertySet implements PropertySet, Serializable {
  ...
  public DefaultPropertySet() {
    for (PropertyDefinition<?> pdef :
        PropertyDefinitions.PROPERTY_KEY_TO_PROPERTY_DEFINITION.values()) {
  ...
  public <T> RuntimeProperty<T> getProperty(PropertyKey key) {
    try {
      ...
      // for some of PropertyKey values we don't have property definitions, thus they are cached as
      // custom properties
      if (prop == null) {
  ...
  public void initializeProperties(Properties props) {
    if (props != null) {
      ...
      // Translate legacy SSL properties if sslMode isn't explicitly set. Default sslMode is
      // PREFERRED.
      ...
      if (!sslMode.isExplicitlySet()) {
        ...
        RuntimeProperty<Boolean> verifyServerCertificate =
            this.getBooleanProperty(PropertyKey.verifyServerCertificate);
        RuntimeProperty<Boolean> requireSSL = this.getBooleanProperty(PropertyKey.requireSSL);
        if (useSSL.isExplicitlySet()
            || verifyServerCertificate.isExplicitlySet()
            || requireSSL.isExplicitlySet()) {
          if (!useSSL.getValue()) {
      ...
      for (Object key : infoCopy.keySet()) {
        ...
        PropertyDefinition<String> def =
            new StringPropertyDefinition(
                ...
                null,
                val,
                PropertyDefinitions.RUNTIME_MODIFIABLE,
