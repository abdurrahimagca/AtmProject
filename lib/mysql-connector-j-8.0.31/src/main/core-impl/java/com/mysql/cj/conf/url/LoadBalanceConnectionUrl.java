...
public class LoadBalanceConnectionUrl extends ConnectionUrl {
   ...
   * initializations and validations. A load-balanced connection cannot deal with multiple hosts
   * with same host:port.
   *
  ...
  public LoadBalanceConnectionUrl(ConnectionUrlParser connStrParser, Properties info) {
    ...
    // creating load-balanced connections
    ...
    //                        Messages.getString("ConnectionString.12", new Object[] {
    // hi.getHostPortPair(), Type.LOADBALANCE_CONNECTION.getProtocol() }));
    //            }
   ...
   * global set of properties instead of connection string parsing. {@link ConnectionUrl} instances
   * created by this process are not cached.
   *
  ...
  public LoadBalanceConnectionUrl(List<HostInfo> hosts, Map<String, String> properties) {
    this.originalConnStr =
        ConnectionUrl.Type.LOADBALANCE_CONNECTION.getScheme()
  ...
  protected void injectPerTypeProperties(Map<String, String> props) {
    if (props.containsKey(PropertyKey.loadBalanceAutoCommitStatementThreshold.getKeyName())) {
      try {
        int autoCommitSwapThreshold =
            Integer.parseInt(
                props.get(PropertyKey.loadBalanceAutoCommitStatementThreshold.getKeyName()));
