...
public class ReplicationConnectionUrl extends ConnectionUrl {
  ...
  @Deprecated private static final String TYPE_SOURCE_DEPRECATED = "MASTER";
  @Deprecated private static final String TYPE_REPLICA_DEPRECATED = "SLAVE";
  ...
  public ReplicationConnectionUrl(ConnectionUrlParser connStrParser, Properties info) {
    ...
    for (HostInfo hi : this.hosts) {
      ...
      if (hostProperties.containsKey(PropertyKey.TYPE.getKeyName())) {
        if (TYPE_SOURCE.equalsIgnoreCase(hostProperties.get(PropertyKey.TYPE.getKeyName()))
            || TYPE_SOURCE_DEPRECATED.equalsIgnoreCase(
                hostProperties.get(PropertyKey.TYPE.getKeyName()))) {
          this.sourceHosts.add(hi);
        } else if (TYPE_REPLICA.equalsIgnoreCase(hostProperties.get(PropertyKey.TYPE.getKeyName()))
            || TYPE_REPLICA_DEPRECATED.equalsIgnoreCase(
                hostProperties.get(PropertyKey.TYPE.getKeyName()))) {
    ...
    // Although this should be required, it also is incompatible with our current tests which are
    ...
    //                            Messages.getString("ConnectionString.13", new Object[] {
    // hi.getHostPortPair(), Type.REPLICATION_CONNECTION.getProtocol() }));
    //                }
  ...
  /**
   * Constructs an instance of a {@link ReplicationConnectionUrl} based on a list of source hosts, a
   * list of replica hosts and a global set of properties instead of connection string parsing.
   ...
   */
  public ReplicationConnectionUrl(
      List<HostInfo> sources, List<HostInfo> replicas, Map<String, String> properties) {
    this.originalConnStr =
        ConnectionUrl.Type.REPLICATION_CONNECTION.getScheme()
            + "//**internally_generated**"
            + System.currentTimeMillis()
            + "**";
    this.originalDatabase =
        properties.containsKey(PropertyKey.DBNAME.getKeyName())
            ? properties.get(PropertyKey.DBNAME.getKeyName())
            : "";
    ...
    sources.stream()
        ...
        .forEach(
            this.hosts::add); // Fix the hosts info based on the new properties before adding them.
    replicas.stream()
        .map(this::fixHostInfo)
        .peek(this.replicaHosts::add)
        .forEach(
            this.hosts::add); // Fix the hosts info based on the new properties before adding them.
  }
  ...
  /**
   * Returns an existing source host info with the same host:port part or spawns a new isolated host
   * info based on this connection URL if none was found.
   *
   ...
   * pairs in the corresponding hosts list. Isolated host info elements are spawned for the missing
   * elements.
   *
  ...
  public List<HostInfo> getSourceHostsListFromHostPortPairs(Collection<String> hostPortPairs) {
    return hostPortPairs.stream()
        .map(this::getSourceHostOrSpawnIsolated)
        .collect(Collectors.toList());
  }

  /**
   * Returns an existing replica host info with the same host:port part or spawns a new isolated
  ...
  public List<HostInfo> getReplicaHostsListFromHostPortPairs(Collection<String> hostPortPairs) {
    return hostPortPairs.stream()
        .map(this::getReplicaHostOrSpawnIsolated)
        .collect(Collectors.toList());
  }
}
