...
public class ReplicationDnsSrvConnectionUrl extends ConnectionUrl {
  ...
  /**
   * Constructs an instance of {@link ReplicationDnsSrvConnectionUrl}, performing all the required
   * initializations.
   *
  ...
  public ReplicationDnsSrvConnectionUrl(ConnectionUrlParser connStrParser, Properties info) {
    ...
    Map<String, String> hostPropsSource =
        srvHostSource == null ? Collections.emptyMap() : srvHostSource.getHostProperties();
    HostInfo srvHostReplica = this.replicaHosts.isEmpty() ? null : this.replicaHosts.get(0);
    Map<String, String> hostPropsReplica =
        srvHostReplica == null ? Collections.emptyMap() : srvHostReplica.getHostProperties();
    if (srvHostSource == null
        || srvHostReplica == null
        || DEFAULT_HOST.equals(srvHostSource.getHost())
        || DEFAULT_HOST.equals(srvHostReplica.getHost())) {
    ...
    if (hostPropsSource.containsKey(PropertyKey.dnsSrv.getKeyName())
        || hostPropsReplica.containsKey(PropertyKey.dnsSrv.getKeyName())) {
      if (!BooleanPropertyDefinition.booleanFrom(
              PropertyKey.dnsSrv.getKeyName(),
              hostPropsSource.get(PropertyKey.dnsSrv.getKeyName()),
              null)
          || !BooleanPropertyDefinition.booleanFrom(
              PropertyKey.dnsSrv.getKeyName(),
              hostPropsReplica.get(PropertyKey.dnsSrv.getKeyName()),
              null)) {
        throw ExceptionFactory.createException(
    ...
    if (hostPropsSource.containsKey(PropertyKey.PROTOCOL.getKeyName())
            && hostPropsSource.get(PropertyKey.PROTOCOL.getKeyName()).equalsIgnoreCase("PIPE")
    ...
    if (hostPropsSource.containsKey(PropertyKey.replicationConnectionGroup.getKeyName())
      ...
      throw ExceptionFactory.createException(
          ...
          Messages.getString(
              "ConnectionString.25",
              new Object[] {PropertyKey.replicationConnectionGroup.getKeyName()}));
    }
  }
