  ...
  public XDevApiConnectionUrl(ConnectionUrlParser connStrParser, Properties info) {
    ...
    for (HostInfo hi : this.hosts) {
      if (first) {
        ...
        if (!StringUtils.nullSafeEqual(user, hi.getUser())
            || !StringUtils.nullSafeEqual(password, hi.getPassword())) {
          throw ExceptionFactory.createException(
              WrongArgumentException.class,
              Messages.getString(
                  "ConnectionString.14", new Object[] {Type.XDEVAPI_SESSION.getScheme()}));
        }
        if (hasPriority ^ hi.getHostProperties().containsKey(PropertyKey.PRIORITY.getKeyName())) {
          throw ExceptionFactory.createException(
              WrongArgumentException.class,
              Messages.getString(
                  "ConnectionString.15", new Object[] {Type.XDEVAPI_SESSION.getScheme()}));
        }
      }
      if (hasPriority) {
        try {
          ...
          if (priority < 0 || priority > 100) {
            throw ExceptionFactory.createException(
                WrongArgumentException.class,
                Messages.getString(
                    "ConnectionString.16", new Object[] {Type.XDEVAPI_SESSION.getScheme()}));
          }
  ...
  public List<HostInfo> getHostsList(HostsListView view) {
    if (this.prioritySorted) {
      if (this.hasDuplicatedPriorities) { // Randomly sort hosts with same priority.
        Map<Integer, List<HostInfo>> hostsByPriority =
            this.hosts.stream()
                .collect(
                    Collectors.groupingBy(
                        hi ->
                            Integer.valueOf(
                                hi.getHostProperties().get(PropertyKey.PRIORITY.getKeyName()))));
        this.hosts =
            hostsByPriority.entrySet().stream()
                .sorted(
                    Comparator.<Map.Entry<Integer, List<HostInfo>>, Integer>comparing(Entry::getKey)
                        .reversed())
                .map(Entry::getValue)
