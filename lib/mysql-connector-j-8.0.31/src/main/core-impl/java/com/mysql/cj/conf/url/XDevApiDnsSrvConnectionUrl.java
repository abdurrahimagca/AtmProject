...
public class XDevApiDnsSrvConnectionUrl extends ConnectionUrl {
  ...
  /**
   * Constructs an instance of {@link XDevApiDnsSrvConnectionUrl}, performing all the required
   * initializations.
   *
  ...
  public XDevApiDnsSrvConnectionUrl(ConnectionUrlParser connStrParser, Properties info) {
    ...
    if (hostProps.containsKey(PropertyKey.xdevapiDnsSrv.getKeyName())) {
      if (!BooleanPropertyDefinition.booleanFrom(
          PropertyKey.xdevapiDnsSrv.getKeyName(),
          hostProps.get(PropertyKey.xdevapiDnsSrv.getKeyName()),
          null)) {
        throw ExceptionFactory.createException(
            InvalidConnectionAttributeException.class,
            Messages.getString(
                "ConnectionString.23", new Object[] {PropertyKey.xdevapiDnsSrv.getKeyName()}));
      }
    }
  }
