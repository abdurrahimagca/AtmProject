...
public class BaseMetricsHolder {
  ...
  private void addToTablesAccessedHistogram(long value, int numberOfTimes) {
    ...
    addToHistogram(
        this.numTablesMetricsHistCounts,
  ...
  private void repartitionHistogram(
      int[] histCounts, long[] histBreakpoints, long currentLowerBound, long currentUpperBound) {
    ...
    for (int i = 0; i < HISTOGRAM_BUCKETS; i++) {
      addToHistogram(
          histCounts,
          histBreakpoints,
          this.oldHistBreakpoints[i],
          this.oldHistCounts[i],
          currentLowerBound,
          currentUpperBound);
    }
  }
  ...
  public void reportMetrics(Log log) {
    ...
    if (this.numTablesMetricsHistBreakpoints != null) {
      ...
      for (int i = 0; i < (HISTOGRAM_BUCKETS - 1); i++) {
        ...
        int numPointsToGraph =
            (int) (maxNumPoints * ((double) this.numTablesMetricsHistCounts[i] / highestCount));
      ...
      if (this.numTablesMetricsHistBreakpoints[HISTOGRAM_BUCKETS - 2]
          < this.maximumNumberTablesAccessed) {
        logMessage.append("\n\tbetween ");
  ...
  public void incrementNumberOfPreparedExecutes() {
    ...
    // We need to increment this, because server-side prepared statements bypass any execution by
  ...
  public void reportQueryTime(long millisOrNanos) {
    ...
    this.queryTimeMean =
        ((this.queryTimeMean * (this.queryTimeCount - 1)) + millisOrNanos) / this.queryTimeCount;
  }
   ...
   *
   * <p>Used in case autoSlowLog=true.
   *
  ...
  public boolean checkAbonormallyLongQuery(long millisOrNanos) {
    ...
    if (this.queryTimeCount > 14) { // need a minimum amount for this to make sense
      double stddev =
          Math.sqrt(
              (this.queryTimeSumSquares
                      - ((this.queryTimeSum * this.queryTimeSum) / this.queryTimeCount))
