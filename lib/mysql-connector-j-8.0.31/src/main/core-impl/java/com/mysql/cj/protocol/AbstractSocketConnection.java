...
public abstract class AbstractSocketConnection implements SocketConnection {
  ...
  // We do this to break the chain between MysqlIO and Connection, so that we can have
  // PhantomReferences on connections that let the driver clean up the
  // socket connection without having to use finalize() somewhere (which although more
  // straightforward, is horribly inefficient).
    ...
    try {
      if (socketFactoryClassName == null) {
        throw ExceptionFactory.createException(
            ...
            Messages.getString("SocketConnection.0"),
      ...
      throw ExceptionFactory.createException(
          UnableToConnectException.class,
          Messages.getString("SocketConnection.1", new String[] {socketFactoryClassName}),
          getExceptionInterceptor());
    }
  }
}
