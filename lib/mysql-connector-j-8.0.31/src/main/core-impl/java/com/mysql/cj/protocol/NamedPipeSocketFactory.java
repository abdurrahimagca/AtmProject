...
public class NamedPipeSocketFactory implements SocketFactory {
        ...
        try {
          ...
          if (timeout == 0) { // No timeout was set.
            throw new IOException(
                "Named pipe busy error (ERROR_PIPE_BUSY).\n"
                    + "Consider setting a value for 'connectTimeout' or"
                    + " DriverManager.setLoginTimeout(int) to repeatedly try opening the named pipe"
                    + " before failing.",
                e);
          }
  ...
  public <T extends Closeable> T connect(
      String host, int portNumber /* ignored */, PropertySet props, int loginTimeout)
      throws IOException {
    ...
    if (namedPipePath == null) {
      ...
      throw new SocketException(
          Messages.getString("NamedPipeSocketFactory.2")
              + PropertyKey.PATH.getCcAlias()
              + Messages.getString("NamedPipeSocketFactory.3"));
    }

    int connectTimeout =
        props.getIntegerProperty(PropertyKey.connectTimeout.getKeyName()).getValue();
    int timeout =
        (connectTimeout > 0 && loginTimeout > 0)
            ? Math.min(connectTimeout, loginTimeout)
