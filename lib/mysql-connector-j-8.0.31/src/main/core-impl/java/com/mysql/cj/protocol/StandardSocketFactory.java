...
public class StandardSocketFactory implements SocketFactory {
   ...
   */
  protected void configureSocket(Socket sock, PropertySet pset)
  ...
  public <T extends Closeable> T connect(
    ...
    if (pset != null) {
      ...
      String localSocketHostname =
          pset.getStringProperty(PropertyKey.localSocketAddress).getValue();
      InetSocketAddress localSockAddr =
          localSocketHostname != null && localSocketHostname.length() > 0
  ...
  public <T extends Closeable> T performTlsHandshake(
    ...
    this.sslSocket =
        ExportControlled.performTlsHandshake(
            this.rawSocket,
            socketConnection,
  ...
  protected int getRealTimeout(int expectedTimeout) {
    if (this.loginTimeoutCountdown > 0
        && (expectedTimeout == 0 || expectedTimeout > this.loginTimeoutCountdown)) {
