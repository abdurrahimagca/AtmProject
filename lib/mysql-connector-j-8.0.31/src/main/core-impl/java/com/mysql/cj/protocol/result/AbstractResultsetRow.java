...
public abstract class AbstractResultsetRow implements ResultsetRow {
   ...
   * @return value
   */
  private <T> T decodeAndCreateReturnValue(
      int columnIndex, byte[] bytes, int offset, int length, ValueFactory<T> vf) {
    Field f = this.metadata.getFields()[columnIndex];

    // First, figure out which decoder method to call basing on the protocol value type from
    // metadata;
    // it's the best way to find the appropriate decoder, we can't rely completely on MysqlType here
    // because the same MysqlType can be represented by different protocol types and also
    // DatabaseMetaData methods,
    // eg. buildResultSet(), could imply unexpected conversions when substitutes RowData in
    // ResultSet;
    switch (f.getMysqlTypeId()) {
      ...
      case MysqlType.FIELD_TYPE_TINY:
        return f.isUnsigned()
            ? this.valueDecoder.decodeUInt1(bytes, offset, length, vf)
            : this.valueDecoder.decodeInt1(bytes, offset, length, vf);
      ...
      case MysqlType.FIELD_TYPE_SHORT:
        return f.isUnsigned()
            ? this.valueDecoder.decodeUInt2(bytes, offset, length, vf)
            : this.valueDecoder.decodeInt2(bytes, offset, length, vf);

      case MysqlType.FIELD_TYPE_LONG:
        return f.isUnsigned()
            ? this.valueDecoder.decodeUInt4(bytes, offset, length, vf)
            : this.valueDecoder.decodeInt4(bytes, offset, length, vf);
      ...
      case MysqlType.FIELD_TYPE_LONGLONG:
        return f.isUnsigned()
            ? this.valueDecoder.decodeUInt8(bytes, offset, length, vf)
            : this.valueDecoder.decodeInt8(bytes, offset, length, vf);
    ...
    // If the protocol type isn't available then select decoder basing on MysqlType; that's for some
    // internal
    // code that creates rows without MySQL protocol types, only MysqlType types, including PS
    // bindings as RS, DBMD
  ...
  /**
   * Get a value from a byte array. The byte array is interpreted by the {@link
   * com.mysql.cj.protocol.ValueDecoder} which uses the value factory create the return value.
   *
   ...
   * @return value
   */
  protected <T> T getValueFromBytes(
      int columnIndex, byte[] bytes, int offset, int length, ValueFactory<T> vf) {
    if (getNull(columnIndex)) {
      return vf.createFromNull();
    }

    // value factory may return null for zeroDateTimeBehavior=CONVERT_TO_NULL so check the return
    // value
