...
public class DefaultColumnDefinition implements ColumnDefinition {
  ...
  public void buildIndexMapping() {
    ...
    // We do this in reverse order, so that the 'first' column with a given name ends up as the
    // final mapping in the hashtable...
    //
    // Quoting the JDBC Spec:
    //
    // "Column names used as input to getter methods are case insensitive. When a getter method is
    // called with a column name and several columns have the
   ...
   * this ColumnDefinition. This check is used for making a decision about whether we want to force
   * a buffer row (better for rows with large fields).
   *
