...
/**
 * A {@link SaslClient} implementation for SCRAM-SHA mechanisms as specified in <a
 * href="https://tools.ietf.org/html/rfc5802">RFC 5802</a>. Subclasses of this class must implement
 * the hashing algorithms for the corresponding authentication mechanism.
 */
public abstract class ScramShaSaslClient implements SaslClient {
  ...
  public ScramShaSaslClient(String authorizationId, String authenticationId, String password)
      throws SaslException {
    this.authorizationId = StringUtils.isNullOrEmpty(authorizationId) ? "" : authorizationId;
    this.authenticationId =
        StringUtils.isNullOrEmpty(authenticationId) ? this.authorizationId : authenticationId;
  ...
  /**
   * Returns the real IANA-registered mechanism name of this SASL client. This is the same as {@link
   * SaslClient#getMechanismName()} except that subclasses may use custom mechanism names to avoid
   * future name clashes.
   *
      ...
      switch (this.scramStage) {
        case CLIENT_FIRST: // Process client-first-message.
          ...
          this.gs2Header =
              GS2_CBIND_FLAG
                  + ","
                  + (StringUtils.isNullOrEmpty(this.authorizationId)
                      ? ""
                      : "a=" + prepUserName(this.authorizationId))
                  + ",";
          this.cNonce = generateRandomPrintableAsciiString(32);
          this.clientFirstMessageBare =
              "n=" + prepUserName(this.authenticationId) + ",r=" + this.cNonce;
        ...
        case SERVER_FIRST_CLIENT_FINAL: // Process server-first-message & client-final-message.
          ...
          if (!serverFirstAttributes.containsKey("r")
              || !serverFirstAttributes.containsKey("s")
              || !serverFirstAttributes.containsKey("i")) {
          ...
          if (!sNonce.startsWith(this.cNonce)) {
            throw new SaslException(
                "Invalid server nonce for " + getIanaMechanismName() + " authentication.");
          }
          ...
          if (iterations < MINIMUM_ITERATIONS) {
            throw new SaslException(
                "Announced " + getIanaMechanismName() + " iteration count is too low.");
          }
          ...
          String clientFinalMessageWithoutProof =
              "c="
                  + Base64.getEncoder()
                      .encodeToString(StringUtils.getBytes(this.gs2Header, "UTF-8"))
                  + ",r="
                  + sNonce;
          ...
          byte[] saltedPassword =
              hi(SaslPrep.prepare(this.password, StringType.STORED), salt, iterations);
        ...
        case SERVER_FINAL: // Process server-final-message.
          ...
          if (!MessageDigest.isEqual(this.serverSignature, verifier)) {
            throw new SaslException(
                getIanaMechanismName() + " server signature could not be verified.");
          }
          break;
  ...
  /**
   * The "H(str)" cryptographic hash function as described in <a
   * href="https://tools.ietf.org/html/rfc5802#section-2.2">RFC 5802, Section 2.2</a>.
   *
  ...
  /**
   * The "HMAC(key, str)" HMAC keyed hash algorithm as described in <a
   * href="https://tools.ietf.org/html/rfc5802#section-2.2">RFC 5802, Section 2.2</a>.
   *
  ...
  /**
   * The "Hi(str, salt, i)" PBKDF2 function as described in <a
   * href="https://tools.ietf.org/html/rfc5802#section-2.2">RFC 5802, Section 2.2</a>.
   *
