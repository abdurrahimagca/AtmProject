...
/**
 * A {@link SaslClientFactory} for {@link ScramSha1SaslClient} and {@link ScramSha256SaslClient}
 * instances.
 */
public class ScramShaSaslClientFactory implements SaslClientFactory {
  private static final String[] SUPPORTED_MECHANISMS = {
    ScramSha1SaslClient.MECHANISM_NAME, ScramSha256SaslClient.MECHANISM_NAME
  };
    ...
    for (String mech : mechanisms) {
      if (mech.equals(ScramSha1SaslClient.MECHANISM_NAME)) {
        return new ScramSha1SaslClient(
            authorizationId, getUsername(mech, authorizationId, cbh), getPassword(mech, cbh));
      }
      if (mech.equals(ScramSha256SaslClient.MECHANISM_NAME)) {
        return new ScramSha256SaslClient(
            authorizationId, getUsername(mech, authorizationId, cbh), getPassword(mech, cbh));
      }
    }
    return null;
  ...
  /**
   * Gets the authentication id, which is provided by a {@link CallbackHandler} to be implemented by
   * the requester of this service.
   *
   * @param prefix the prefix to use in the prompt.
   * @param authorizationId the authorization id that can be used as default authentication id if
   *     none is provided.
   * @param cbh the callback handler to use.
   * @return an authentication id
   * @throws SaslException if the callback handler is null or not supported by the callback handler
   *     implementer.
   */
  private String getUsername(String prefix, String authorizationId, CallbackHandler cbh)
      throws SaslException {
    if (cbh == null) {
    ...
    try {
      ...
      NameCallback ncb =
          StringUtils.isNullOrEmpty(authorizationId)
              ? new NameCallback(prompt)
              : new NameCallback(prompt, authorizationId);
  ...
  /**
   * Gets the password, which is provided by a {@link CallbackHandler} to be implemented by the
   * requester of this service.
   *
   * @param prefix the prefix to use in the prompt.
   * @param cbh the callback handler to use.
   * @return a password
