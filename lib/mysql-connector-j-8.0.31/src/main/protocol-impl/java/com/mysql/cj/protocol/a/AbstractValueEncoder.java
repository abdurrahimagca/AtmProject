...
public abstract class AbstractValueEncoder implements ValueEncoder {
  ...
  public void init(
    ...
    this.sendFractionalSecondsForTime =
        pset.getBooleanProperty(PropertyKey.sendFractionalSecondsForTime);
    ...
    try {
      ...
      try {
        ...
        throw ExceptionFactory.createException(
            ...
            Messages.getString(
                "PreparedStatement.65", new Object[] {scaleOrLength, x.toPlainString()}),
  ...
  protected LocalTime adjustLocalTime(LocalTime x, Field f) {
    ...
    return TimeUtil.adjustNanosPrecision(
        x, f == null ? 6 : f.getDecimals(), !this.serverSession.isServerTruncatesFracSecs());
  }
  ...
  protected void writeTime(Message msg, InternalTime time) {
    ...
    intoPacket.ensureCapacity(
        (hasFractionalSeconds
                ? NativeConstants.BIN_LEN_TIME_WITH_MICROS
                : NativeConstants.BIN_LEN_TIME_NO_FRAC)
            + 1);
  ...
  protected void writeDateTime(Message msg, InternalTimestamp ts) {
    ...
    intoPacket.ensureCapacity(
        (microseconds > 0
                ? NativeConstants.BIN_LEN_TIMESTAMP_WITH_MICROS
                : NativeConstants.BIN_LEN_TIMESTAMP_NO_FRAC)
            + 1);
    intoPacket.writeInteger(
        IntegerDataType.INT1,
        microseconds > 0
  ...
  public void writeDateTimeWithOffset(Message msg, InternalTimestamp timestamp) {
    ...
    intoPacket.writeInteger(
        IntegerDataType.INT4, TimeUnit.NANOSECONDS.toMicros(timestamp.getNanos()));
  ...
  protected byte[] escapeBytesIfNeeded(byte[] x) {
    if (this.serverSession.isNoBackslashEscapesSet()
        || this.serverSession
            .getCharsetSettings()
            .isMultibyteCharset(this.charEncoding.getValue())) {
  ...
  public long getTextLength(BindValue binding) {
    ...
    return binding.isStream() && binding.getScaleOrLength() != -1
        ? binding.getScaleOrLength() * 2 /* for safety in escaping */
