  ...
  public Resultset read(
    ...
    if (columnCount > 0) {
      ...
      ColumnDefinition cdef =
          this.protocol.read(
              ColumnDefinition.class, new MergingColumnDefinitionFactory(columnCount, metadata));

      boolean isCursorPossible =
          this.protocol.getPropertySet().getBooleanProperty(PropertyKey.useCursorFetch).getValue()
      ...
      // 1. If CLIENT_DEPRECATE_EOF is not set then an EOF packet is always expected to be the next
      // one.
      // 2. If CLIENT_DEPRECATE_EOF is set and a cursor was created then the next packet is an OK
      // with 0xFE signature.
      // 3. If CLIENT_DEPRECATE_EOF is set and a cursor was not created then the next packet is a
      // ProtocolBinary::ResultsetRow.
      // If CLIENT_DEPRECATE_EOF is set, there is no way to tell which one, OK or ResultsetRow, is
      // the next packet, so it should be read with a special caching method.
      if (isCursorPossible || !this.protocol.getServerSession().isEOFDeprecated()) {
        // Read the next packet but leave it in the reader cache. In case it's not the OK or EOF one
        // it will be read again by ResultSet factories.
        NativePacketPayload rowPacket =
            this.protocol.probeMessage(this.protocol.getReusablePacket());
      ...
      if (isCursorPossible && this.protocol.getServerSession().cursorExists()) {
        ...
        BinaryRowFactory brf =
            new BinaryRowFactory(
                this.protocol, cdef, resultSetFactory.getResultSetConcurrency(), false);
      ...
      if (columnCount == NativePacketPayload.NULL_LENGTH) {
        ...
        String fileName =
            resultPacket.readString(
                ...
                this.protocol.getServerSession().getCharsetSettings().doesPlatformDbCharsetMatches()
                    ? null
                    : charEncoding);
      ...
      OkPacket ok =
          this.protocol.readServerStatusForResultSets(
              resultPacket, false); // oldStatus set in sendCommand()
