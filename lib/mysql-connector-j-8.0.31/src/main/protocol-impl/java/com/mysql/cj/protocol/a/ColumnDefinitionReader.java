...
public class ColumnDefinitionReader
  ...
  public ColumnDefinition read(ProtocolEntityFactory<ColumnDefinition, NativePacketPayload> sf) {
    ...
    for (int i = 0; i < columnCount; i++) {
      ...
      fields[i] =
          unpackField(
              fieldPacket,
              this.protocol.getServerSession().getCharsetSettings().getMetadataEncoding());
    }
  ...
  protected Field unpackField(NativePacketPayload packet, String characterSetMetadata) {
    ...
    LazyString databaseName =
        new LazyString(packet.getByteBuffer(), offset, length, characterSetMetadata);
    ...
    short colFlag =
        (short)
            packet.readInteger(
                this.protocol.getServerSession().hasLongColumnInfo()
    ...
    String encoding =
        this.protocol
            .getServerSession()
            .getCharsetSettings()
            .getJavaEncodingForCollationIndex(collationIndex);

    MysqlType mysqlType =
        NativeProtocol.findMysqlType(
    ...
    // Protocol returns precision and scale differently for some types. We need to align then to
    // I_S.
    switch (mysqlType) {
      case TINYINT:
      ...
      case DOUBLE_UNSIGNED:
        // According to SQL standard, NUMERIC_SCALE should be NULL for approximate numeric data
        // types.
