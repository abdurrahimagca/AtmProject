...
public class CompressedInputStream extends InputStream {
  ...
  public CompressedInputStream(
      InputStream streamFromServer, RuntimeProperty<Boolean> traceProtocol, Log log) {
  ...
  private void getNextPacketFromServer() throws IOException {
    ...
    if (uncompressedLength > 0) {
      ...
      //
      // Read data, note this this code is reached when using compressed packets that have not been
      // compressed, as well
      //
  ...
  /**
   * Determines if another packet needs to be read from the server to be able to read numBytes from
   * the stream.
   *
