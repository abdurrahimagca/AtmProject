 ...
 *
 * <p>TODO: add support for pre-allocated buffer for large packets (if there's a demonstrable perf
 * improvement)
 */
public class CompressedPacketSender implements MessageSender<NativePacketPayload> {
  ...
  private void sendCompressedPacket(int uncompressedPayloadLen) throws IOException {
    writeCompressedHeader(
        this.compressedPayloadLen, this.compressedSequenceId++, uncompressedPayloadLen);
  ...
  /**
   * Packet sender implementation for the compressed MySQL protocol. For compressed transmission of
   * multi-packets, split the packets up in the same way as the uncompressed protocol. We fit up to
   * MAX_PACKET_SIZE bytes of split uncompressed packet, including the header, into an compressed
   * packet. The first packet of the multi-packet is 4 bytes of header and MAX_PACKET_SIZE - 4 bytes
   * of the payload. The next packet must send the remaining four bytes of the payload followed by a
   * new header and payload. If the second split packet is also around MAX_PACKET_SIZE in length,
   * then only MAX_PACKET_SIZE - 4 (from the previous packet) - 4 (for the new header) can be sent.
   * This means the payload will be limited by 8 bytes and this will continue to increase by 4 at
   * every iteration.
   *
  ...
  public void send(byte[] packet, int packetLen, byte packetSequence) throws IOException {
    ...
    if (packetLen < MIN_COMPRESS_LEN) {
      writeCompressedHeader(
          packetLen + NativeConstants.HEADER_LENGTH, this.compressedSequenceId, 0);
    ...
    while (true) {
      ...
      if (packetSplitter.nextPacket()) {
        ...
        // if remaining is 0 then we are sending a very huge packet such that are 4-byte header-size
        // carryover from last packet accumulated to the size
        // of a whole packet itself. We don't handle this. Would require 4 million packet segments
        // (64 gigs in one logical packet)
        int len =
            Math.min(remaining, NativeConstants.HEADER_LENGTH + packetSplitter.getPacketLen());
