  ...
  protected byte[] streamToBytes(InputStream in, long length, NativePacketPayload packet) {
    ...
    try {
      ...
      if (packet == null) {
        ...
        packet.writeBytes(
            StringLengthDataType.STRING_FIXED, StringUtils.getBytes(hexEscape ? "x" : "_binary"));
      ...
      while (bcnt > 0) {
        ...
        if (useLength) {
          bcnt =
              Util.readBlock(
                  in, this.streamConvertBuf, lengthLeftToRead, this.exceptionInterceptor);
          if (bcnt > 0) {
