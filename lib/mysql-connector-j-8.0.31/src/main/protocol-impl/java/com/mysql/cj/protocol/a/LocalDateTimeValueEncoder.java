    ...
    switch (binding.getMysqlType()) {
      ...
      case TIME:
        ...
        sb.append(
            adjustLocalDateTime(
                    LocalDateTime.of(
                        ((LocalDateTime) binding.getValue()).toLocalDate(),
                        ((LocalDateTime) binding.getValue()).toLocalTime()),
                    binding.getField())
                .toLocalTime()
            ...
            ((LocalDateTime) binding.getValue())
                .format(
                    this.sendFractionalSeconds.getValue()
                            && ((LocalDateTime) binding.getValue()).getNano() > 0
    ...
    switch (binding.getMysqlType()) {
      case DATE:
        writeDate(
            ...
            InternalDate.from(
                adjustLocalDateTime(
                        LocalDateTime.of(ldt.toLocalDate(), ldt.toLocalTime()), binding.getField())
                    .toLocalDate()));
        return;
      case TIME:
        writeTime(
