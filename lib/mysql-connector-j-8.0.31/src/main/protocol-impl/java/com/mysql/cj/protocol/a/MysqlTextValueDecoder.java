 ...
 *
 * <p>Refer to MySQL documentation for format of values as strings.
 *
 * <p>Numeric values are returned as ASCII (encoding=63/binary).
 */
public class MysqlTextValueDecoder implements ValueDecoder {
  ...
  public static final Pattern TIME_PTRN =
      Pattern.compile("[-]{0,1}\\d{2,3}:\\d{2}:\\d{2}(\\.\\d{1,9})?");
  ...
  public <T> T decodeTimestamp(
      byte[] bytes, int offset, int length, int scale, ValueFactory<T> vf) {
  ...
  public static int getInt(byte[] buf, int offset, int endpos) throws NumberFormatException {
    ...
    if (l < Integer.MIN_VALUE || l > Integer.MAX_VALUE) {
      throw new NumberOutOfRange(
          Messages.getString(
              "StringUtils.badIntFormat",
              new Object[] {StringUtils.toString(buf, offset, endpos - offset)}));
    }
    return (int) l;
  }
  ...
  public static BigInteger getBigInteger(byte[] buf, int offset, int length)
  ...
  public static boolean isDate(String s) {
    return s.length() == DATE_BUF_LEN
        && s.charAt(4) == '-'
        && s.charAt(7) == '-'; // TODO also check proper date parts ranges
  }
  ...
  public static boolean isTimestamp(String s) {
    Pattern DATETIME_PTRN =
        Pattern.compile("\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}(\\.\\d{1,9}){0,1}");
  ...
  public static InternalDate getDate(byte[] bytes, int offset, int length) {
    if (length != DATE_BUF_LEN) {
      throw new DataReadException(
          Messages.getString("ResultSet.InvalidLengthForType", new Object[] {length, "DATE"}));
    }
  ...
  public static InternalTime getTime(byte[] bytes, int offset, int length, int scale) {
    ...
    if (length < TIME_STR_LEN_MIN || length > TIME_STR_LEN_MAX_WITH_MICROS) {
      throw new DataReadException(
          Messages.getString("ResultSet.InvalidLengthForType", new Object[] {length, "TIME"}));
    }
    ...
    if (segmentLen == 0 || bytes[offset + pos + segmentLen] != ':') {
      throw new DataReadException(
          Messages.getString(
              "ResultSet.InvalidFormatForType",
    ...
    // parse seconds field
    for (segmentLen = 0;
        offset + pos + segmentLen < offset + length
            && Character.isDigit((char) bytes[offset + pos + segmentLen]);
        segmentLen++) {
      ;
    }
    if (segmentLen != 2) {
    ...
    if (length > pos) {
      ...
      // additional digit increasing the range by a factor of
  ...
  public static InternalTimestamp getTimestamp(byte[] bytes, int offset, int length, int scale) {
    if (length < TIMESTAMP_STR_LEN_NO_FRAC
        || (length > TIMESTAMP_STR_LEN_WITH_MICROS && length != TIMESTAMP_STR_LEN_WITH_NANOS)) {
      throw new DataReadException(
          Messages.getString("ResultSet.InvalidLengthForType", new Object[] {length, "TIMESTAMP"}));
    } else if (length != TIMESTAMP_STR_LEN_NO_FRAC) {
      // need at least two extra bytes for fractional, '.' and a digit
      if (bytes[offset + TIMESTAMP_STR_LEN_NO_FRAC] != (byte) '.'
          || length < TIMESTAMP_STR_LEN_NO_FRAC + 2) {
        throw new DataReadException(
            Messages.getString(
                "ResultSet.InvalidFormatForType",
    ...
    if (bytes[offset + 4] != (byte) '-'
        || bytes[offset + 7] != (byte) '-'
        || bytes[offset + 10] != (byte) ' '
        || bytes[offset + 13] != (byte) ':'
    ...
    if (length == TIMESTAMP_STR_LEN_WITH_NANOS) {
      ...
      nanos =
          (length == TIMESTAMP_STR_LEN_NO_FRAC) ? 0 : getInt(bytes, offset + 20, offset + length);
      // scale out nanos appropriately. mysql supports up to 6 digits of fractional seconds, each
      // additional digit increasing the range by a factor of
      // 10. one digit is tenths, two is hundreths, etc
