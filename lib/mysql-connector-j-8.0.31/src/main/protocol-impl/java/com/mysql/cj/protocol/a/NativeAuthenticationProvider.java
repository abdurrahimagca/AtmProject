...
public class NativeAuthenticationProvider implements AuthenticationProvider<NativePacketPayload> {
  ...
  /**
   * Contains instances of authentication plugins that implements {@link AuthenticationPlugin}
   * interface. Key values are MySQL protocol plugin names, for example "mysql_native_password" and
  ...
  @Override
  public void init(
      Protocol<NativePacketPayload> prot,
      PropertySet propSet,
  ...
  public void connect(String user, String pass, String db) {
    ...
    StringBuilder fullSeed =
        new StringBuilder(
            authPluginDataLength > 0 ? authPluginDataLength : NativeConstants.SEED_LENGTH);
    ...
    long clientParam =
        capabilityFlags & NativeServerSession.CLIENT_LONG_PASSWORD //
            ...
            | (this.useConnectWithDb
                ? capabilityFlags & NativeServerSession.CLIENT_CONNECT_WITH_DB
                : 0) //
            | (this.propertySet.getBooleanProperty(PropertyKey.useCompression).getValue()
                ? //
                capabilityFlags & NativeServerSession.CLIENT_COMPRESS
                : 0) //
            | (this.propertySet.getBooleanProperty(PropertyKey.allowLoadLocalInfile).getValue()
                    || this.propertySet
                        .getStringProperty(PropertyKey.allowLoadLocalInfileInPath)
                        .isExplicitlySet()
                ? //
            ...
            | (this.propertySet.getBooleanProperty(PropertyKey.interactiveClient).getValue()
                ? //
                capabilityFlags & NativeServerSession.CLIENT_INTERACTIVE
                : 0) //
            | (this.propertySet.<SslMode>getEnumProperty(PropertyKey.sslMode).getValue()
                    != SslMode.DISABLED
                ? //
                capabilityFlags & NativeServerSession.CLIENT_SSL
                : 0) //
            | capabilityFlags
                & NativeServerSession.CLIENT_TRANSACTIONS // Required to get server status values.
            ...
            | (this.propertySet.getBooleanProperty(PropertyKey.allowMultiQueries).getValue()
                ...
                : 0) //
            | capabilityFlags
                & NativeServerSession.CLIENT_MULTI_RESULTS // Always allow multiple result sets.
            | capabilityFlags
                & NativeServerSession
                    .CLIENT_PS_MULTI_RESULTS // Always allow multiple result sets for SSPS.
            | NativeServerSession.CLIENT_PLUGIN_AUTH //
            | (NONE.equals(
                    this.propertySet.getStringProperty(PropertyKey.connectionAttributes).getValue())
                ? //
                0
            ...
            | (this.propertySet.getBooleanProperty(PropertyKey.trackSessionState).getValue()
                ? //
                capabilityFlags & NativeServerSession.CLIENT_SESSION_TRACK
                : 0) //
    ...
    if (buf.isOKPacket()) {
      throw ExceptionFactory.createException(
          Messages.getString("AuthenticationProvider.UnexpectedAuthenticationApproval"),
          getExceptionInterceptor());
    }
   ...
   *
   * <p>Starts by filling the map with instances of the built-in authentication plugins. Then
   * creates instances of plugins listed in the "authenticationPlugins" connection property and adds
   * them to the map too.
   *
   * <p>The key for the map entry is got by {@link AuthenticationPlugin#getProtocolPluginName()}
   * thus it is possible to replace built-in plugins with custom implementations. To do it, the
   * custom plugin should return one of the values "mysql_native_password", "mysql_clear_password",
   * "sha256_password", "caching_sha2_password", "mysql_old_password",
   * "authentication_ldap_sasl_client" or "authentication_kerberos_client" from its own
   * getProtocolPluginName() method.
   */
  @SuppressWarnings("unchecked")
  private void loadAuthenticationPlugins() {
    // default plugin
    RuntimeProperty<String> defaultAuthenticationPluginProp =
        this.propertySet.getStringProperty(PropertyKey.defaultAuthenticationPlugin);
    String defaultAuthenticationPluginValue = defaultAuthenticationPluginProp.getValue();
    if (defaultAuthenticationPluginValue == null
        || "".equals(defaultAuthenticationPluginValue.trim())) {
      throw ExceptionFactory.createException(
          WrongArgumentException.class,
          Messages.getString(
              "AuthenticationProvider.BadDefaultAuthenticationPlugin",
              new Object[] {defaultAuthenticationPluginValue}),
          getExceptionInterceptor());
    }

    // disabled plugins
    String disabledPlugins =
        this.propertySet.getStringProperty(PropertyKey.disabledAuthenticationPlugins).getValue();
    ...
    if (authenticationPluginClasses != null && !"".equals(authenticationPluginClasses.trim())) {
      ...
      for (String className : pluginsToCreate) {
        try {
          pluginsToInit.add(
              (AuthenticationPlugin<NativePacketPayload>) Class.forName(className).newInstance());
        } catch (Throwable t) {
          throw ExceptionFactory.createException(
              WrongArgumentException.class,
              Messages.getString(
                  "AuthenticationProvider.BadAuthenticationPlugin", new Object[] {className}),
              t,
    ...
    for (AuthenticationPlugin<NativePacketPayload> plugin : pluginsToInit) {
      ...
      if (disabledByProtocolName || disabledByClassName) {
        ...
        if (!defaultFound
            && (defaultAuthenticationPluginValue.equals(pluginProtocolName)
                || defaultAuthenticationPluginValue.equals(pluginClassName))) {
          throw ExceptionFactory.createException(
              WrongArgumentException.class,
              Messages.getString(
                  "AuthenticationProvider.BadDisabledAuthenticationPlugin",
        ...
        if (!defaultFound
          ...
          this.clientDefaultAuthenticationPluginExplicitelySet =
              defaultAuthenticationPluginProp.isExplicitlySet();
          defaultFound = true;
        }
      }
    }
  ...
  /**
   * Get an authentication plugin instance from the authentication plugins map by pluginName key. If
   * such plugin is found, its method {@link AuthenticationPlugin#isReusable()} is called and if the
   * value returned is <code>false</code> then a new instance of the plugin is returned otherwise
   * the instance that already exists is returned.
   *
   * <p>If plugin is not found method returns null, in such case the subsequent behavior of
   * handshake process depends on type of last packet received from server: if it was Auth Challenge
   * Packet then handshake will proceed with default plugin, if it was Auth Method Switch Request
   * Packet then handshake will be interrupted with exception.
   *
   * @param pluginName mysql protocol plugin names, for example "mysql_native_password" and
   *     "mysql_old_password" for built-in plugins
   * @return null if plugin is not found or authentication plugin instance initialized with current
  ...
  private void checkConfidentiality(AuthenticationPlugin<?> plugin) {
    if (plugin.requiresConfidentiality()
        && !this.protocol.getSocketConnection().isSSLEstablished()) {
      throw ExceptionFactory.createException(
          Messages.getString(
              "AuthenticationProvider.AuthenticationPluginRequiresSSL",
              new Object[] {plugin.getProtocolPluginName()}),
          getExceptionInterceptor());
    }
  }
   ...
   *
   * <p>This method will use registered authentication plugins as requested by the server.
   *
  ...
  private void proceedHandshakeWithPluggableAuthentication(final NativePacketPayload challenge) {
    ...
    if (pluginName.equals(Sha256PasswordPlugin.PLUGIN_NAME)
        && !pluginName.equals(this.clientDefaultAuthenticationPluginName)
        && !this.protocol.getSocketConnection().isSSLEstablished()
    ...
    // to seed sent by the server during the initial handshake,
    // thus we reuse it here. Servers affected by Bug#70865 will just ignore it and send the Auth
    // Switch.
    ...
    while (0 < counter--) {
       ...
       * call plugin
       */
      plugin.setAuthenticationParameters(
          this.username, skipPassword ? null : getNthFactorPassword(mfaNthFactor));
      ...
      if (firstPacket) {
        NativePacketPayload authData =
            toServer.isEmpty() ? new NativePacketPayload(0) : toServer.get(0);
        if (forChangeUser) {
          // write COM_CHANGE_USER Packet
          lastSent =
              createChangeUserPacket(serverSession, plugin.getProtocolPluginName(), authData);
          this.protocol.send(lastSent, lastSent.getPosition());
        } else {
          // write HandshakeResponse packet
          lastSent =
              createHandshakeResponsePacket(
                  serverSession, plugin.getProtocolPluginName(), authData);
          this.protocol.send(lastSent, lastSent.getPosition());
        }
        firstPacket = false;
        ...
        if (plugin.getProtocolPluginName().equals(pluginName)) {
          ...
          if (plugin == null) {
            throw ExceptionFactory.createException(
                WrongArgumentException.class,
                Messages.getString(
                    "AuthenticationProvider.BadAuthenticationPlugin", new Object[] {pluginName}),
                getExceptionInterceptor());
          }
        }
    ...
    switch (nthFactor) {
      case 1:
        return this.password == null
            ? this.protocol.getPropertySet().getStringProperty(PropertyKey.password1).getValue()
            : this.password;
      case 2:
  ...
  private NativePacketPayload createHandshakeResponsePacket(
      ServerSession serverSession, String pluginName, NativePacketPayload authData) {
    ...
    if ((clientParam & NativeServerSession.CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA) != 0) {
      ...
      last_sent.writeBytes(
          StringSelfDataType.STRING_LENENC, authData.readBytes(StringSelfDataType.STRING_EOF));
    } else {
    ...
    if (this.useConnectWithDb) {
      last_sent.writeBytes(
          StringSelfDataType.STRING_TERM, StringUtils.getBytes(this.database, enc));
    }
  ...
  private NativePacketPayload createChangeUserPacket(
      ServerSession serverSession, String pluginName, NativePacketPayload authData) {
    ...
    // User/Password data
    last_sent.writeBytes(StringSelfDataType.STRING_TERM, StringUtils.getBytes(this.username, enc));
    // 'auth-response-len' is limited to one Byte but, in case of success, COM_CHANGE_USER will be
    // followed by an AuthSwitchRequest anyway
    if (authData.getPayloadLength() < 256) {
      // non-mysql servers may use this information to authenticate without requiring another
      // round-trip
      last_sent.writeInteger(IntegerDataType.INT1, authData.getPayloadLength());
      last_sent.writeBytes(
          StringSelfDataType.STRING_EOF, authData.getByteBuffer(), 0, authData.getPayloadLength());
    } else {
    ...
    last_sent.writeInteger(
        IntegerDataType.INT1, 0); // two (little-endian) bytes for charset in this packet
