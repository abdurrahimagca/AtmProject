    ...
    try {
      this.serverVersion =
          ServerVersion.parseVersion(
              initialHandshakePacket.readString(StringSelfDataType.STRING_TERM, "ASCII"));
      ...
      this.serverDefaultCollationIndex =
          (int) initialHandshakePacket.readInteger(IntegerDataType.INT1);
      ...
      // X Protocol message type byte (NOTICE = 11) coincides with MySQL protocol version byte in
      // the Initial Handshake Packet.
      if (this.protocolVersion == 11
          && IndexOutOfBoundsException.class.isAssignableFrom(t.getClass())) {
