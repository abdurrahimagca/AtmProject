 ...
 * etc predicates.
 *
 * <p>A position is maintained for reading/writing data. A payload length is maintained allowing the
...
public class NativePacketPayload implements Message {
  ...
  public String toString() {
    ...
    String dumped =
        StringUtils.dumpAsHex(
            readBytes(StringLengthDataType.STRING_FIXED, numBytesToDump), numBytesToDump);
  ...
  /**
   * Get the actual length of payload the buffer contains. It can be smaller than underlying buffer
   * size because it can be reused after a big packet.
   *
  ...
  public void setPayloadLength(int bufLengthToSet) {
    if (bufLengthToSet > this.byteBuffer.length) {
      throw ExceptionFactory.createException(
          WrongArgumentException.class, Messages.getString("Buffer.0"));
    }
  ...
  /**
   * Is it a Protocol::AuthSwitchRequest packet. See
  ...
  public final boolean isResultSetOKPacket() {
    return (this.byteBuffer[0] & 0xff) == TYPE_ID_EOF
        && (this.payloadLength > 5)
        && (this.payloadLength < 16777215);
  }

  /**
   * Is it a Protocol::AuthMoreData packet. See
  ...
  /**
   * Is it a Protocol::AuthNextFactor packet. See
  ...
  public byte[] readBytes(StringSelfDataType type) {
    ...
    switch (type) {
      ...
      case STRING_LENENC:
        ...
        return l == NULL_LENGTH
            ? null
            : (l == 0
                ? Constants.EMPTY_BYTE_ARRAY
                : readBytes(StringLengthDataType.STRING_FIXED, (int) l));
  ...
  /**
   * Set position to next value in internal buffer skipping the current value according to {@link
   * StringSelfDataType}.
   *
   * @param type {@link StringSelfDataType}
   */
  ...
  /**
   * Read bytes from internal buffer starting from current position decoding them into String using
   * the specified character encoding. The length of data to read depends on {@link
   * StringSelfDataType}.
   *
   * @param type {@link StringSelfDataType}
  ...
  public String readString(StringSelfDataType type, String encoding) {
    ...
    switch (type) {
      ...
      case STRING_LENENC:
        ...
        return l == NULL_LENGTH
            ? null
            : (l == 0 ? "" : readString(StringLengthDataType.STRING_FIXED, encoding, (int) l));

      case STRING_EOF:
        return readString(
            StringLengthDataType.STRING_FIXED, encoding, this.payloadLength - this.position);
    }
    return res;
  }
    ...
    switch (type) {
      ...
      case STRING_VAR:
        if ((this.position + len) > this.payloadLength) {
          throw ExceptionFactory.createException(
              WrongArgumentException.class, Messages.getString("Buffer.1"));
        }
