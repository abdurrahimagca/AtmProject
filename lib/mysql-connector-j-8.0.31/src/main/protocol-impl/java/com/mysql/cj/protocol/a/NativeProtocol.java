...
public class NativeProtocol extends AbstractProtocol<NativePacketPayload>
  ...
  protected static final int MAX_QUERY_SIZE_TO_EXPLAIN =
      1024 * 1024; // don't explain queries above 1MB
  ...
  private static final String[] EXPLAINABLE_STATEMENT_EXTENSION =
      new String[] {"INSERT", "UPDATE", "REPLACE", "DELETE"};
   ...
   *
   * <p>We use a SoftReference, so that we don't penalize intermittent use of this feature
   */
  ...
  protected Map<
          Class<? extends ProtocolEntity>,
          ProtocolEntityReader<? extends ProtocolEntity, ? extends Message>>
  ...
  /**
   * The comment (if any) that we'll prepend to all queries sent to the server (to show up in "SHOW
   * PROCESSLIST")
   */
  ...
  public void init(
    ...
    this.logSlowQueries =
        this.propertySet.getBooleanProperty(PropertyKey.logSlowQueries).getValue();
    ...
    this.autoGenerateTestcaseScript =
        this.propertySet.getBooleanProperty(PropertyKey.autoGenerateTestcaseScript).getValue();
    ...
    this.authProvider.init(
        this, this.getPropertySet(), this.socketConnection.getExceptionInterceptor());

    Map<
            Class<? extends ProtocolEntity>,
            ProtocolEntityReader<? extends ProtocolEntity, NativePacketPayload>>
        protocolEntityClassToTextReader = new HashMap<>();
    ...
    this.PROTOCOL_ENTITY_CLASS_TO_TEXT_READER =
        Collections.unmodifiableMap(protocolEntityClassToTextReader);

    Map<
            Class<? extends ProtocolEntity>,
            ProtocolEntityReader<? extends ProtocolEntity, NativePacketPayload>>
        protocolEntityClassToBinaryReader = new HashMap<>();
    ...
    this.PROTOCOL_ENTITY_CLASS_TO_BINARY_READER =
        Collections.unmodifiableMap(protocolEntityClassToBinaryReader);
  }
  ...
  private NativeMessageBuilder getCommandBuilder() {
    ...
    return this.commandBuilder =
        new NativeMessageBuilder(this.serverSession.supportsQueryAttributes());
  }
  ...
  public Supplier<ValueEncoder> getValueEncoderSupplier(Object obj) {
    ...
    if (res == null) {
      Optional<Supplier<ValueEncoder>> mysqlType =
          DEFAULT_ENCODERS.entrySet().stream()
              .filter(m -> m.getKey().isAssignableFrom(obj.getClass()))
              .map(m -> m.getValue())
              .findFirst();
  ...
  public void negotiateSSLConnection() {
    if (!ExportControlled.enabled()) {
      throw new CJConnectionFeatureNotAvailableException(
          this.getPropertySet(), this.serverSession, this.getPacketSentTimeHolder(), null);
    }
    ...
    packet.writeInteger(
        IntegerDataType.INT1, this.serverSession.getCharsetSettings().configurePreHandshake(false));
    packet.writeBytes(
        StringLengthDataType.STRING_FIXED, new byte[23]); // Set of bytes reserved for future use.
    ...
    try {
      ...
      throw new CJConnectionFeatureNotAvailableException(
          this.getPropertySet(), this.serverSession, this.getPacketSentTimeHolder(), nae);
    } catch (IOException ioEx) {
      throw ExceptionFactory.createCommunicationsException(
          this.propertySet,
          this.serverSession,
  ...
  public void rejectProtocol(NativePacketPayload msg) {
    ...
    throw ExceptionFactory.createException(
        MysqlErrorNumbers.get(xOpen) + ", " + errorBuf.toString(),
        xOpen,
        errno,
        false,
        null,
  ...
  public void afterHandshake() {
    ...
    try {
      ...
      //
      if (((this.serverSession.getCapabilities().getCapabilityFlags()
                  & NativeServerSession.CLIENT_COMPRESS)
              != 0)
          && this.propertySet.getBooleanProperty(PropertyKey.useCompression).getValue()
          && !(this.socketConnection.getMysqlInput().getUnderlyingStream()
              instanceof CompressedInputStream)) {
        ...
        this.socketConnection.setMysqlInput(
            new FullReadInputStream(
                new CompressedInputStream(
                    ...
                    this.propertySet.getBooleanProperty(PropertyKey.traceProtocol),
                    this.log)));
        this.compressedPacketSender =
            new CompressedPacketSender(this.socketConnection.getMysqlOutput());
    ...
    RuntimeProperty<Boolean> useInformationSchema =
        this.propertySet.<Boolean>getProperty(PropertyKey.useInformationSchema);
    if (versionMeetsMinimum(8, 0, 3)
        && !useInformationSchema.getValue()
        && !useInformationSchema.isExplicitlySet()) {
   ...
   */
  public void applyPacketDecorators(
      MessageSender<NativePacketPayload> sender,
    ...
    synchronized (this.packetSender) {
      ...
      this.setPacketReceivedTimeHolder(
          ttReader != null ? ttReader : new PacketReceivedTimeHolder() {});
    }
  }
  ...
  public void changeDatabase(String database) {
    ...
    try {
      ...
      if (this.getPropertySet()
          .getBooleanProperty(PropertyKey.createDatabaseIfNotExist)
          .getValue()) {
        sendCommand(
            getCommandBuilder()
                .buildComQuery(
                    getSharedSendPacket(),
                    "CREATE DATABASE IF NOT EXISTS " + StringUtils.quoteIdentifier(database, true)),
            false,
            0);
  ...
  public final NativePacketPayload readMessage(NativePacketPayload reuse) {
    try {
      ...
      throw ExceptionFactory.createException(
          oom.getMessage(),
          MysqlErrorNumbers.SQL_STATE_MEMORY_ALLOCATION_ERROR,
          0,
          false,
          oom,
  ...
  @Override
  public final NativePacketPayload sendCommand(
      Message queryPacket, boolean skipCheck, int timeoutMillis) {
    int command = queryPacket.getByteBuffer()[0];
    this.commandCount++;

    if (this.queryInterceptors != null) {
      NativePacketPayload interceptedPacketPayload =
          (NativePacketPayload) invokeQueryInterceptorsPre(queryPacket, false);
    ...
    try {
      ...
      // Clear serverStatus...this value is guarded by an external mutex, as you can only ever be
      ...
      if (!skipCheck) {
        if ((command == NativeConstants.COM_STMT_EXECUTE)
            || (command == NativeConstants.COM_STMT_RESET)) {
          this.packetReader.resetMessageSequence();
        }
        ...
        if (this.queryInterceptors != null) {
          returnPacket =
              (NativePacketPayload) invokeQueryInterceptorsPost(queryPacket, returnPacket, false);
        }
      }
  ...
  private NativePacketPayload checkErrorMessage(int command) {
    ...
    try {
      // Check return value, if we get a java.io.EOFException, the server has gone away. We'll pass
      // it on up the exception chain and let someone higher up
    ...
    if (statusCode == (byte) 0xff) {
      ...
      serverErrorMessage =
          resultPacket.readString(
              ...
              this.serverSession.getCharsetSettings().getErrorMessageEncoding());
      ...
      boolean useOnlyServerErrorMessages =
          this.propertySet.getBooleanProperty(PropertyKey.useOnlyServerErrorMessages).getValue();
      ...
      if (xOpen != null) {
        ...
        if (errno == MysqlErrorNumbers.ER_MUST_CHANGE_PASSWORD) {
          throw ExceptionFactory.createException(
              PasswordExpiredException.class, errorBuf.toString(), getExceptionInterceptor());
      ...
      throw ExceptionFactory.createException(
          errorBuf.toString(), xOpen, errno, false, null, getExceptionInterceptor());
    }
  }
  ...
  public void clearInputStream() {
    try {
      ...
      // Due to a bug in some older Linux kernels (fixed after the patch "tcp: fix
      // FIONREAD/SIOCINQ"), our SocketInputStream.available() may return 1 even
      // if there is no data in the Stream, so, we need to check if InputStream.skip() actually
      // skipped anything.
      while ((len = this.socketConnection.getMysqlInput().available()) > 0
          && this.socketConnection.getMysqlInput().skip(len) > 0) {
        continue;
      }
    } catch (IOException ioEx) {
      throw ExceptionFactory.createCommunicationsException(
  ...
  public final <T extends Resultset> T sendQueryString(
    ...
    if (this.propertySet
        .getBooleanProperty(PropertyKey.includeThreadNamesAsStatementComment)
        .getValue()) {
      statementComment =
    ...
    // We don't know exactly how many bytes we're going to get from the query. Since we're dealing
    // with UTF-8, the max is 4, so pad it
    ...
    if (!supportsQueryAttributes
        && callingQuery != null
        && callingQuery.getQueryAttributesBindings().getCount() > 0) {
    ...
    if (supportsQueryAttributes) {
      ...
      if (queryAttributes != null && queryAttributes.getCount() > 0) {
        ...
        packLength +=
            (queryAttributes.getCount() + 7) / 8 /* null_bitmap */ + 1 /* new_params_bind_flag */;
        for (int i = 0; i < queryAttributes.getCount(); i++) {
          BindValue queryAttribute = queryAttributes.getAttributeValue(i);
          packLength +=
              2 /* parameter_type */
                  + queryAttribute.getName().length() /* parameter_name */
                  + queryAttribute.getBinaryLength();
        }
      } else {
    ...
    if (commentAsBytes != null) {
      ...
      sendPacket.writeBytes(
          StringLengthDataType.STRING_FIXED, Constants.SPACE_STAR_SLASH_SPACE_AS_BYTES);
    }

    if (!this.session.getServerSession().getCharsetSettings().doesPlatformDbCharsetMatches()
        && StringUtils.startsWithIgnoreCaseAndWs(query, "LOAD DATA")) {
      sendPacket.writeBytes(StringLengthDataType.STRING_FIXED, StringUtils.getBytes(query));
    } else {
      sendPacket.writeBytes(
          StringLengthDataType.STRING_FIXED, StringUtils.getBytes(query, characterEncoding));
    }

    return sendQueryPacket(
        callingQuery, sendPacket, maxRows, streamResults, cachedMetadata, resultSetFactory);
  }
  ...
  public final <T extends Resultset> T sendQueryPacket(
    ...
    try {
      ...
      boolean queryWasSlow =
          this.logSlowQueries
              && (this.useAutoSlowLog
                  ? this.metricsHolder.checkAbonormallyLongQuery(queryDuration)
                  : queryDuration
                      > this.propertySet
                          .getIntegerProperty(PropertyKey.slowQueryThresholdMillis)
                          .getValue());
      ...
      T rs =
          readAllResults(
              maxRows, streamResults, resultPacket, false, cachedMetadata, resultSetFactory);
      ...
      if (this.profileSQL || queryWasSlow) {
        ...
        int extractPosition =
            truncated ? this.maxQuerySizeToLog.getValue() + queryPosition : oldPacketPosition;
        String extractedQuery =
            StringUtils.toString(queryBuf, queryPosition, (extractPosition - queryPosition));
        if (truncated) {
          ...
          if (queryWasSlow) {
            eventSink.processEvent(
                ...
                Messages.getString(
                    ...
                    new Object[] {
                      this.useAutoSlowLog
                          ? " 95% of all queries "
                          : String.valueOf(this.slowQueryThreshold),
                      this.queryTimingUnits,
                      Long.valueOf(queryDuration),
                      extractedQuery
                    }));
            ...
            if (this.propertySet.getBooleanProperty(PropertyKey.explainSlowQueries).getValue()) {
              if (oldPacketPosition - queryPosition < MAX_QUERY_SIZE_TO_EXPLAIN) {
                queryPacket.setPosition(
                    queryPosition); // skip until the query is located in the packet
                explainSlowQuery(query.toString(), extractedQuery);
              } else {
                this.log.logWarn(
                    Messages.getString("Protocol.3", new Object[] {MAX_QUERY_SIZE_TO_EXPLAIN}));
              }
            }
          }
      ...
      if (this.queryInterceptors != null) {
        ...
        invokeQueryInterceptorsPost(
            ...
            callingQuery,
            null,
            false); // we don't do anything with the result set in this case
      }
  ...
  public <T extends Resultset> T invokeQueryInterceptorsPre(
      Supplier<String> sql, Query interceptedQuery, boolean forceExecute) {
    ...
    for (int i = 0, s = this.queryInterceptors.size(); i < s; i++) {
      ...
      boolean shouldExecute =
          (executeTopLevelOnly && (this.statementExecutionDepth == 1 || forceExecute))
              || (!executeTopLevelOnly);
  ...
  public <M extends Message> M invokeQueryInterceptorsPre(M queryPacket, boolean forceExecute) {
    ...
    for (int i = 0, s = this.queryInterceptors.size(); i < s; i++) {
      ...
      //            boolean executeTopLevelOnly = interceptor.executeTopLevelOnly();
      //            boolean shouldExecute = (executeTopLevelOnly && (this.statementExecutionDepth ==
      // 1 || forceExecute)) || (!executeTopLevelOnly);
      //            if (shouldExecute) {
  ...
  public <T extends Resultset> T invokeQueryInterceptorsPost(
      Supplier<String> sql, Query interceptedQuery, T originalResultSet, boolean forceExecute) {
    ...
    for (int i = 0, s = this.queryInterceptors.size(); i < s; i++) {
      ...
      if (shouldExecute) {
        T interceptedResultSet =
            interceptor.postProcess(sql, interceptedQuery, originalResultSet, this.serverSession);
   ...
   * @return T instance
   */
  public <M extends Message> M invokeQueryInterceptorsPost(
      M queryPacket, M originalResponsePacket, boolean forceExecute) {
  ...
  public long getCurrentTimeNanosOrMillis() {
    return this.useNanosForElapsedTime
        ? TimeUtil.getCurrentTimeNanosOrMillis()
  ...
  public void explainSlowQuery(String query, String truncatedQuery) {
    if (StringUtils.startsWithIgnoreCaseAndWs(truncatedQuery, EXPLAINABLE_STATEMENT)
        || (versionMeetsMinimum(5, 6, 3)
            && StringUtils.startsWithIgnoreCaseAndWs(
                    truncatedQuery, EXPLAINABLE_STATEMENT_EXTENSION)
                != -1)) {

      try {
        NativePacketPayload resultPacket =
            sendCommand(
                getCommandBuilder().buildComQuery(getSharedSendPacket(), "EXPLAIN " + query),
                false,
                0);
      ...
      try {
        if (!ExportControlled.isSSLEstablished(
            this.socketConnection
                .getMysqlSocket())) { // Fix for Bug#56979 does not apply to secure sockets.
          if (!this.socketConnection.getMysqlSocket().isClosed()) {
            try {
              // The response won't be read, this fixes BUG#56979 [Improper connection closing logic
  ...
  private void calculateSlowQueryThreshold() {
    this.slowQueryThreshold =
        this.propertySet.getIntegerProperty(PropertyKey.slowQueryThresholdMillis).getValue();

    if (this.propertySet.getBooleanProperty(PropertyKey.useNanosForElapsedTime).getValue()) {
      long nanosThreshold =
          this.propertySet.getLongProperty(PropertyKey.slowQueryThresholdNanos).getValue();
  ...
  public void dumpPacketRingBuffer() {
    ...
    if (localPacketDebugRingBuffer != null) {
      ...
      dumpBuffer.append(
          "Last "
              + localPacketDebugRingBuffer.size()
              + " packets received from server, from oldest->newest:\n");
      dumpBuffer.append("\n");

      for (Iterator<StringBuilder> ringBufIter = localPacketDebugRingBuffer.iterator();
          ringBufIter.hasNext(); ) {
  ...
  public boolean versionMeetsMinimum(int major, int minor, int subminor) {
    return this.serverSession
        .getServerVersion()
        .meetsMinimum(new ServerVersion(major, minor, subminor));
  }
  ...
  public static MysqlType findMysqlType(
    ...
    boolean isImplicitTemporaryTable =
        tableName.length() > 0 && tableName.toString().startsWith("#sql_");

    boolean isOpaqueBinary =
        (isBinary
                && collationIndex == CharsetMapping.MYSQL_COLLATION_INDEX_binary
                && (mysqlTypeId == MysqlType.FIELD_TYPE_STRING
                    || mysqlTypeId == MysqlType.FIELD_TYPE_VAR_STRING
                    || mysqlTypeId == MysqlType.FIELD_TYPE_VARCHAR))
            ?
    ...
    switch (mysqlTypeId) {
      ...
      case MysqlType.FIELD_TYPE_TINY:
        ...
        if (!isUnsigned
            && length == 1
            && propertySet.getBooleanProperty(PropertyKey.tinyInt1isBit).getValue()) {
      ...
      case MysqlType.FIELD_TYPE_VAR_STRING:
        if (isOpaqueBinary
            && !(isFromFunction
                && propertySet
                    .getBooleanProperty(PropertyKey.functionsNeverReturnBlobs)
                    .getValue())) {
  ...
  public <T extends ProtocolEntity> T read(
      Class<T> requiredClass, ProtocolEntityFactory<T, NativePacketPayload> protocolEntityFactory)
      throws IOException {
    @SuppressWarnings("unchecked")
    ProtocolEntityReader<T, NativePacketPayload> sr =
        (ProtocolEntityReader<T, NativePacketPayload>)
            this.PROTOCOL_ENTITY_CLASS_TO_TEXT_READER.get(requiredClass);
    if (sr == null) {
      throw ExceptionFactory.createException(
          FeatureNotAvailableException.class,
          "ProtocolEntityReader isn't available for class " + requiredClass);
    }
  ...
  public <T extends ProtocolEntity> T readNextResultset(
    ...
    T result = null;
    if (Resultset.class.isAssignableFrom(currentProtocolEntity.getClass())
        && this.serverSession.useMultiResults()) {
  ...
  public <T extends Resultset> T readAllResults(
    ...
    if (this.serverSession.hasMoreResults()) {
      ...
      if (streamResults) {
        currentResultSet =
            readNextResultset(currentResultSet, maxRows, true, isBinaryEncoded, resultSetFactory);
      } else {
        while (this.serverSession.hasMoreResults()) {
          currentResultSet =
              readNextResultset(
                  currentResultSet, maxRows, false, isBinaryEncoded, resultSetFactory);
        }
        clearInputStream();
      }
    }
  ...
  @SuppressWarnings("unchecked")
  public final <T> T readServerStatusForResultSets(
      NativePacketPayload rowPacket, boolean saveOldStatus) {
    T result = null;
    if (rowPacket.isEOFPacket()) {
      ...
      this.serverSession.setStatusFlags(
          (int) rowPacket.readInteger(IntegerDataType.INT2), saveOldStatus);
      checkTransactionState();
    } else {
      // read OK packet
      OkPacket ok =
          OkPacket.parse(
              rowPacket, this.serverSession.getCharsetSettings().getErrorMessageEncoding());
      result = (T) ok;

      this.serverSession.setStatusFlags(ok.getStatusFlags(), saveOldStatus);
      this.serverSession
          .getServerSessionStateController()
          .setSessionStateChanges(ok.getSessionStateChanges());
      checkTransactionState();
  ...
  public final NativePacketPayload sendFileToServer(String fileName) {
    NativePacketPayload filePacket =
        (this.loadFileBufRef == null) ? null : this.loadFileBufRef.get();

    int bigPacketLength =
        Math.min(
            this.maxAllowedPacket.getValue() - (NativeConstants.HEADER_LENGTH * 3),
            alignPacketSize(this.maxAllowedPacket.getValue() - 16, 4096)
                - (NativeConstants.HEADER_LENGTH * 3));
    int oneMeg = 1024 * 1024;
    int smallerPacketSizeAligned =
        Math.min(
            oneMeg - (NativeConstants.HEADER_LENGTH * 3),
    ...
    if (filePacket == null) {
      try {
        ...
        throw ExceptionFactory.createException(
            Messages.getString("MysqlIO.111", new Object[] {packetLength}),
            MysqlErrorNumbers.SQL_STATE_MEMORY_ALLOCATION_ERROR,
            0,
            false,
            oom,
    ...
    try {
      ...
      throw ExceptionFactory.createException(
          messageBuf.toString(), ioEx, this.exceptionInterceptor);
    } finally {
      if (fileIn != null) {
        try {
          fileIn.close();
          ...
          throw ExceptionFactory.createException(
              Messages.getString("MysqlIO.65"), ex, this.exceptionInterceptor);
        }
  ...
  private BufferedInputStream getFileStream(String fileName) throws IOException {
    RuntimeProperty<Boolean> allowLoadLocalInfile =
        this.propertySet.getBooleanProperty(PropertyKey.allowLoadLocalInfile);
    RuntimeProperty<String> allowLoadLocaInfileInPath =
        this.propertySet.getStringProperty(PropertyKey.allowLoadLocalInfileInPath);
    RuntimeProperty<Boolean> allowUrlInLocalInfile =
        this.propertySet.getBooleanProperty(PropertyKey.allowUrlInLocalInfile);

    if (!allowLoadLocalInfile.getValue() && !allowLoadLocaInfileInPath.isExplicitlySet()) {
      throw ExceptionFactory.createException(
          Messages.getString("MysqlIO.LoadDataLocalNotAllowed"), this.exceptionInterceptor);
    }
    ...
    // Given the code paths above, allowLoadLocaInfileInPath.isExplicitlySet() must be true and
    // restrictions to "LOAD DATA LOCAL INFILE" apply.
    ...
    if (safePathValue.length() == 0) {
      throw ExceptionFactory.createException(
          Messages.getString(
              "MysqlIO.60",
              new Object[] {safePathValue, PropertyKey.allowLoadLocalInfileInPath.getKeyName()}),
          this.exceptionInterceptor);
    }
    try {
    ...
    if (allowUrlInLocalInfile.getValue()) {
      try {
        ...
        if (!urlFromFileName.getProtocol().equalsIgnoreCase("file")) {
          throw ExceptionFactory.createException(
              Messages.getString("MysqlIO.66", new Object[] {urlFromFileName.getProtocol()}),
              this.exceptionInterceptor);
        }
        ...
        try {
          ...
          if (!addr.isLoopbackAddress()) {
            throw ExceptionFactory.createException(
                Messages.getString("MysqlIO.67", new Object[] {urlFromFileName.getHost()}),
                this.exceptionInterceptor);
          }
        } catch (UnknownHostException e) {
          throw ExceptionFactory.createException(
              Messages.getString("MysqlIO.68", new Object[] {fileName}),
              e,
              this.exceptionInterceptor);
        }
        ...
        if (!filePath.startsWith(safePath)) {
          throw ExceptionFactory.createException(
              Messages.getString("MysqlIO.61", new Object[] {filePath, safePath}),
              this.exceptionInterceptor);
        }
  ...
  public void checkForOutstandingStreamingData() {
    if (this.streamingData != null) {
      boolean shouldClobber =
          this.propertySet.getBooleanProperty(PropertyKey.clobberStreamingResults).getValue();
  ...
  public void unsetStreamingData(ResultsetRows streamer) {
    if (this.streamingData == null) {
      throw ExceptionFactory.createException(
          Messages.getString("MysqlIO.17") + streamer + Messages.getString("MysqlIO.18"),
          this.exceptionInterceptor);
    }

    // don't try to discard streamer other than passed in; it's probably already replaced by the
    // next result in a multi-resultset
  ...
  public void scanForAndThrowDataTruncation() {
    if (this.streamingData == null
        && this.propertySet.getBooleanProperty(PropertyKey.jdbcCompliantTruncation).getValue()
        && getWarningCount() > 0) {
  ...
  private void appendDeadlockStatusInformation(Session sess, String xOpen, StringBuilder errorBuf) {
    if (sess.getPropertySet()
            .getBooleanProperty(PropertyKey.includeInnodbStatusInDeadlockExceptions)
            .getValue()
        && xOpen != null
        && (xOpen.startsWith("40") || xOpen.startsWith("41"))
        && getStreamingData() == null) {

      try {
        NativePacketPayload resultPacket =
            sendCommand(
                getCommandBuilder()
                    .buildComQuery(getSharedSendPacket(), "SHOW ENGINE INNODB STATUS"),
                false,
                0);
        ...
        errorBuf
            .append("\n\n")
            .append(Messages.getString("MysqlIO.InnoDBStatusFailed"))
            .append("\n\n")
    ...
    if (sess.getPropertySet()
        .getBooleanProperty(PropertyKey.includeThreadDumpInDeadlockExceptions)
        .getValue()) {
   ...
   *
   * <p>If 'forTruncationOnly' is true, only looks for truncation warnings, and actually throws
  ...
  public SQLWarning convertShowWarningsToSQLWarnings(boolean forTruncationOnly) {
    ...
    try {
      ...
      NativePacketPayload resultPacket =
          sendCommand(
              getCommandBuilder().buildComQuery(getSharedSendPacket(), "SHOW WARNINGS"), false, 0);

      Resultset warnRs =
          readAllResults(
              -1,
              this.warningCount > 99 /* stream large warning counts */,
              resultPacket,
              false,
              null,
      ...
      while ((r = rows.next()) != null) {
        ...
        if (forTruncationOnly) {
          if (code == MysqlErrorNumbers.ER_WARN_DATA_TRUNCATED
              || code == MysqlErrorNumbers.ER_WARN_DATA_OUT_OF_RANGE) {
            DataTruncation newTruncation =
                new MysqlDataTruncation(
                    r.getValue(messageFieldIndex, svf), 0, false, false, 0, 0, code);
          ...
          SQLWarning newWarning =
              new SQLWarning(message, MysqlErrorNumbers.mysqlToSqlState(code), code);
          if (currentWarning == null) {
  ...
  public void configureTimeZone() {
    ...
    if (connectionTimeZone == null
        || StringUtils.isEmptyOrWhitespaceOnly(connectionTimeZone)
        || "LOCAL".equals(connectionTimeZone)) {
    ...
    if (getPropertySet()
        .getBooleanProperty(PropertyKey.forceConnectionTimeZoneToSession)
        .getValue()) {
      // TODO don't send 'SET SESSION time_zone' if time_zone is already equal to the selectedTz
      // (but it requires time zone detection)
  ...
  public void initServerSession() {
    ...
    if (this.serverSession.getServerVariables().containsKey("max_allowed_packet")) {
      ...
      if (serverMaxAllowedPacket != -1
          && (!this.maxAllowedPacket.isExplicitlySet()
              || serverMaxAllowedPacket < this.maxAllowedPacket.getValue())) {
        this.maxAllowedPacket.setValue(serverMaxAllowedPacket);
      }

      if (this.useServerPrepStmts.getValue()) {
        RuntimeProperty<Integer> blobSendChunkSize =
            this.propertySet.getProperty(PropertyKey.blobSendChunkSize);
        ...
        int allowedBlobSendChunkSize =
            Math.min(preferredBlobSendChunkSize, this.maxAllowedPacket.getValue())
                - packetHeaderSize;

        if (allowedBlobSendChunkSize <= 0) {
          throw ExceptionFactory.createException(
              Messages.getString("Connection.15", new Object[] {packetHeaderSize}),
