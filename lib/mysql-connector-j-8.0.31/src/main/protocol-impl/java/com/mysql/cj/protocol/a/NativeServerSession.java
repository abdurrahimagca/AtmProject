...
public class NativeServerSession implements ServerSession {
  ...
  public static final int SERVER_STATUS_LAST_ROW_SENT =
      128; // The server status for 'last-row-sent'
  ...
  public static final int CLIENT_PS_MULTI_RESULTS =
      0x00040000; // Enable/disable multi-results for server prepared statements
  ...
  public NativeServerSession(PropertySet propertySet) {
    ...
    this.cacheDefaultTimeZone =
        this.propertySet.getBooleanProperty(PropertyKey.cacheDefaultTimeZone);
  ...
  public boolean useMultiResults() {
    return (this.clientParam & CLIENT_MULTI_RESULTS) != 0
        || (this.clientParam & CLIENT_PS_MULTI_RESULTS) != 0;
  }
  ...
  public int getServerVariable(String variableName, int fallbackValue) {
    try {
      ...
      // getLog().logWarn(
      //        Messages.getString("Connection.BadValueInServerVariables", new Object[] {
      // variableName, getServerVariable(variableName), fallbackValue }));
    }
   ...
   */
  public boolean isSetNeededForAutoCommitMode(
      boolean autoCommitFlag, boolean elideSetAutoCommitsFlag) {
    if (elideSetAutoCommitsFlag) {
      boolean autoCommitModeOnServer = isAutocommit();

      if (autoCommitModeOnServer && !autoCommitFlag) {
        // When trying to set autoCommit == false and SERVER_STATUS_AUTOCOMMIT = true we need to
        // check
        // SERVER_STATUS_IN_TRANS flag, because SERVER_STATUS_AUTOCOMMIT isn't always correct.
        // If a transaction is in progress on the server, then we must be already in autoCommit ==
        // false
  ...
  public boolean isLowerCaseTableNames() {
    ...
    return "on".equalsIgnoreCase(lowerCaseTables)
      ...
      if (configuredTimeZoneOnServer != null) {
        try {
          this.sessionTimeZone =
              TimeZone.getTimeZone(TimeUtil.getCanonicalTimeZone(configuredTimeZoneOnServer, null));
