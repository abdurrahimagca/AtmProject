    ...
    public NativeServerSessionStateChanges init(NativePacketPayload buf, String encoding) {
      ...
      while (totalLen > 0 && end > start) {
        ...
        NativePacketPayload b =
            new NativePacketPayload(buf.readBytes(StringSelfDataType.STRING_LENENC));
        switch (type) {
          case SESSION_TRACK_SYSTEM_VARIABLES:
            this.sessionStateChanges.add(
                new SessionStateChange(type) //
          ...
          case SESSION_TRACK_GTIDS:
            b.readInteger(
                IntegerDataType
                    .INT1); // skip the byte reserved for the encoding specification, see WL#6128
            this.sessionStateChanges.add(
                new SessionStateChange(type) //
                    .addValue(b.readString(StringSelfDataType.STRING_LENENC, encoding)));
            break;
          ...
          default:
            ...
            this.sessionStateChanges.add(
                new SessionStateChange(type) //
                    .addValue(
                        b.readString(
                            StringLengthDataType.STRING_FIXED, encoding, b.getPayloadLength())));
        }
