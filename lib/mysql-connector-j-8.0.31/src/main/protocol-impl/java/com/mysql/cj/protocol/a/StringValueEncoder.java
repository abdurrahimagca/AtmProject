...
public class StringValueEncoder extends AbstractValueEncoder {
    ...
    switch (binding.getMysqlType()) {
      ...
      case LONGBLOB:
        ...
        if (binding.isNational()
            && !this.charEncoding.getValue().equalsIgnoreCase("UTF-8")
            && !this.charEncoding.getValue().equalsIgnoreCase("utf8")) {
        ...
        if (isEscapeNeededForString(x, stringLength)) {
          String escString =
              StringUtils.escapeString(
                      new StringBuilder((int) (x.length() * 1.1)),
                      x,
                      this.serverSession.useAnsiQuotedIdentifiers(),
                      this.charsetEncoder)
                  .toString();
  ...
  public String getString(BindValue binding) {
    ...
    switch (binding.getMysqlType()) {
      ...
      case BIT:
        ...
        if ("true".equalsIgnoreCase(x) || "Y".equalsIgnoreCase(x)) {
          ...
          throw ExceptionFactory.createException(
              ...
              Messages.getString("PreparedStatement.66", new Object[] {x}),
      ...
      case DATE:
        ...
        throw ExceptionFactory.createException(
            ...
            Messages.getString(
                ...
                new Object[] {dt.getClass().getName(), binding.getMysqlType().toString()}),
            this.exceptionInterceptor);
      case DATETIME:
      case TIMESTAMP:
        ...
        if (dt instanceof LocalDate) {
          sb = new StringBuilder("'");
          sb.append(
              LocalDateTime.of((LocalDate) dt, TimeUtil.DEFAULT_TIME)
          ...
          sb = new StringBuilder("'");
          sb.append(
              adjustLocalDateTime((LocalDateTime) dt, binding.getField())
                  .format(TimeUtil.DATETIME_FORMATTER_WITH_OPTIONAL_MICROS));
          sb.append("'");
          return sb.toString();
        }
      ...
      case TIME:
        ...
        if (dt instanceof LocalTime) {
          sb = new StringBuilder("'");
          sb.append(
              adjustLocalTime((LocalTime) dt, binding.getField())
          ...
          sb = new StringBuilder("'");
          sb.append(
              adjustLocalTime(((LocalDateTime) dt).toLocalTime(), binding.getField())
                  .format(TimeUtil.TIME_FORMATTER_WITH_OPTIONAL_MICROS));
          sb.append("'");
          return sb.toString();
    ...
    throw ExceptionFactory.createException(
        ...
        Messages.getString(
            "PreparedStatement.67",
            new Object[] {
              binding.getValue().getClass().getName(), binding.getMysqlType().toString()
            }),
        this.exceptionInterceptor);
  }
  ...
  public void encodeAsQueryAttribute(Message msg, BindValue binding) {
    ...
    intoPacket.writeBytes(
        StringSelfDataType.STRING_LENENC, StringUtils.getBytes(x, this.charEncoding.getValue()));
  }
    ...
    switch (binding.getMysqlType()) {
      ...
      case DECIMAL_UNSIGNED:
        ...
        intoPacket.writeBytes(
            ...
            StringUtils.getBytes(bd.toPlainString(), this.charEncoding.getValue()));
        return;
      case CHAR:
      case ENUM:
      case SET:
      ...
      case LONGBLOB:
        if (binding.isNational()
            ...
            && !this.charEncoding.getValue().equalsIgnoreCase("utf8")) {
          throw ExceptionFactory.createException(
              Messages.getString("ServerPreparedStatement.31"), this.exceptionInterceptor);
        }
        try {
          intoPacket.writeBytes(
              StringSelfDataType.STRING_LENENC,
              StringUtils.getBytes(x, this.charEncoding.getValue()));
        } catch (CJException uEE) {
          throw ExceptionFactory.createException(
              Messages.getString("ServerPreparedStatement.31") + this.charEncoding.getValue() + "'",
              uEE,
              this.exceptionInterceptor);
        }
        return;
      ...
      case TIME:
        ...
        if (dt instanceof LocalTime) {
          ...
          writeTime(
              ...
              InternalTime.from(
                  adjustDuration(
                      Duration.ofNanos(((Duration) binding.getValue()).toNanos()),
                      binding.getField())));
          return;
        }
        break;
      case YEAR:
        ...
        if (dt instanceof LocalDate) {
          ...
          intoPacket.writeInteger(
              IntegerDataType.INT4, Long.valueOf(((LocalDateTime) dt).getYear()));
          return;
        }
        break;
