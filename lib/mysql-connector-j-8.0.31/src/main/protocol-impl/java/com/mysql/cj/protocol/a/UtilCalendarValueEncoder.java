...
public class UtilCalendarValueEncoder extends AbstractValueEncoder {
  ...
  public String getString(BindValue binding) {
    ...
    switch (binding.getMysqlType()) {
      ...
      case DATE:
        return binding.getCalendar() != null
            ...
            : TimeUtil.getSimpleDateFormat(
                    null, "''yyyy-MM-dd''", this.serverSession.getDefaultTimeZone())
                .format(new java.sql.Date(x.getTimeInMillis()));
      case TIMESTAMP:
        Timestamp ts =
            adjustTimestamp(
                new java.sql.Timestamp(((Calendar) binding.getValue()).getTimeInMillis()),
                binding.getField(),
                binding.keepOrigNanos());
        StringBuffer buf = new StringBuffer();
        if (binding.getCalendar() != null) {
          buf.append(
              TimeUtil.getSimpleDateFormat("''yyyy-MM-dd HH:mm:ss", binding.getCalendar())
                  .format(x));
        } else {
          buf.append(
              TimeUtil.getSimpleDateFormat(
                      null,
                      "''yyyy-MM-dd HH:mm:ss",
                      binding.getMysqlType() == MysqlType.TIMESTAMP
                              && this.preserveInstants.getValue()
      ...
      case LONGTEXT:
        ZonedDateTime zdt =
            ZonedDateTime.ofInstant(x.toInstant(), x.getTimeZone().toZoneId())
        ...
        sb.append(
            zdt.format(
                zdt.getNano() > 0
                        && this.serverSession.getCapabilities().serverSupportsFracSecs()
      ...
      case TIME:
        ...
        sb.append(
            adjustLocalTime(
                    ((Calendar) binding.getValue())
                        .toInstant()
                        .atZone(this.serverSession.getDefaultTimeZone().toZoneId())
                        .toLocalTime(),
                    binding.getField())
  ...
  public void encodeAsQueryAttribute(Message msg, BindValue binding) {
    ...
    InternalTimestamp internalTimestamp =
        InternalTimestamp.from(
            calendar, (int) TimeUnit.MILLISECONDS.toNanos(calendar.get(Calendar.MILLISECOND)));
    internalTimestamp.setOffset(
        (int)
