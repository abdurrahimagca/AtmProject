...
public class AuthenticationKerberosClient implements AuthenticationPlugin<NativePacketPayload> {
      ...
      (cbs) -> {
        for (Callback cb : cbs) {
          if (NameCallback.class.isAssignableFrom(cb.getClass())) {
            ...
            ((PasswordCallback) cb)
                .setPassword(this.password == null ? new char[0] : this.password.toCharArray());
          } else {
  ...
  public boolean nextAuthenticationStep(
      ...
      try {
        ...
        String servicePrincipalName =
            fromServer.readString(
                StringLengthDataType.STRING_VAR, "ASCII", servicePrincipalNameLength);
        // A typical Kerberos V5 principal has the structure "primary/instance@REALM".
        String primary = "";
        String instance = "";
        // Being a bit lenient here: the spec allows escaping characters
        // (https://tools.ietf.org/html/rfc1964#section-2.1.1).
        ...
        String userPrincipalRealm =
            fromServer.readString(
                StringLengthDataType.STRING_VAR, "ASCII", userPrincipalRealmLength);
                  ...
                  (PrivilegedExceptionAction<SaslClient>)
                      () ->
                          Sasl.createSaslClient(
                              new String[] {AUTHENTICATION_MECHANISM},
                              null,
                              localPrimary,
                              localInstance,
                              null,
        ...
        throw ExceptionFactory.createException(
            Messages.getString(
                "AuthenticationKerberosClientPlugin.FailCreateSaslClient",
                new Object[] {AUTHENTICATION_MECHANISM}),
            e);
      }
    ...
    if (!this.saslClient.isComplete()) {
      ...
      try {
        ...
        throw ExceptionFactory.createException(
            Messages.getString(
                "AuthenticationKerberosClientPlugin.ErrProcessingAuthIter",
                new Object[] {AUTHENTICATION_MECHANISM}),
            e.getException());
      }
    }
  ...
  private void initializeAuthentication() {
    if (this.subject != null
        && this.cachedPrincipalName != null
        && this.cachedPrincipalName.equals(this.userPrincipalName)) {
      // Already initialized with the right user.
      return;
    }

    // In-memory login configuration. Used only if system property 'java.security.auth.login.config'
    // is not set.
          ...
          new Configuration() {
            ...
            public AppConfigurationEntry[] getAppConfigurationEntry(String name) {
              ...
              options.put(
                  "debug",
                  Boolean.toString(
                      debug)); // Hook debugging on system property 'sun.security.jgss.debug'.
    ...
    try {
      loginContext =
          new LoginContext(LOGIN_CONFIG_ENTRY, null, this.credentialsCallbackHandler, loginConfig);
      loginContext.login();
      this.subject = loginContext.getSubject();
      this.cachedPrincipalName = this.subject.getPrincipals().iterator().next().getName();
    } catch (LoginException e) {
      throw ExceptionFactory.createException(
          Messages.getString("AuthenticationKerberosClientPlugin.FailAuthenticateUser"), e);
    }
  }
}
