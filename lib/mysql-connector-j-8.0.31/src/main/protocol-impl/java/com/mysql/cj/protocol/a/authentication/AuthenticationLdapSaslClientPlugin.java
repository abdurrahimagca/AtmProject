...
public class AuthenticationLdapSaslClientPlugin
  ...
  private enum AuthenticationMechanisms {
    ...
    SCRAM_SHA_256(
        ScramSha256SaslClient.IANA_MECHANISM_NAME, ScramSha256SaslClient.MECHANISM_NAME), //
    ...
    static AuthenticationMechanisms fromValue(String mechName) {
      ...
      throw ExceptionFactory.createException(
          Messages.getString(
              "AuthenticationLdapSaslClientPlugin.UnsupportedAuthMech", new String[] {mechName}));
    }
      ...
      (cbs) -> {
        for (Callback cb : cbs) {
          if (NameCallback.class.isAssignableFrom(cb.getClass())) {
            ...
            char[] passwordChars =
                this.password == null ? new char[0] : this.password.toCharArray();
  ...
  public void reset() {
    ...
    this.subject = null;
    // this.firstPass must not be reset to 'true'. It is required to give it a second chance when
    // AuthenticationLdapSaslClientPlugin is the default plugin.
  }
    ...
    if (this.saslClient == null) {
      ...
      try {
        ...
        if (this.firstPass) {
          ...
          // Payload could be a salt (auth-plugin-data) value instead of an authentication mechanism
          // identifier.
          // Give it another try in the expectation of receiving a Protocol::AuthSwitchRequest or a
          // Protocol::AuthNextFactor next time.
          return true;
        }
        throw e;
      }
      ...
      try {
        switch (this.authMech) {
          case GSSAPI:
            ...
            String ldapServerHostname =
                this.protocol
                    .getPropertySet()
                    .getStringProperty(PropertyKey.ldapServerHostname)
                    .getValue();
            if (StringUtils.isNullOrEmpty(
                ldapServerHostname)) { // Use the default KDC short name instead.
            ...
            if (StringUtils.isNullOrEmpty(ldapServerHostname)) {
              throw ExceptionFactory.createException(
                  Messages.getString(
                      "AuthenticationLdapSaslClientPlugin.MissingLdapServerHostname"));
            }

            // In-memory login configuration. Used only if system property
            ...
            LoginContext loginContext =
                new LoginContext(
                    LOGIN_CONFIG_ENTRY, null, this.credentialsCallbackHandler, loginConfig);
            loginContext.login();
                      ...
                      (PrivilegedExceptionAction<SaslClient>)
                          () ->
                              Sasl.createSaslClient(
                                  new String[] {this.authMech.getSaslServiceName()},
                                  null,
                                  LDAP_SERVICE_NAME,
                                  localLdapServerHostname,
                                  null,
                                  null));
        ...
        throw ExceptionFactory.createException(
            Messages.getString(
                "AuthenticationLdapSaslClientPlugin.FailCreateSaslClient",
                new Object[] {this.authMech.getMechName()}),
            e);
      }
      ...
      try {
        Subject.doAs(
            ...
            (PrivilegedExceptionAction<Void>)
                () -> {
                  byte[] response =
                      this.saslClient.evaluateChallenge(
                          fromServer.readBytes(StringSelfDataType.STRING_EOF));
                  if (response != null) {
        ...
        throw ExceptionFactory.createException(
            Messages.getString(
                "AuthenticationLdapSaslClientPlugin.ErrProcessingAuthIter",
                new Object[] {this.authMech.getMechName()}),
            e.getException());
      }
    }
