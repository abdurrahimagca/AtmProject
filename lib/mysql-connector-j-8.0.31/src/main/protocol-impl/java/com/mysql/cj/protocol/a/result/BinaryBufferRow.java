...
public class BinaryBufferRow extends AbstractBufferRow {
  ...
  protected int findAndSeekToOffset(int index) {
    ...
    for (int i = startingIndex; i < index; i++) {
      ...
      if (type != MysqlType.FIELD_TYPE_NULL) {
        int length =
            NativeUtils.getBinaryEncodedLength(this.metadata.getFields()[i].getMysqlTypeId());
        if (length == 0) {
          this.rowFromServer.skipBytes(StringSelfDataType.STRING_LENENC);
        } else if (length == -1) {
          throw ExceptionFactory.createException(
              Messages.getString(
                  "MysqlIO.97", new Object[] {type, i + 1, this.metadata.getFields().length}),
    ...
    switch (type) {
      ...
      default:
        ...
        if (length == 0) {
          ...
          throw ExceptionFactory.createException(
              Messages.getString(
                  "MysqlIO.97", new Object[] {type, index + 1, this.metadata.getFields().length}),
              this.exceptionInterceptor);
        } else {
  ...
  /**
   * Unpacks the bitmask at the head of the row packet that tells us what columns hold null values,
   * and sets the "home" position directly after the bitmask.
   */
    ...
    if (!getNull(columnIndex)) {
      if (length == 0) {
        ...
        throw ExceptionFactory.createException(
            Messages.getString(
                "MysqlIO.97",
                new Object[] {type, columnIndex + 1, this.metadata.getFields().length}),
            this.exceptionInterceptor);
      }
    }
      ...
      if (length == 0) {
        ...
        if (length != value.length) {
          throw ExceptionFactory.createException(
              ...
              "Value length doesn't match the expected one for type " + type,
              this.exceptionInterceptor);
        }
        // write value
