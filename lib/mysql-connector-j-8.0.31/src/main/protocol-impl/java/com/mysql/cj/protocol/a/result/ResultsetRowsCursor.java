...
public class ResultsetRowsCursor extends AbstractResultsetRows implements ResultsetRows {
  ...
  public ResultsetRowsCursor(NativeProtocol ioChannel, ColumnDefinition columnDefinition) {
    ...
    this.rowFactory =
        new BinaryRowFactory(this.protocol, this.metadata, Concurrency.READ_ONLY, false);
    this.commandBuilder =
    ...
    synchronized (this.owner.getSyncMutex()) {
      try {
        ...
        this.protocol.sendCommand(
            this.commandBuilder.buildComStmtFetch(
                this.protocol.getSharedSendPacket(),
                this.owner.getOwningStatementServerId(),
                numRowsToFetch),
            true,
            0);
