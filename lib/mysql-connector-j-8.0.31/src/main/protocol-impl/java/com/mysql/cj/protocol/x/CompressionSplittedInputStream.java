...
/**
 * An {@link InputStream} wrapper that analyzes X Protocol frames and, if compressed, routes them to
 * a secondary compressor-able {@link InputStream} that also knows how to rebuild uncompressed X
 * Protocol frames from compressed ones.
 */
public class CompressionSplittedInputStream extends FilterInputStream {
  ...
  /**
   * Same as {@link InputStream#available()}, except that the exact number of bytes that can be read
   * from the underlying {@link InputStream} may not be accurate until it is known if the next bytes
   * contain compressed data or not.
   *
  ...
  public int available() throws IOException {
    ...
    return (this.frameHeaderConsumed > 0 ? HEADER_LENGTH - this.frameHeaderConsumed : 0)
        + this.in.available();
  }
  ...
  /**
   * Reads bytes from the underlying {@link InputStream} either from the one that gets data directly
   * from the original source {@link InputStream} or from a compressor able {@link InputStream}, if
   * reading of a compressed X Protocol frame is in progress.
   *
  ...
  /**
   * Checks the header of the next X Protocol frame and, depending on its type, sets up this class
   * to read from an alternative compressor able underlying {@link InputStream}.
   *
  ...
  private void peekNextFrame() throws IOException {
    ...
    if (isCompressedFrame()) {
      ...
      this.compressorIn =
          new ConfinedInputStream(
              this.compressorIoStreamsFactory.getInputStreamInstance(
                  new ByteArrayInputStream(compressedMessage.getPayload().toByteArray())),
              (int) compressedMessage.getUncompressedSize());

      // Preemptively set as all bytes consumed since next reads will be redirected to the
   ...
   *
   * @return <code>true</code> if the type of current frame is {@link
   *     com.mysql.cj.x.protobuf.Mysqlx.ServerMessages.Type#COMPRESSION}, <code>false</code>
   *     otherwise.
   */
  private boolean isCompressedFrame() {
    return ServerMessages.Type.forNumber(this.xMessageHeader.getMessageType())
        == ServerMessages.Type.COMPRESSION;
  }
  ...
  private Compression parseCompressedMessage() {
    Parser<Compression> parser =
        (Parser<Compression>)
            MessageConstants.MESSAGE_CLASS_TO_PARSER.get(
                MessageConstants.MESSAGE_TYPE_TO_CLASS.get(ServerMessages.Type.COMPRESSION_VALUE));
    ...
    try {
      ...
      throw ExceptionFactory.createException(
          CJCommunicationsException.class, Messages.getString("Protocol.Compression.Streams.0"), e);
    }
    ...
    try {
      ...
      throw ExceptionFactory.createException(
          WrongArgumentException.class, Messages.getString("Protocol.Compression.Streams.1"), e);
    }
  }
   ...
   *
   * @return <code>true</code> if this frame's bytes weren't all consumed yet, <code>false</code>
   *     otherwise.
   * @throws IOException if any of the underlying I/O operations fail.
   */
  private boolean isDataAvailable() throws IOException {
    return isCompressedDataAvailable()
        || this.frameHeaderConsumed > 0 && this.frameHeaderConsumed < HEADER_LENGTH
  ...
  /**
   * Reads the number of bytes required to fill the given buffer from the underlying {@link
   ...
   * @exception IOException if any of the underlying I/O operations fail.
   */
  ...
  /**
   * Reads the exact number of requested bytes from the underlying {@link InputStream}, blocking if
   * needed.
   *
   * @param b the buffer into which the data is read.
  ...
  /**
   * Reads the exact number of requested bytes from the given {@link InputStream}, blocking if
   * needed.
   *
