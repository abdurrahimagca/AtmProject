...
/**
 * An {@link OutputStream} wrapper that analyzes X Protocol frames and routes them directly to the
 * original underlying {@link OutputStream} or passes them through to a compressor-able {@link
 * OutputStream} to compresses the frame, rebuilds it and then sends a newly compressed X Protocol
 * frame, depending on defined data size threshold.
 */
public class CompressionSplittedOutputStream extends FilterOutputStream {
  ...
  public CompressionSplittedOutputStream(
      OutputStream out, CompressorStreamsFactory ioStreamsFactory) {
    super(out);
  ...
  /**
   * Analyzes the given bytes as an X Protocol frame and, depending on its size, writes it as-is in
   * the underlying {@link OutputStream} or rebuilds it as a compressed X Protocol packet.
   *
  ...
  public void write(byte[] b, int off, int len) throws IOException {
    ...
    if ((off | len | (b.length - (len + off)) | (off + len))
        < 0) { // Borrowed from FilterOutputStream.
    ...
    if (isFrameHeaderBuffered() && !isFrameHeaderWriteComplete()) {
      ...
      this.compressionEnabled =
          this.framePayloadLength
              >= 250; // Compression threshold. May be user configurable in the future.

      if (this.compressionEnabled) {
        this.bufferOut = new ByteArrayOutputStream();
        this.compressorOut =
            this.compressorIoStreamsFactory.getOutputStreamInstance(this.bufferOut);
  ...
  /**
   * Checks if the X Protocol frame payload is ready to be written on the underlying {@link
   * OutputStream}.
   *
   ...
   *
   * @return <code>true</code> if the frame currently in progress was fully written, <code>false
   *     </code> otherwise.
   */
  ...
  /**
   * Finalizes the writing of the compressed {@link OutputStream}, if one is currently in use, by
   * flushing it into a temporary buffer and reassembling the original X Protocol frame into a
   * compressed one. Finally, writes the entire compressed frame into the underlying {@link
   * OutputStream}.
   *
  ...
  private void finalizeWrite() throws IOException {
    if (isWriteComplete()) {
      if (this.compressionEnabled) {
        ...
        Compression compressedMessage =
            Compression.newBuilder()
                .setUncompressedSize(XMessageHeader.HEADER_LENGTH + this.framePayloadLength)
                .setClientMessages(
                    ClientMessages.Type.forNumber(this.xMessageHeader.getMessageType()))
                .setPayload(ByteString.copyFrom(compressedData))
                .build();

        ByteBuffer messageHeader =
            ByteBuffer.allocate(XMessageHeader.HEADER_LENGTH).order(ByteOrder.LITTLE_ENDIAN);
        messageHeader.putInt(
            compressedMessage.getSerializedSize() + XMessageHeader.MESSAGE_TYPE_LENGTH);
