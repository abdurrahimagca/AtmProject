...
/**
 * Factory class for producing inflating and deflating-able {@link InputStream} and {@link
 * OutputStream} instances for a selected compression algorithm.
 */
public class CompressorStreamsFactory {
  ...
  /**
   * Creates an instance of an {@link InputStream} that wraps around the given {@link InputStream}
   * and knows how to inflate data using the algorithm given in this class' constructor.
   *
   * <p>If the compression algorithm operates in steam mode (continuous) then create and reuse one
   * single instance of the compressor {@link InputStream}, else create a new instance every time.
   *
  ...
  public InputStream getInputStreamInstance(InputStream in) {
    ...
    InputStream compressionIn =
        (InputStream)
            Util.getInstance(
                this.compressionAlgorithm.getInputStreamClass().getName(),
                new Class<?>[] {InputStream.class},
                new Object[] {underlyingIn},
                null,
                Messages.getString(
                    "Protocol.Compression.IoFactory.0",
                    new Object[] {
                      this.compressionAlgorithm.getInputStreamClass().getName(),
  ...
  /**
   * Creates an instance of an {@link OutputStream} that wraps around the given {@link OutputStream}
   * and knows how to deflate data using the algorithm given in this class' constructor.
   *
   * <p>If the compression algorithm operates in steam mode (continuous) then create and reuse one
   * single instance of the compressor {@link OutputStream}, else create a new instance every time.
   *
  ...
  public OutputStream getOutputStreamInstance(OutputStream out) {
    ...
    OutputStream compressionOut =
        (OutputStream)
            Util.getInstance(
                this.compressionAlgorithm.getOutputStreamClass().getName(),
                new Class<?>[] {OutputStream.class},
                new Object[] {underlyingOut},
                null,
                Messages.getString(
                    "Protocol.Compression.IoFactory.1",
                    new Object[] {
                      this.compressionAlgorithm.getOutputStreamClass().getName(),
                      this.compressionAlgorithm
                    }));
