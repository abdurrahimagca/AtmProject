...
/**
 * An {@link InputStream} wrapper that reads from a queue of underlying {@link InputStream}s, giving
 * the impression that all data is coming from a single, continuous, source.
 */
public class ContinuousInputStream extends FilterInputStream {
  ...
  /**
   * Reads one byte from the underlying {@link InputStream}. When EOF is reached, then reads from
   * the next {@link InputStream} in the queue.
   *
  ...
  /**
   * Reads bytes from the underlying {@link InputStream}. When EOF is reached, then reads from the
   * next {@link InputStream} in the queue.
   *
  ...
  /**
   * Closes the currently active {@link InputStream} and replaces it by the the head of the {@link
   * InputStream}s queue.
   *
   * @return <code>true</code> if the currently active {@link InputStream} was replaced by a new
   *     one.
