...
public class FieldFactory implements ProtocolEntityFactory<Field, XMessage> {
  ...
  private Field columnMetaDataToField(ColumnMetaData col, String characterSet) {
    try {
      ...
      MysqlType mysqlType =
          findMysqlType(col.getType(), col.getContentType(), col.getFlags(), collationIndex);
      int mysqlTypeId = xProtocolTypeToMysqlType(col.getType(), col.getContentType());

      // flags translation; unsigned is handled in Field by checking the MysqlType, so here we check
      // others
      short flags = (short) 0;
      if (col.getType().equals(FieldType.UINT)
          && 0 < (col.getFlags() & XPROTOCOL_COLUMN_FLAGS_UINT_ZEROFILL)) {
        flags |= MysqlType.FIELD_FLAG_ZEROFILL;
      } else if (col.getType().equals(FieldType.BYTES)
          && 0 < (col.getFlags() & XPROTOCOL_COLUMN_FLAGS_BYTES_RIGHTPAD)) {
        mysqlType = MysqlType.CHAR;
      } else if (col.getType().equals(FieldType.DATETIME)
          && 0 < (col.getFlags() & XPROTOCOL_COLUMN_FLAGS_DATETIME_TIMESTAMP)) {
      ...
      // DECIMAL_NOT_SPECIFIED=31 is the MySQL internal constant value used to indicate that
      // NUMERIC_SCALE is not applicable.
      // It's probably a mistake that it's exposed by protocol as a decimals and it should be
      // replaced with 0.
      switch (mysqlType) {
        case FLOAT:
        case FLOAT_UNSIGNED:
  ...
  private MysqlType findMysqlType(FieldType type, int contentType, int flags, int collationIndex) {
    switch (type) {
      ...
      case FLOAT:
        return 0 < (flags & XPROTOCOL_COLUMN_FLAGS_FLOAT_UNSIGNED)
            ? MysqlType.FLOAT_UNSIGNED
            : MysqlType.FLOAT;
      case DOUBLE:
        return 0 < (flags & XPROTOCOL_COLUMN_FLAGS_DOUBLE_UNSIGNED)
            ? MysqlType.DOUBLE_UNSIGNED
            : MysqlType.DOUBLE;
      case DECIMAL:
        return 0 < (flags & XPROTOCOL_COLUMN_FLAGS_DECIMAL_UNSIGNED)
  ...
  /**
   * Map a X Protocol type code from `ColumnMetaData.FieldType' to a MySQL type constant. These are
   * the only types that will be present in {@link XProtocolRow} results.
   *
   * @param type the type as the ColumnMetaData.FieldType
   * @param contentType the inner type
   * @return A <b>FIELD_TYPE</b> constant from {@link MysqlType} corresponding to the combination of
   *     input parameters.
   */
  private int xProtocolTypeToMysqlType(FieldType type, int contentType) {
    switch (type) {
      ...
      case DATETIME:
        // may be a timestamp or just a date if time values are missing. metadata doesn't
