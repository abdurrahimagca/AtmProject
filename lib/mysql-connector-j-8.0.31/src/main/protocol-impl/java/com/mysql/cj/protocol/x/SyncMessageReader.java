...
/**
 * Synchronous-only implementation of {@link MessageReader}. This implementation wraps a {@link
 * java.io.InputStream}.
 */
public class SyncMessageReader implements MessageReader<XMessageHeader, XMessage> {
  ...
  public SyncMessageReader(
      FullReadInputStream inputStream, ProtocolEventHandler protocolEventHandler) {
  ...
  private <T extends GeneratedMessageV3> T readMessageLocal(
      Class<T> messageClass, boolean fromQueue) {
    ...
    try {
      ...
      if (msg instanceof Frame
          && ((Frame) msg).getType() == Frame.Type.WARNING_VALUE
          && ((Frame) msg).getScope() == Frame.Scope.GLOBAL) {
        XWarning w = new XWarning((Frame) msg);
        int code = (int) w.getCode();
        if (code == MysqlErrorNumbers.ER_SERVER_SHUTDOWN
            || code == MysqlErrorNumbers.ER_IO_READ_ERROR
          ...
          if (this.protocolEventHandler != null) {
            this.protocolEventHandler.invokeListeners(
                code == MysqlErrorNumbers.ER_SERVER_SHUTDOWN
                    ? EventType.SERVER_SHUTDOWN
                    : EventType.SERVER_CLOSED_SESSION,
                ex);
          }
  ...
  public XMessage readMessage(Optional<XMessage> reuse, int expectedType) throws IOException {
    ...
    synchronized (this.waitingSyncOperationMonitor) {
      try {
        Class<? extends GeneratedMessageV3> expectedClass =
            MessageConstants.getMessageClassForType(expectedType);

        List<Notice> notices = null;
        XMessageHeader hdr;
        while ((hdr = readHeader()).getMessageType() == ServerMessages.Type.NOTICE_VALUE
            && expectedType != ServerMessages.Type.NOTICE_VALUE) {
          if (notices == null) {
          ...
          notices.add(
              Notice.getInstance(
                  new XMessage(
                      readMessageLocal(
                          MessageConstants.getMessageClassForType(ServerMessages.Type.NOTICE_VALUE),
                          true))));
        }

        Class<? extends GeneratedMessageV3> messageClass =
            MessageConstants.getMessageClassForType(hdr.getMessageType());
        // ensure that parsed message class matches incoming tag
        if (expectedClass != messageClass) {
          throw new WrongArgumentException(
              "Unexpected message class. Expected '"
  ...
  private class ListenersDispatcher implements Runnable {
    /**
     * The timeout value for queue.poll(timeout, unit) defining the time after which we close and
     * unregister the dispatching thread. On the other hand, a bigger timeout value allows us to
      ...
      synchronized (SyncMessageReader.this.waitingSyncOperationMonitor) {
        ...
        try {
          while (true) {
            ...
            if ((l =
                    SyncMessageReader.this.messageListenerQueue.poll(
