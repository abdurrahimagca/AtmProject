...
public class SyncMessageSender implements MessageSender<XMessage>, PacketSentTimeHolder {
    ...
    synchronized (this.waitingAsyncOperationMonitor) {
      ...
      try {
        ...
        if (this.maxAllowedPacket > 0 && size > this.maxAllowedPacket) {
          throw new CJPacketTooBigException(
              Messages.getString(
                  "PacketTooBigException.1", new Object[] {size, this.maxAllowedPacket}));
        }
        // for debugging
        // System.err.println("Initiating write of message (size=" + size + ", tag=" +
        // ClientMessages.Type.valueOf(type) + ")");
        byte[] sizeHeader =
  ...
  public CompletableFuture<?> send(
      XMessage message, CompletableFuture<?> future, Runnable callback) {
    synchronized (this.waitingAsyncOperationMonitor) {
      CompletionHandler<Long, Void> resultHandler =
          new ErrorToFutureCompletionHandler<>(future, callback);
      MessageLite msg = message.getMessage();
      try {
        send(message);
