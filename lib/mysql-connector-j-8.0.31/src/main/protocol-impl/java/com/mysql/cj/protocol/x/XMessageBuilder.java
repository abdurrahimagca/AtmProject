...
public class XMessageBuilder implements MessageBuilder<XMessage> {
    ...
    keyValuePair.forEach(
        (k, v) -> {
          ...
          if (XServerCapabilities.KEY_SESSION_CONNECT_ATTRS.equals(k)
              || XServerCapabilities.KEY_COMPRESSION.equals(k)) {
            MysqlxDatatypes.Object.Builder attrB = MysqlxDatatypes.Object.newBuilder();
            ((Map<String, Object>) v)
                .forEach(
                    (name, value) ->
                        attrB.addFld(
                            ObjectField.newBuilder()
                                .setKey(name)
                                .setValue(ExprUtil.argObjectToScalarAny(value))
                                .build()));
   ...
   */
  public XMessage buildDocInsert(
      String schemaName, String collectionName, List<String> json, boolean upsert) {
    Insert.Builder builder =
        Insert.newBuilder().setCollection(ExprUtil.buildCollection(schemaName, collectionName));
  ...
  /**
   * Initialize a {@link Insert.Builder} for table data model with common data for prepared and
   * non-prepared executions.
   *
   * @param schemaName the schema name
   ...
   */
  @SuppressWarnings("unchecked")
  private Insert.Builder commonRowInsertBuilder(
      String schemaName, String tableName, InsertParams insertParams) {
    Insert.Builder builder =
        Insert.newBuilder()
  ...
  /**
   * Initialize an {@link Update.Builder} for collection data model with common data for prepared
   * and non-prepared executions.
   *
   ...
   */
  private Update.Builder commonDocUpdateBuilder(
      FilterParams filterParams, List<UpdateSpec> updates) {
    Update.Builder builder =
        Update.newBuilder().setCollection((Collection) filterParams.getCollection());
    updates.forEach(
        u -> {
   ...
   * @return an {@link XMessage} instance
   */
  public XMessage buildPrepareDocUpdate(
      int preparedStatementId, FilterParams filterParams, List<UpdateSpec> updates) {
    Update.Builder updateBuilder = commonDocUpdateBuilder(filterParams, updates);
    applyFilterParams(
        filterParams,
        updateBuilder::addAllOrder,
        updateBuilder::setLimitExpr,
        updateBuilder::setCriteria);

    Prepare.Builder builder = Prepare.newBuilder().setStmtId(preparedStatementId);
    builder.setStmt(
        OneOfMessage.newBuilder()
            .setType(OneOfMessage.Type.UPDATE)
  ...
  /**
   * Initialize an {@link Update.Builder} for table data model with common data for prepared and
   * non-prepared executions.
   *
   * @param filterParams the filter parameters
   ...
   */
  @SuppressWarnings("unchecked")
  private Update.Builder commonRowUpdateBuilder(
      FilterParams filterParams, UpdateParams updateParams) {
    Update.Builder builder =
        Update.newBuilder()
            .setDataModel(DataModel.TABLE)
            .setCollection((Collection) filterParams.getCollection());
    ((Map<ColumnIdentifier, Expr>) updateParams.getUpdates())
        .entrySet().stream()
            .map(
                e ->
                    UpdateOperation.newBuilder()
                        .setOperation(UpdateType.SET)
                        .setSource(e.getKey())
                        .setValue(e.getValue())
                        .build())
   ...
   * @return an {@link XMessage} instance
   */
  public XMessage buildPrepareRowUpdate(
      int preparedStatementId, FilterParams filterParams, UpdateParams updateParams) {
  ...
  /**
   * Initialize a {@link Find.Builder} for collection data model with common data for prepared and
   * non-prepared executions.
   *
   * @param filterParams the filter parameters
  ...
  private Find.Builder commonFindBuilder(FilterParams filterParams) {
    Find.Builder builder =
        Find.newBuilder().setCollection((Collection) filterParams.getCollection());
  ...
  public XMessage buildPrepareFind(int preparedStatementId, FilterParams filterParams) {
    ...
    applyFilterParams(
        filterParams,
        findBuilder::addAllOrder,
        findBuilder::setLimitExpr,
        findBuilder::setCriteria);

    Prepare.Builder builder = Prepare.newBuilder().setStmtId(preparedStatementId);
    builder.setStmt(
        OneOfMessage.newBuilder()
            .setType(OneOfMessage.Type.FIND)
            .setFind(findBuilder.build())
            .build());
    return new XMessage(builder.build());
  }
  ...
  private Delete.Builder commonDeleteBuilder(FilterParams filterParams) {
    Delete.Builder builder =
        Delete.newBuilder().setCollection((Collection) filterParams.getCollection());
    return builder;
  }
  ...
  public XMessage buildPrepareDelete(int preparedStatementId, FilterParams filterParams) {
    ...
    applyFilterParams(
        filterParams,
        deleteBuilder::addAllOrder,
        deleteBuilder::setLimitExpr,
        deleteBuilder::setCriteria);

    Prepare.Builder builder = Prepare.newBuilder().setStmtId(preparedStatementId);
    builder.setStmt(
        OneOfMessage.newBuilder()
            .setType(OneOfMessage.Type.DELETE)
  ...
  /**
   * Initialize a {@link StmtExecute.Builder} with common data for prepared and non-prepared
   * executions.
   *
  ...
  public XMessage buildSqlStatement(String statement, List<Object> args) {
    ...
    if (args != null) {
      builder.addAllArgs(
          args.stream().map(ExprUtil::argObjectToScalarAny).collect(Collectors.toList()));
    }
    return new XMessage(builder.build());
  }
  ...
  public XMessage buildPrepareSqlStatement(int preparedStatementId, String statement) {
    ...
    builder.setStmt(
        OneOfMessage.newBuilder()
            .setType(OneOfMessage.Type.STMT)
            .setStmtExecute(stmtExecBuilder.build())
            .build());
    return new XMessage(builder.build());
  }
   ...
   *
   * <p>Abstract the process of setting the filter params on the operation message builder.
   *
  ...
  private static void applyFilterParams(
      ...
      Consumer<List<Order>> setOrder,
      Consumer<Limit> setLimit,
      Consumer<Expr> setCriteria,
  ...
  /**
   * Apply the given filter params to the builder object (represented by the setter methods) using
   * the variant that takes a <code>LimitExpr</code> and no <code>Args</code>. This variant is
   * suitable for building prepared statements prepare messages.
   *
   * <p>Abstract the process of setting the filter params on the operation message builder.
   *
   * @param filterParams the filter params to apply
  ...
  private static void applyFilterParams(
    ...
    if (filterParams.getLimit() != null) {
      LimitExpr.Builder lb =
          LimitExpr.newBuilder().setRowCount(ExprUtil.buildPlaceholderExpr(numberOfArgs));
  ...
  public XMessage buildPrepareExecute(int preparedStatementId, FilterParams filterParams) {
    ...
    if (filterParams.getArgs() != null) {
      builder.addAllArgs(
          ((List<Scalar>) filterParams.getArgs())
              .stream()
    ...
    if (filterParams.getLimit() != null) {
      ...
      if (filterParams.supportsOffset()) {
        builder.addArgs(
            ExprUtil.anyOf(
                ExprUtil.scalarOf(
                    filterParams.getOffset() != null ? filterParams.getOffset() : 0)));
      }
    }
    return new XMessage(builder.build());
  }
  ...
  public XMessage buildCreateCollection(
      String schemaName, String collectionName, CreateCollectionOptions options) {
    if (schemaName == null) {
      throw new XProtocolError(
          Messages.getString("CreateTableStatement.0", new String[] {"schemaName"}));
    }
    if (collectionName == null) {
      throw new XProtocolError(
          Messages.getString("CreateTableStatement.0", new String[] {"collectionName"}));
    }

    Builder argsBuilder =
        com.mysql.cj.x.protobuf.MysqlxDatatypes.Object.newBuilder()
            .addFld(
                ObjectField.newBuilder().setKey("name").setValue(ExprUtil.buildAny(collectionName)))
            .addFld(
                ObjectField.newBuilder().setKey("schema").setValue(ExprUtil.buildAny(schemaName)));
    ...
    if (options.getReuseExisting() != null) {
      ...
      optBuilder.addFld(
          ObjectField.newBuilder()
              .setKey("reuse_existing")
              .setValue(ExprUtil.buildAny(options.getReuseExisting())));
    }
    ...
    if (options.getValidation() != null) {
      ...
      if (options.getValidation().getSchema() != null) {
        validationBuilder.addFld(
            ObjectField.newBuilder()
                .setKey("schema")
                .setValue(ExprUtil.buildAny(options.getValidation().getSchema())));
      }
      if (options.getValidation().getLevel() != null) {
        validationBuilder.addFld(
            ObjectField.newBuilder()
                .setKey("level")
                .setValue(
                    ExprUtil.buildAny(options.getValidation().getLevel().name().toLowerCase())));
      }
      optBuilder.addFld(
          ObjectField.newBuilder()
              .setKey("validation")
              .setValue(Any.newBuilder().setType(Any.Type.OBJECT).setObj(validationBuilder)));
    }
    if (hasOptions) {
      argsBuilder.addFld(
          ObjectField.newBuilder()
              .setKey("options")
              .setValue(Any.newBuilder().setType(Any.Type.OBJECT).setObj(optBuilder)));
    }
  ...
  public XMessage buildModifyCollectionOptions(
      String schemaName, String collectionName, ModifyCollectionOptions options) {
    if (schemaName == null) {
      throw new XProtocolError(
          Messages.getString("CreateTableStatement.0", new String[] {"schemaName"}));
    }
    ...
    return new XMessage(
        buildXpluginCommand(
            XpluginStatementCommand.XPLUGIN_STMT_MODIFY_COLLECTION_OPTIONS,
            Any.newBuilder().setType(Any.Type.OBJECT).setObj(argsBuilder).build()));
  }
  ...
  public XMessage buildCreateCollection(String schemaName, String collectionName) {
    ...
    return new XMessage(
        buildXpluginCommand(
            XpluginStatementCommand.XPLUGIN_STMT_CREATE_COLLECTION,
            Any.newBuilder()
                .setType(Any.Type.OBJECT)
                .setObj(
                    com.mysql.cj.x.protobuf.MysqlxDatatypes.Object.newBuilder()
                        .addFld(
                            ObjectField.newBuilder()
                                .setKey("name")
                                .setValue(ExprUtil.buildAny(collectionName)))
                        .addFld(
                            ObjectField.newBuilder()
                                .setKey("schema")
                                .setValue(ExprUtil.buildAny(schemaName))))
                .build()));
  }
  ...
  public XMessage buildListObjects(String schemaName, String pattern) {
    ...
    return new XMessage(
        buildXpluginCommand(
            ...
            Any.newBuilder().setType(Any.Type.OBJECT).setObj(obj).build()));
  }

  public XMessage buildEnableNotices(String... notices) {
    com.mysql.cj.x.protobuf.MysqlxDatatypes.Array.Builder abuilder =
        com.mysql.cj.x.protobuf.MysqlxDatatypes.Array.newBuilder();
                    ...
                    com.mysql.cj.x.protobuf.MysqlxDatatypes.Object.newBuilder()
                        .addFld(
                            ObjectField.newBuilder()
                                .setKey("notice")
                                .setValue(
                                    Any.newBuilder().setType(Any.Type.ARRAY).setArray(abuilder))))
                .build()));
  }
  ...
  public XMessage buildCreateCollectionIndex(
      String schemaName, String collectionName, CreateIndexParams params) {
    com.mysql.cj.x.protobuf.MysqlxDatatypes.Object.Builder builder =
        com.mysql.cj.x.protobuf.MysqlxDatatypes.Object.newBuilder();
    builder
        .addFld(
            ObjectField.newBuilder()
                .setKey("name")
                .setValue(ExprUtil.buildAny(params.getIndexName())))
        .addFld(
            ObjectField.newBuilder()
                .setKey("collection")
                .setValue(ExprUtil.buildAny(collectionName)))
    ...
    if (params.getIndexType() != null) {
      builder.addFld(
          ObjectField.newBuilder()
              .setKey("type")
              .setValue(ExprUtil.buildAny(params.getIndexType())));
    }

    com.mysql.cj.x.protobuf.MysqlxDatatypes.Array.Builder aBuilder =
        com.mysql.cj.x.protobuf.MysqlxDatatypes.Array.newBuilder();
    for (IndexField indexField : params.getFields()) {
      com.mysql.cj.x.protobuf.MysqlxDatatypes.Object.Builder fBuilder =
          com.mysql.cj.x.protobuf.MysqlxDatatypes.Object.newBuilder()
              .addFld(
                  ObjectField.newBuilder()
                      .setKey("member")
                      .setValue(ExprUtil.buildAny(indexField.getField())))
              .addFld(
                  ObjectField.newBuilder()
                      .setKey("type")
                      .setValue(ExprUtil.buildAny(indexField.getType())));
      if (indexField.isRequired() != null) {
        fBuilder.addFld(
            ObjectField.newBuilder()
                .setKey("required")
                .setValue(ExprUtil.buildAny(indexField.isRequired())));
      }
      if (indexField.getOptions() != null) {
        fBuilder.addFld(
            ObjectField.newBuilder()
                ...
                .setValue(
                    Any.newBuilder()
                        .setType(Any.Type.SCALAR)
                        .setScalar(
                            Scalar.newBuilder()
                                .setType(Scalar.Type.V_UINT)
                                .setVUnsignedInt(indexField.getOptions()))
                        .build()));
      }
                ...
                .setValue(
                    Any.newBuilder()
                        ...
                        .setScalar(
                            Scalar.newBuilder()
                                .setType(Scalar.Type.V_UINT)
                                .setVUnsignedInt(indexField.getSrid()))
                        .build()));
      }
      if (indexField.isArray() != null) {
        fBuilder.addFld(
            ObjectField.newBuilder()
                .setKey("array")
                .setValue(ExprUtil.buildAny(indexField.isArray())));
      }
    ...
    builder.addFld(
        ObjectField.newBuilder()
            .setKey("constraint")
            .setValue(Any.newBuilder().setType(Any.Type.ARRAY).setArray(aBuilder)));
    return new XMessage(
        buildXpluginCommand(
  ...
  public XMessage buildDropCollectionIndex(
      String schemaName, String collectionName, String indexName) {
    return new XMessage(
        buildXpluginCommand(
                ...
                .setObj(
                    com.mysql.cj.x.protobuf.MysqlxDatatypes.Object.newBuilder()
                        .addFld(
                            ObjectField.newBuilder()
                                .setKey("name")
                                .setValue(ExprUtil.buildAny(indexName)))
                        .addFld(
                            ObjectField.newBuilder()
                                .setKey("collection")
                                .setValue(ExprUtil.buildAny(collectionName)))
  ...
  public XMessage buildSha256MemoryAuthContinue(
      String user, String password, byte[] nonce, String database) {
    // TODO: encoding for all this?
    String encoding = "UTF8";
    byte[] databaseBytes =
        database == null ? new byte[] {} : StringUtils.getBytes(database, encoding);
    byte[] userBytes = user == null ? new byte[] {} : StringUtils.getBytes(user, encoding);
    byte[] passwordBytes =
        password == null || password.length() == 0
            ? new byte[] {}
            : StringUtils.getBytes(password, encoding);
  ...
  public XMessage buildMysql41AuthContinue(
      String user, String password, byte[] salt, String database) {
    // TODO: encoding for all this?
    String encoding = "UTF8";
    byte[] userBytes = user == null ? new byte[] {} : StringUtils.getBytes(user, encoding);
    byte[] passwordBytes =
    ...
    if (password != null && password.length() > 0) {
      ...
      hashedPassword =
          String.format("*%040x", new java.math.BigInteger(1, hashedPassword)).getBytes();
    }

    // this is what would happen in the SASL provider but we don't need the overhead of all the
    // plumbing.
    byte[] reply = new byte[databaseBytes.length + userBytes.length + hashedPassword.length + 2];
  ...
  public XMessage buildPlainAuthStart(String user, String password, String database) {
    // SASL requests information from the app through callbacks. We provide the username and
    // password by these callbacks. This implementation works for
    // PLAIN and would also work for CRAM-MD5. Additional standardized methods may require
    ...
    try {
      // now we create the client object we use which can handle PLAIN mechanism for "X Protocol" to
      // "serverName"
      String[] mechanisms = new String[] {"PLAIN"};
      String authorizationId =
          database == null || database.trim().length() == 0
              ? null
              : database; // as per protocol spec
      String protocol = "X Protocol";
      Map<String, ?> props = null;
      // TODO: >> serverName. Is this of any use in our X Protocol exchange? Should be defined to be
      // blank or something.
      String serverName = "<unknown>";
      SaslClient saslClient =
          Sasl.createSaslClient(
              mechanisms, authorizationId, protocol, serverName, props, callbackHandler);
  ...
  public XMessage buildExternalAuthStart(String database) {
    ...
    try {
      // now we create the client object we use which can handle EXTERNAL mechanism for "X Protocol"
      // to "serverName"
  ...
  public XMessage buildExpectOpen() {
    return new XMessage(
        MysqlxExpect.Open.newBuilder()
            .addCond(
                MysqlxExpect.Open.Condition.newBuilder()
                    .setConditionKey(MysqlxExpect.Open.Condition.Key.EXPECT_FIELD_EXIST_VALUE)
                    .setConditionValue(ByteString.copyFromUtf8("6.1")))
            .build());
  }

  @Override
  public XMessage buildComQuery(
      XMessage sharedPacket,
      Session sess,
