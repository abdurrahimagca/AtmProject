...
public class XProtocol extends AbstractProtocol<XMessage> implements Protocol<XMessage> {
  ...
  /**
   * Keeps track of whether this X Server session supports prepared statements. True by default
   * until first failure of a statement prepare.
   */
  ...
  private ReferenceQueue<PreparableStatement<?>> preparableStatementRefQueue =
      new ReferenceQueue<>();
  private Map<Integer, PreparableStatementFinalizer> preparableStatementFinalizerReferences =
      new TreeMap<>();
  ...
  public XProtocol(HostInfo hostInfo, PropertySet propertySet) {
    ...
    // Override common connectTimeout with xdevapi.connect-timeout to provide unified logic in
    // StandardSocketFactory
    RuntimeProperty<Integer> connectTimeout =
        propertySet.getIntegerProperty(PropertyKey.connectTimeout);
    RuntimeProperty<Integer> xdevapiConnectTimeout =
        propertySet.getIntegerProperty(PropertyKey.xdevapiConnectTimeout);
  ...
  public void init(
    ...
    this.messageToProtocolEntityFactory.put(
        ColumnMetaData.class,
        new FieldFactory("latin1")); // TODO configure metadata character set from server session
    ...
    this.messageToProtocolEntityFactory.put(
        FetchDoneMoreResultsets.class, new FetchDoneMoreResultsFactory());
    ...
    this.messageToProtocolEntityFactory.put(
        com.mysql.cj.x.protobuf.Mysqlx.Ok.class, new OkFactory());
  }
  ...
  /**
   * Set client capabilities of current session. Must be done before authentication ({@link
  ...
  public void sendCapabilities(Map<String, Object> keyValuePair) {
    keyValuePair.forEach(
        (k, v) -> ((XServerCapabilities) getServerSession().getCapabilities()).setCapability(k, v));
  ...
  public void negotiateSSLConnection() {
    ...
    if (!((XServerCapabilities) this.serverSession.getCapabilities())
        .hasCapability(XServerCapabilities.KEY_TLS)) {
      throw new CJCommunicationsException(
          "A secure connection is required but the server is not configured with SSL.");
    }

    // the message reader is async and is always "reading". we need to stop it to use the socket for
    // the TLS handshake
  ...
  public void negotiateCompression() {
    Compression compression =
        this.propertySet
            .<Compression>getEnumProperty(PropertyKey.xdevapiCompression.getKeyName())
            .getValue();
    if (compression == Compression.DISABLED) {
      return;
    }

    Map<String, List<String>> compressionCapabilities =
        this.serverSession.serverCapabilities.getCompression();
    if (compressionCapabilities.isEmpty()
        ...
        || compressionCapabilities
            .get(XServerCapabilities.SUBKEY_COMPRESSION_ALGORITHM)
            .isEmpty()) {
      if (compression == Compression.REQUIRED) {
        throw ExceptionFactory.createException(
            WrongArgumentException.class, Messages.getString("Protocol.Compression.0"));
      } // TODO Log "Compression negotiation failed. Connection will proceed uncompressed."
      return;
    }

    RuntimeProperty<String> compressionAlgorithmsProp =
        this.propertySet.getStringProperty(PropertyKey.xdevapiCompressionAlgorithms.getKeyName());
    String compressionAlgorithmsList = compressionAlgorithmsProp.getValue();
    compressionAlgorithmsList =
        compressionAlgorithmsList == null ? "" : compressionAlgorithmsList.trim();
    String[] compressionAlgorithmsOrder;
    String[] compressionAlgsOrder = compressionAlgorithmsList.split("\\s*,\\s*");
    compressionAlgorithmsOrder =
        Arrays.stream(compressionAlgsOrder)
            .sequential()
    ...
    String compressionExtensions =
        this.propertySet
            .getStringProperty(PropertyKey.xdevapiCompressionExtensions.getKeyName())
            .getValue();
    compressionExtensions = compressionExtensions == null ? "" : compressionExtensions.trim();
    Map<String, CompressionAlgorithm> compressionAlgorithms =
        getCompressionExtensions(compressionExtensions);

    Optional<String> algorithmOpt =
        Arrays.stream(compressionAlgorithmsOrder)
            .sequential()
            .filter(
                compressionCapabilities.get(XServerCapabilities.SUBKEY_COMPRESSION_ALGORITHM)
                    ::contains)
    ...
    if (!algorithmOpt.isPresent()) {
      if (compression == Compression.REQUIRED) {
        throw ExceptionFactory.createException(
            WrongArgumentException.class, Messages.getString("Protocol.Compression.2"));
      } // TODO Log "Compression negotiation failed. Connection will proceed uncompressed."
      return;
    }
  ...
  public void beforeHandshake() {
    ...
    String attributes =
        this.propertySet.getStringProperty(PropertyKey.xdevapiConnectionAttributes).getValue();
    if (attributes == null || !attributes.equalsIgnoreCase("false")) {
      Map<String, String> attMap =
          getConnectionAttributesMap("true".equalsIgnoreCase(attributes) ? "" : attributes);
      this.clientCapabilities.put(XServerCapabilities.KEY_SESSION_CONNECT_ATTRS, attMap);
    }

    // Override JDBC (global) SSL properties with X DevAPI ones to provide unified logic in
    // ExportControlled via common SSL properties.
    RuntimeProperty<XdevapiSslMode> xdevapiSslMode =
        this.propertySet.<XdevapiSslMode>getEnumProperty(PropertyKey.xdevapiSslMode);
    RuntimeProperty<SslMode> jdbcSslMode =
        this.propertySet.<SslMode>getEnumProperty(PropertyKey.sslMode);
    ...
    RuntimeProperty<String> xdevapiSslKeyStoreUrl =
        this.propertySet.getStringProperty(PropertyKey.xdevapiSslKeyStoreUrl);
    RuntimeProperty<String> jdbcClientCertKeyStoreUrl =
        this.propertySet.getStringProperty(PropertyKey.clientCertificateKeyStoreUrl);
    ...
    RuntimeProperty<String> xdevapiSslKeyStoreType =
        this.propertySet.getStringProperty(PropertyKey.xdevapiSslKeyStoreType);
    RuntimeProperty<String> jdbcClientCertKeyStoreType =
        this.propertySet.getStringProperty(PropertyKey.clientCertificateKeyStoreType);
    ...
    RuntimeProperty<String> xdevapiSslKeyStorePassword =
        this.propertySet.getStringProperty(PropertyKey.xdevapiSslKeyStorePassword);
    RuntimeProperty<String> jdbcClientCertKeyStorePassword =
        this.propertySet.getStringProperty(PropertyKey.clientCertificateKeyStorePassword);
    if (xdevapiSslKeyStorePassword.isExplicitlySet()
        || !jdbcClientCertKeyStorePassword.isExplicitlySet()) {
      jdbcClientCertKeyStorePassword.setValue(xdevapiSslKeyStorePassword.getValue());
    }
    RuntimeProperty<Boolean> xdevapiFallbackToSystemKeyStore =
        this.propertySet.getBooleanProperty(PropertyKey.xdevapiFallbackToSystemKeyStore);
    RuntimeProperty<Boolean> jdbcFallbackToSystemKeyStore =
        this.propertySet.getBooleanProperty(PropertyKey.fallbackToSystemKeyStore);
    if (xdevapiFallbackToSystemKeyStore.isExplicitlySet()
        || !jdbcFallbackToSystemKeyStore.isExplicitlySet()) {
      jdbcFallbackToSystemKeyStore.setValue(xdevapiFallbackToSystemKeyStore.getValue());
    }
    RuntimeProperty<String> xdevapiSslTrustStoreUrl =
        this.propertySet.getStringProperty(PropertyKey.xdevapiSslTrustStoreUrl);
    RuntimeProperty<String> jdbcTrustCertKeyStoreUrl =
        this.propertySet.getStringProperty(PropertyKey.trustCertificateKeyStoreUrl);
    ...
    RuntimeProperty<String> xdevapiSslTrustStoreType =
        this.propertySet.getStringProperty(PropertyKey.xdevapiSslTrustStoreType);
    RuntimeProperty<String> jdbcTrustCertKeyStoreType =
        this.propertySet.getStringProperty(PropertyKey.trustCertificateKeyStoreType);
    if (xdevapiSslTrustStoreType.isExplicitlySet()
        || !jdbcTrustCertKeyStoreType.isExplicitlySet()) {
      jdbcTrustCertKeyStoreType.setValue(xdevapiSslTrustStoreType.getValue());
    }
    RuntimeProperty<String> xdevapiSslTrustStorePassword =
        this.propertySet.getStringProperty(PropertyKey.xdevapiSslTrustStorePassword);
    RuntimeProperty<String> jdbcTrustCertKeyStorePassword =
        this.propertySet.getStringProperty(PropertyKey.trustCertificateKeyStorePassword);
    if (xdevapiSslTrustStorePassword.isExplicitlySet()
        || !jdbcTrustCertKeyStorePassword.isExplicitlySet()) {
      jdbcTrustCertKeyStorePassword.setValue(xdevapiSslTrustStorePassword.getValue());
    }
    RuntimeProperty<Boolean> xdevapiFallbackToSystemTrustStore =
        this.propertySet.getBooleanProperty(PropertyKey.xdevapiFallbackToSystemTrustStore);
    RuntimeProperty<Boolean> jdbcFallbackToSystemTrustStore =
        this.propertySet.getBooleanProperty(PropertyKey.fallbackToSystemTrustStore);
    if (xdevapiFallbackToSystemTrustStore.isExplicitlySet()
        || !jdbcFallbackToSystemTrustStore.isExplicitlySet()) {
    ...
    if (sslMode.getValue()
        == SslMode.PREFERRED) { // PREFERRED mode is not applicable for X Protocol.
      sslMode.setValue(SslMode.REQUIRED);
    }

    if (sslMode.getValue() != SslMode.DISABLED) {
      RuntimeProperty<String> xdevapiTlsVersions =
          this.propertySet.getStringProperty(PropertyKey.xdevapiTlsVersions);
      RuntimeProperty<String> jdbcEnabledTlsProtocols =
          this.propertySet.getStringProperty(PropertyKey.tlsVersions);
      ...
      RuntimeProperty<String> xdevapiTlsCiphersuites =
          this.propertySet.getStringProperty(PropertyKey.xdevapiTlsCiphersuites);
      RuntimeProperty<String> jdbcEnabledSslCipherSuites =
          this.propertySet.getStringProperty(PropertyKey.tlsCiphersuites);
    ...
    if (this.clientCapabilities.size() > 0) {
      try {
        ...
        // happens when connecting to xplugin which doesn't support this feature. Just ignore this
        // error.
  ...
  /**
   * Parses and validates the value given for the connection option
   * 'xdevapi.compression-extensions'. With the information obtained, creates a map of supported
   * compression algorithms.
   *
   * @param compressionExtensions the value of the option 'xdevapi.compression-algorithm' containing
   *     a comma separated list of triplets with the format
   *     "algorithm-name:inflater-InputStream-class-name:deflater-OutputStream-class-name".
   * @return a map with all the supported compression algorithms, both natively supported and user
   *     configured.
   */
  private Map<String, CompressionAlgorithm> getCompressionExtensions(String compressionExtensions) {
    Map<String, CompressionAlgorithm> compressionExtensionsMap =
        CompressionAlgorithm.getDefaultInstances();
    ...
    for (String compressionExtAlg : compressionExtAlgs) {
      ...
      if (compressionExtAlgParts.length != 3) {
        throw ExceptionFactory.createException(
            WrongArgumentException.class, Messages.getString("Protocol.Compression.1"));
      }
      ...
      CompressionAlgorithm compressionAlg =
          new CompressionAlgorithm(algorithmName, inputStreamClassName, outputStreamClassName);
        ...
        this.reader =
            new SyncMessageReader(
                new FullReadInputStream(
                    new CompressionSplittedInputStream(
      ...
      try {
        this.sender =
            new SyncMessageSender(
                new CompressionSplittedOutputStream(
                    this.socketConnection.getMysqlOutput(),
                    new CompressorStreamsFactory(this.compressionAlgorithm)));
      } catch (IOException e) {
  ...
  public void initServerSession() {
    ...
    // TODO: can use a simple default for this as we don't need metadata. need to prevent against
    // exceptions though
    ColumnDefinition metadata = readMetadata();
    long count =
        new XProtocolRowInputStream(metadata, this, null)
            .next()
            .getValue(0, new LongValueFactory(this.propertySet));
            ...
            switch (((XSessionStateChanged) notice).getParamType()) {
              case Notice.SessionStateChanged_CLIENT_ID_ASSIGNED:
                this.getServerSession()
                    .getCapabilities()
                    .setThreadId(((XSessionStateChanged) notice).getValue().getVUnsignedInt());
                break;
  ...
  public byte[] readAuthenticateContinue() {
    try {
      AuthenticateContinue msg =
          (AuthenticateContinue)
              this.reader
                  .readMessage(null, ServerMessages.Type.SESS_AUTHENTICATE_CONTINUE_VALUE)
                  .getMessage();
      byte[] data = msg.getAuthData().toByteArray();
      if (data.length != 20) {
        throw AssertionFailedException.shouldNotHappen(
            "Salt length should be 20, but is " + data.length);
      }
      return data;
    } catch (IOException e) {
      throw new XProtocolError(e.getMessage(), e);
  ...
  public boolean hasMoreResults() {
    try {
      if (((SyncMessageReader) this.reader).getNextNonNoticeMessageType()
          == ServerMessages.Type.RESULTSET_FETCH_DONE_MORE_RESULTSETS_VALUE) {
        this.reader.readMessage(
            null, ServerMessages.Type.RESULTSET_FETCH_DONE_MORE_RESULTSETS_VALUE);
        if (((SyncMessageReader) this.reader).getNextNonNoticeMessageType()
            == ServerMessages.Type.RESULTSET_FETCH_DONE_VALUE) {
    ...
    try {
      ...
      while (!done) {
        ...
        Class<? extends GeneratedMessageV3> msgClass =
            (Class<? extends GeneratedMessageV3>) mess.getMessage().getClass();
        ...
        if (Error.class.equals(msgClass)) {
          ...
          throw new WrongArgumentException(
              "Unhandled msg class (" + msgClass + ") + msg=" + mess.getMessage());
        }
        ...
        if ((notices = mess.getNotices()) != null) {
          notices.stream().forEach(resultBuilder::addProtocolEntity);
            ...
            resultBuilder.addProtocolEntity(
                this.messageToProtocolEntityFactory.get(msgClass).createFromMessage(mess));
      }
  ...
  public boolean hasResults() {
    try {
      return ((SyncMessageReader) this.reader).getNextNonNoticeMessageType()
  ...
  public void drainRows() {
    try {
      while (((SyncMessageReader) this.reader).getNextNonNoticeMessageType()
          == ServerMessages.Type.RESULTSET_ROW_VALUE) {
  ...
  public ColumnDefinition readMetadata(Consumer<Notice> noticeConsumer) {
    try {
      ...
      @SuppressWarnings("unchecked")
      ProtocolEntityFactory<Field, XMessage> fieldFactory =
          (ProtocolEntityFactory<Field, XMessage>)
              this.messageToProtocolEntityFactory.get(ColumnMetaData.class);
  ...
  public ColumnDefinition readMetadata(Field f, Consumer<Notice> noticeConsumer) {
    try {
      ...
      while (((SyncMessageReader) this.reader).getNextNonNoticeMessageType()
          == ServerMessages.Type
              .RESULTSET_COLUMN_META_DATA_VALUE) { // use this construct to read at least one
        XMessage mess =
            this.reader.readMessage(null, ServerMessages.Type.RESULTSET_COLUMN_META_DATA_VALUE);
        if (noticeConsumer != null && (notices = mess.getNotices()) != null) {
          notices.stream().forEach(noticeConsumer::accept);
  ...
  /**
   * Checks if enough statements have been executed in this MySQL server so that another prepare
   * statement attempt should be done.
   *
   * @return {@code true} if enough executions have been done since last time a prepared statement
   *     failed to prepare
   */
  ...
  /**
   * Returns an id to be used as a client-managed prepared statement id. The method {@link
   * #freePreparedStatementId(int)} must be called when the prepared statement is deallocated so
   * that the same id can be re-used.
   *
  ...
  public int getNewPreparedStatementId(PreparableStatement<?> preparableStatement) {
    ...
    this.preparableStatementFinalizerReferences.put(
        preparedStatementId,
        new PreparableStatementFinalizer(
            preparableStatement, this.preparableStatementRefQueue, preparedStatementId));
    return preparedStatementId;
  }

  /**
   * Frees a prepared statement id so that it can be reused. Note that freeing an id from an active
   * prepared statement will result in a statement prepare conflict next time one gets prepared with
   * the same released id.
   *
  ...
  public boolean failedPreparingStatement(int preparedStatementId, XProtocolError e) {
    ...
    if (e.getErrorCode() == MysqlErrorNumbers.ER_UNKNOWN_COM_ERROR
        && this.preparableStatementFinalizerReferences.isEmpty()) {
      // The server doesn't recognize the protocol message, so it doesn't support prepared
      // statements.
   ...
   * command at a time. Results are read lazily from the wire. It is necessary to flush any pending
   * result before starting a new command. This method performs the flush if necessary.
   */
  protected void newCommand() {
    ...
    // Before continuing clean up any abandoned prepared statements that were not properly
    // deallocated.
    if (this.supportsPreparedStatements) {
      ...
      while ((ref = this.preparableStatementRefQueue.poll()) != null) {
        ...
        try {
          this.sender.send(
              ((XMessageBuilder) this.messageBuilder)
                  .buildPrepareDeallocate(psf.getPreparedStatementId()));
          readQueryResult(new OkBuilder());
        } catch (XProtocolError e) {
  ...
  public <M extends Message, R extends QueryResult> R query(
  ...
  public <M extends Message, R extends QueryResult> CompletableFuture<R> queryAsync(
      M message, ResultBuilder<R> resultBuilder) {
    newCommand();
    CompletableFuture<R> f = new CompletableFuture<>();
    MessageListener<XMessage> l =
        new ResultMessageListener<>(this.messageToProtocolEntityFactory, resultBuilder, f);
   ...
   *
   * <p><b>NOTE:</b> This must be called before authentication.
   *
  ...
  public ServerCapabilities readServerCapabilities() {
    try {
      ...
      return new XServerCapabilities(
          ((Capabilities)
                  this.reader
                      .readMessage(null, ServerMessages.Type.CONN_CAPABILITIES_VALUE)
                      .getMessage())
              .getCapabilitiesList().stream()
                  .collect(toMap(Capability::getName, Capability::getValue)));
  ...
  public void reset() {
    ...
    if (this.useSessionResetKeepOpen == null) {
      try {
        ...
        if (e.getErrorCode() != MysqlErrorNumbers.ER_X_EXPECT_FIELD_EXISTS_FAILED
            && /* for MySQL 5.7 */ e.getErrorCode()
                != MysqlErrorNumbers.ER_X_EXPECT_BAD_CONDITION) {
          throw e;
        }
    ...
    if (this.useSessionResetKeepOpen) {
      ...
      if (this.clientCapabilities.containsKey(XServerCapabilities.KEY_SESSION_CONNECT_ATTRS)) {
        // this code may never work because xplugin connection attributes were introduced later than
        // new session reset
        Map<String, Object> reducedClientCapabilities = new HashMap<>();
        reducedClientCapabilities.put(
            XServerCapabilities.KEY_SESSION_CONNECT_ATTRS,
    ...
    // No prepared statements survived to Mysqlx.Session.Reset. Reset all related control
    // structures.
    if (this.supportsPreparedStatements) {
      this.retryPrepareStatementCountdown = 0;
  ...
  public boolean versionMeetsMinimum(int major, int minor, int subminor) {
    // TODO: expose this via ServerVersion so calls look like
    // x.getServerVersion().meetsMinimum(major, minor, subminor)
    throw ExceptionFactory.createException(CJOperationNotSupportedException.class, "Not supported");
  }
  ...
  public <T extends ProtocolEntity> T read(
      Class<T> requiredClass, ProtocolEntityFactory<T, XMessage> protocolEntityFactory)
      throws IOException {
    throw ExceptionFactory.createException(CJOperationNotSupportedException.class, "Not supported");
  }
