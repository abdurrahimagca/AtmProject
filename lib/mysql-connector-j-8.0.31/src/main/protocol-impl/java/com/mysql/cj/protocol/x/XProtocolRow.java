...
public class XProtocolRow implements com.mysql.cj.result.Row {
  ...
  public <T> T getValue(int columnIndex, ValueFactory<T> vf) {
    ...
    // for debugging
    // System.err.println("getValue bytes = " +
    // com.mysql.cj.core.util.StringUtils.dumpAsHex(byteString.toByteArray(),
    // byteString.toByteArray().length));
    // try {
    ...
    switch (f.getMysqlTypeId()) {
      case MysqlType.FIELD_TYPE_BIT:
        this.wasNull = false;
        return XProtocolDecoder.instance.decodeBit(
            byteString.toByteArray(), 0, byteString.size(), vf);

      case MysqlType.FIELD_TYPE_DATETIME:
        this.wasNull = false;
        // TODO scale is unavailable from X Protocol
        // return XProtocolDecoder.instance.decodeTimestamp(byteString.toByteArray(), 0,
        // byteString.size(), f.getDecimals(), vf);
        return XProtocolDecoder.instance.decodeTimestamp(
            byteString.toByteArray(), 0, byteString.size(), 6, vf);

      case MysqlType.FIELD_TYPE_DOUBLE:
        this.wasNull = false;
        return XProtocolDecoder.instance.decodeDouble(
            byteString.toByteArray(), 0, byteString.size(), vf);

      case MysqlType.FIELD_TYPE_ENUM:
        this.wasNull = false;
        return XProtocolDecoder.instance.decodeByteArray(
            byteString.toByteArray(), 0, byteString.size(), f, vf);

      case MysqlType.FIELD_TYPE_FLOAT:
        this.wasNull = false;
        return XProtocolDecoder.instance.decodeFloat(
            byteString.toByteArray(), 0, byteString.size(), vf);
      ...
      case MysqlType.FIELD_TYPE_JSON:
        this.wasNull = false;
        // TODO: do we need to really do anything special with JSON? just return correct stuff with
        // getObject() I guess
        return XProtocolDecoder.instance.decodeByteArray(
            byteString.toByteArray(), 0, byteString.size(), f, vf);
      ...
      case MysqlType.FIELD_TYPE_LONGLONG:
        ...
        this.wasNull = false;
        if (f.isUnsigned()) {
          return XProtocolDecoder.instance.decodeUInt8(
              byteString.toByteArray(), 0, byteString.size(), vf);
        }
        return XProtocolDecoder.instance.decodeInt8(
            byteString.toByteArray(), 0, byteString.size(), vf);

      case MysqlType.FIELD_TYPE_NEWDECIMAL:
        this.wasNull = false;
        return XProtocolDecoder.instance.decodeDecimal(
            byteString.toByteArray(), 0, byteString.size(), vf);

      case MysqlType.FIELD_TYPE_SET:
        this.wasNull = false;
        return XProtocolDecoder.instance.decodeSet(
            byteString.toByteArray(), 0, byteString.size(), f, vf);
        // return XProtocolDecoder.instance.decodeByteArray(byteString.toByteArray(), 0,
        // byteString.size(), vf);

      case MysqlType.FIELD_TYPE_TIME:
        this.wasNull = false;
        // TODO scale is unavailable from X Protocol
        // return XProtocolDecoder.instance.decodeTime(byteString.toByteArray(), 0,
        // byteString.size(), f.getDecimals(), vf);
        return XProtocolDecoder.instance.decodeTime(
            byteString.toByteArray(), 0, byteString.size(), 6, vf);
    ...
    // XProtocolDecoder.MYSQL_TYPE_TO_DECODER_FUNCTION.get(f.getMysqlTypeId());
    //            if (decoderFunction != null) {
    //                this.wasNull = false;
    //                return
    // decoderFunction.apply(CodedInputStream.newInstance(byteString.toByteArray()), vf);
    //            }
    //            throw new DataReadException("Unknown MySQL type constant: " + f.getMysqlTypeId());
