...
/**
 * This interface contains methods that are considered the "vendor extension" to the JDBC API for
 * MySQL's implementation of java.sql.Connection.
 *
 * <p>For those looking further into the driver implementation, it is not an API that is used for
 * plugability of implementations inside our driver (which is why there are still references to
 * ConnectionImpl throughout the code).
 */
public interface JdbcConnection
    extends java.sql.Connection, MysqlConnection, TransactionEventHandler {
   ...
   * @throws SQLException if authentication fails, or some other error occurs while performing the
   *     command.
   */
  ...
  /**
   * Prepares a statement on the client, using client-side emulation (irregardless of the
   * configuration property 'useServerPrepStmts') with the same semantics as the
   * java.sql.Connection.prepareStatement() method with the same argument types.
   *
   * @param sql statement
   ...
   */
  java.sql.PreparedStatement clientPrepareStatement(
   ...
   */
  java.sql.PreparedStatement clientPrepareStatement(String sql, String[] autoGenKeyColNames)
      throws SQLException;
  ...
  /**
   * Reports how long this connection has been idle. This time (reported in milliseconds) is updated
   * once a query has completed.
   *
  ...
  /**
   * Has this connection tried to execute a query on the "source" server (first host in a multiple
   * host list).
   *
  ...
  /**
   * Is this connection connected to the first host in the list if there is a list of servers in the
   * URL?
   *
  ...
  /**
   * Resets the server-side state of this connection. Doesn't work if isParanoid() is set (it will
   * become a no-op in this case). Usually only used from connection pooling code.
   *
  ...
  /**
   * Prepares a statement on the server (irregardless of the configuration property
   * 'useServerPrepStmts') with the same semantics as the java.sql.Connection.prepareStatement()
   ...
   * @see java.sql.Connection#prepareStatement(String, int, int)
   */
  java.sql.PreparedStatement serverPrepareStatement(
      String sql, int resultSetType, int resultSetConcurrency) throws SQLException;
   ...
   * @see java.sql.Connection#prepareStatement(String, String[])
   */
  java.sql.PreparedStatement serverPrepareStatement(String sql, String[] autoGenKeyColNames)
      throws SQLException;
   ...
   *
   * @param comment the comment that will be prepended to all statements sent to the server.
   */
  ...
  /**
   * Returns the -session- value of 'auto_increment_increment' from the server if it exists, or '1'
   * if not.
   *
  ...
  /**
   * Returns cached metadata (or null if not cached) for the given query, which must match
   * _exactly_.
   *
   * <p>This method is synchronized by the caller on getMutex(), so if calling this method from
   * internal code in the driver, make sure it's synchronized on the mutex that guards communication
   * with the server.
   *
   ...
   *
   * @param useSessionStatus in some cases, for example when restoring connection with
   *     autoReconnect=true, we can rely only on saved readOnly state, so use useSessionStatus=false
   *     in that case
   ...
   * @param skipLocalTeardown if true, driver tries to close connection normally, performing
   *     rollbacks, closing open statements etc; otherwise the force close is performed
