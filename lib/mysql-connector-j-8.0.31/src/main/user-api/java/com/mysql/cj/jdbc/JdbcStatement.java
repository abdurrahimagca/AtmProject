 ...
 * MySQL's implementation of java.sql.Statement.
 *
 ...
 * StatementImpl throughout the code).
 */
public interface JdbcStatement extends java.sql.Statement, Query {
  ...
  /**
   * Workaround for containers that 'check' for sane values of Statement.setFetchSize() so that
   * applications can use the Java variant of libmysql's mysql_use_result() behavior.
   *
  ...
  /**
   * Resets this statements fetch size and result set type to the values they had before
   ...
   *
   * <p>This stream will be read to completion upon execution of a "LOAD DATA LOCAL INFILE"
   * statement, and will automatically be closed by the driver, so it needs to be reset before each
   * call to execute*() that would cause the MySQL server to request data to fulfill the request for
   * "LOAD DATA LOCAL INFILE".
   *
   * <p>If this value is set to NULL, the driver will revert to using a FileInputStream or
   ...
   *
   * <p>This method returns NULL if no such stream has been set via setLocalInfileInputStream().
   *
   * @return input stream
   */
