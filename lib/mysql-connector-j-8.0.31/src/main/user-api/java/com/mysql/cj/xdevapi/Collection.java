...
public interface Collection extends DatabaseObject {
  ...
  // TODO we have to keep add(DbDoc document) method because the DbDoc does extend the
  // TreeMap<String, JsonValue>,
  // thus w/o this method the col.add(dbdoc) will call the add(Map<String, ?> doc) method (which is
  // not implemented yet)
   ...
   *
   * <p>Example: collection.createIndex("myIndex", "{\"fields\": [{\"field\": \"$.myGeoJsonField\",
   ...
   *       <li>type: string, (optional) the type of index. One of INDEX or SPATIAL (case
   *           insensitive). Default is INDEX and may be omitted.
   *     </ul>
   ...
   *       <li>type: string, one of the supported SQL column types to map the field into (see below
   *           for a list). For numeric types, the optional UNSIGNED keyword may follow. For the
   *           TEXT type, the length to consider for indexing may be added. Type descriptions are
   *           case insensitive.
   ...
   *       <li>fields: array of IndexField objects, each describing a single document member to be
   *           included in the index (see below)
   *       <li>type: string, (optional) the type of index. One of INDEX or SPATIAL. Default is INDEX
   *           and may be omitted.
   *     </ul>
   *     where single IndexField description consists of the following fields:
   *     <ul>
   ...
   * @param doc the new document, which may contain expressions. If document contains an _id value,
   *     it is ignored.
   ...
   *
   * @param id the document id of the document to be replaced
   * @param doc the new document, which may contain expressions. If doc contains an _id value and it
   *     does not match the given id then the error will be thrown.
   ...
   *
   * @param id the document id of the document to be replaced
   * @param jsonString the new document, given as JSON string, which may contain expressions. If doc
   *     contains an _id value and it does not match the given id then the error will be thrown.
   * @return Result object, which will indicate the number of affected documents (0 - if none, 1 -
   *     if added, 2 - if replaced)
   */
