...
public interface ModifyStatement extends Statement<ModifyStatement, Result> {
  ...
  /**
   * Takes in a patch object and applies it on all documents matching the modify() filter, using the
   * JSON_MERGE_PATCH() function. Please note that {@link DbDoc} does not support expressions as a
   * field values, please use {@link #patch(String)} method if you need such functionality.
   *
   ...
   * difference that document field values can be nested expressions in addition to literal values.
   * <br>
   * Example:<br>
