 ...
 *
 * <p>An application using the Session class can be run against a single MySQL server or large
 * number of MySQL servers forming a sharding cluster with no code changes.
 *
 * <p>When using literal/verbatim SQL the common API patterns are mostly the same compared to using
 * DML and CRUD operations on Tables and Collections. Two differences exist: setting the current
 * schema and escaping names.
 *
 * <p>You cannot call {@link Session#getSchema(String)} or {@link Session#getDefaultSchema()} to
 * obtain a {@link Schema} object against which you can issue verbatin SQL statements. The Schema
 * object does not feature a sql() function.
 *
 * <p>The sql() function is a method of the {@link Session} class. Use {@link Session#sql(String)}
 * and the SQL command USE to change the current schema
 *
 * <p>Session session = SessionFactory.getSession("root:s3kr3t@localhost");<br>
 * session.sql("USE test");
 *
 * <p>If a Session has been established using a data source file the name of the default schema can
 * be obtained to change the current database.
 *
 ...
 *
 * <p>A quoting function exists to escape SQL names/identifiers. StringUtils.quoteIdentifier(String,
 ...
 *
 * <p>// use bind syntax for values<br>
 ...
 *
 * <p>Users of the CRUD API do not need to escape identifiers. This is true for working with
...
public interface Session {
  ...
  /**
   * Creates or replaces a transaction savepoint with the given name. Calling this method more than
  ...
  /**
   * Releases the named savepoint. This method will succeed as long as the given save point has not
   * been already rolled back or released. Rolling back to a savepoint prior to the one named will
