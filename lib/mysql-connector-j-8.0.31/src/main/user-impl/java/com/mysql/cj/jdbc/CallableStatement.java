...
public class CallableStatement extends ClientPreparedStatement
  ...
  public class CallableStatementParamInfo implements ParameterMetaData {
    ...
    /**
     * Constructor that converts a full list of parameter metadata into one that only represents the
     * placeholders present in the {CALL ()}.
     *
      ...
      for (int i = 0; i < parameterMapLength; i++) {
        if (localParameterMap[i] != 0) {
          CallableStatementParam param =
              fullParamInfo.parameterList.get(localParameterMap[i] + offset);
    ...
    protected void checkBounds(int paramIndex) throws SQLException {
      ...
      if ((paramIndex < 0) || (localParamIndex >= this.numParameters)) {
        throw SQLError.createSQLException(
            Messages.getString(
                "CallableStatement.11", new Object[] {paramIndex, this.numParameters}),
    ...
    public String getParameterClassName(int arg0) throws SQLException {
      ...
      switch (mysqlType) {
        case YEAR:
          ...
          // if (length == 1) {
          //    if
          // (propertySet.getBooleanReadableProperty(PropertyKey.transformedBitIsBoolean).getValue()) {
          //        return MysqlType.BOOLEAN;
          //    } else if
          // (propertySet.getBooleanReadableProperty(PropertyKey.tinyInt1isBit).getValue()) {
  ...
  private static String mangleParameterName(String origParameterName) {
    ...
    StringBuilder paramNameBuf =
        new StringBuilder(PARAMETER_NAMESPACE_PREFIX.length() + origParameterName.length());
  ...
  public CallableStatement(JdbcConnection conn, CallableStatementParamInfo paramInfo)
    ...
    if (this.callingStoredFunction) {
      ((PreparedQuery) this.query)
          .setParameterCount(((PreparedQuery) this.query).getParameterCount() + 1);
    }

    this.retrieveGeneratedKeys = true; // not provided for in the JDBC spec

    this.noAccessToProcedureBodies =
        conn.getPropertySet().getBooleanProperty(PropertyKey.noAccessToProcedureBodies).getValue();
  }
   ...
   * @throws SQLException if an error occurs
   */
  protected static CallableStatement getInstance(
      JdbcConnection conn, String sql, String db, boolean isFunctionCall) throws SQLException {
   ...
   */
  protected static CallableStatement getInstance(
      JdbcConnection conn, CallableStatementParamInfo paramInfo) throws SQLException {
  ...
  private void generateParameterMap() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      // if the user specified some parameters as literals, we need to provide a map from the
      // specified placeholders to the actual parameter numbers
      ...
      if (this.paramInfo != null && q.getParameterCount() != parameterCountFromMetaData) {
        ...
        int startPos =
            this.callingStoredFunction
                ? StringUtils.indexOfIgnoreCase(q.getOriginalSql(), "SELECT")
          ...
          if (parenOpenPos != -1) {
            int parenClosePos =
                StringUtils.indexOfIgnoreCase(
                    parenOpenPos, q.getOriginalSql(), ")", "'", "'", SearchMode.__FULL);
  ...
  private CallableStatementParam checkIsOutputParam(int paramIndex) throws SQLException {
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (this.noAccessToProcedureBodies) {
        ...
        throw SQLError.createSQLException(
            Messages.getString("CallableStatement.9", new Object[] {paramIndex}),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
  ...
  private void checkStreamability() throws SQLException {
    if (this.hasOutputParams && createStreamingResultSet()) {
      throw SQLError.createSQLException(
          Messages.getString("CallableStatement.14"),
  ...
  private void fakeParameterTypes(boolean isReallyProcedure) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      for (int i = 0; i < ((PreparedQuery) this.query).getParameterCount(); i++) {
        ...
        row[11] =
            s2b(Integer.toString(java.sql.DatabaseMetaData.procedureNullableUnknown)); // nullable
      ...
      try {
        ...
        List<?> parseList =
            StringUtils.splitDBdotName(
                procName, "", quotedId, this.session.getServerSession().isNoBackslashEscapesSet());
        String tmpDb = "";
        ...
        paramTypesRs =
            this.session
                        ...
                        .getValue()
                    == DatabaseTerm.SCHEMA
                ? dbmd.getProcedureColumns(
                    null, useDb ? this.getCurrentDatabase() : tmpDb /* null */, procName, "%")
                : dbmd.getProcedureColumns(
                    useDb ? this.getCurrentDatabase() : tmpDb /* null */, null, procName, "%");
  ...
  private void convertGetProcedureColumnsToInternalDescriptors(java.sql.ResultSet paramTypesRs)
      throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
  ...
  public boolean execute() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      //
      // $ CREATE  TABLE testBug14562 (row_order INT, signed_field MEDIUMINT, unsigned_field
      // MEDIUMINT UNSIGNED)
      // $ INSERT INTO testBug14562 VALUES (1, -8388608, 0), (2, 8388607, 16777215)
      // $ CREATE  PROCEDURE sp_testBug14562_1 (OUT param_1 MEDIUMINT, OUT param_2 MEDIUMINT
      // UNSIGNED)
      //    BEGIN
      //     SELECT signed_field, unsigned_field INTO param_1, param_2 FROM testBug14562 WHERE
      // row_order=1;
      //    END
      // $ CALL sp_testBug14562_1(@com_mysql_jdbc_outparam_param_1,
  ...
  private String extractProcedureName() throws SQLException {
    ...
    throw SQLError.createSQLException(
        Messages.getString("CallableStatement.1"),
  ...
  protected String fixParameterName(String paramNameIn) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (this.noAccessToProcedureBodies) {
        throw SQLError.createSQLException(
            Messages.getString("CallableStatement.23"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
  ...
  public BigDecimal getBigDecimal(int parameterIndex, int scale) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      BigDecimal retValue =
          rs.getBigDecimal(mapOutputParameterIndexToRsIndex(parameterIndex), scale);
  ...
  protected int getNamedParamIndex(String paramName, boolean forOut) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if (this.noAccessToProcedureBodies) {
        throw SQLError.createSQLException(
            "No access to parameters by name when connection has been configured not to access"
                + " procedure bodies",
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
      ...
      if ((paramName == null) || (paramName.length() == 0)) {
        throw SQLError.createSQLException(
            Messages.getString("CallableStatement.2"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
      ...
      if (this.paramInfo == null
          || (namedParamInfo = this.paramInfo.getParameter(paramName)) == null) {
        throw SQLError.createSQLException(
            Messages.getString("CallableStatement.3", new Object[] {paramName}),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
      ...
      if (forOut && !namedParamInfo.isOut) {
        throw SQLError.createSQLException(
            Messages.getString("CallableStatement.5", new Object[] {paramName}),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
      ...
      throw SQLError.createSQLException(
          Messages.getString("CallableStatement.6", new Object[] {paramName}),
          MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
          getExceptionInterceptor());
    }
  }
  ...
  public <T> T getObject(int parameterIndex, Class<T> type) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      T retVal =
          ((ResultSetImpl) rs).getObject(mapOutputParameterIndexToRsIndex(parameterIndex), type);
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (this.outputParameterResults == null) {
        if (this.paramInfo.numberOfParameters() == 0) {
          throw SQLError.createSQLException(
              Messages.getString("CallableStatement.7"),
              MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
              getExceptionInterceptor());
        }
        throw SQLError.createSQLException(
            Messages.getString("CallableStatement.8"),
            MysqlErrorNumbers.SQL_STATE_GENERAL_ERROR,
            getExceptionInterceptor());
      }
  ...
  protected int mapOutputParameterIndexToRsIndex(int paramIndex) throws SQLException {
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (rsIndex == NOT_OUTPUT_PARAMETER_INDICATOR) {
        throw SQLError.createSQLException(
            Messages.getString("CallableStatement.21", new Object[] {paramIndex}),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
  ...
  protected void registerOutParameter(
      int parameterIndex, MysqlType mysqlType, @SuppressWarnings("unused") int scale)
      throws SQLException {
  ...
  protected void registerOutParameter(
      int parameterIndex, MysqlType mysqlType, @SuppressWarnings("unused") String typeName)
      throws SQLException {
    registerOutParameter(
        parameterIndex, mysqlType); // TODO is that correct that we ignore typeName?
  }
  ...
  public void registerOutParameter(String parameterName, SQLType sqlType, int scale)
    ...
    if (sqlType instanceof MysqlType) {
      ...
      registerOutParameter(
          getNamedParamIndex(parameterName, true), sqlType.getVendorTypeNumber(), scale);
    }
  }
  ...
  private void retrieveOutParams() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (numParameters > 0) {
        ...
        for (Iterator<CallableStatementParam> paramIter = this.paramInfo.iterator();
            paramIter.hasNext(); ) {
        ...
        if (hadOutputParams) {
          // We can't use 'ourself' to execute this query, or any pending result sets would be
          // overwritten
          ...
          try {
            ...
            this.outputParameterResults =
                this.resultSetFactory.createFromResultsetRows(
                    outParamRs.getConcurrency(),
                    outParamRs.getType(),
                    ((com.mysql.cj.jdbc.result.ResultSetInternalMethods) outParamRs)
                        .getRows()); // note, doesn't work for updatable result sets
      ...
      if (this.paramInfo.numParameters > 0) {
        for (Iterator<CallableStatementParam> paramIter = this.paramInfo.iterator();
          ...
          if (inParamInfo.isOut && inParamInfo.isIn) {
            ...
            try {
              setPstmt =
                  ((Wrapper) this.connection.clientPrepareStatement(queryBuf.toString()))
              ...
              setPstmt
                  ...
                  .setFromBindValue(
                      ...
                      ((PreparedQuery) this.query)
                          .getQueryBindings()
                          .getBindValues()[inParamInfo.index]);
          ...
          if (!this.callingStoredFunction && outParamInfo.isOut) {
            ...
            if (this.placeholderToParameterIndexMap == null) {
              ...
              if (!found) {
                throw SQLError.createSQLException(
                    Messages.getString(
                        "CallableStatement.21", new Object[] {outParamInfo.paramName}),
                    MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
                    getExceptionInterceptor());
              }
            }
            ...
            this.setBytes(
                outParamIndex, StringUtils.getBytes(outParameterName, this.charEncoding), false);
          }
        }
      }
    }
        ...
        if (procName.indexOf(".") != -1) {
          ...
          procName =
              StringUtils.toString(
                  StringUtils.stripEnclosure(StringUtils.getBytes(procName), "`", "`"));
        }
        ps =
            this.connection.prepareStatement(
                "SELECT SQL_DATA_ACCESS FROM information_schema.routines WHERE routine_schema = ?"
                    + " AND routine_name = ?");
        ps.setMaxRows(0);
        ps.setFetchSize(0);
  ...
  protected boolean checkReadOnlySafeStatement() throws SQLException {
    if (QueryInfo.isReadOnlySafeQuery(
        ((PreparedQuery) this.query).getOriginalSql(),
        this.session.getServerSession().isNoBackslashEscapesSet())) {
      String sql = ((PreparedQuery) this.query).getOriginalSql();
      int statementKeywordPos =
          QueryInfo.indexOfStatementKeyword(
              sql, this.session.getServerSession().isNoBackslashEscapesSet());
      if (StringUtils.startsWithIgnoreCaseAndWs(sql, "CALL", statementKeywordPos)
        ...
        if (!this.connection
            .isReadOnly()) { // Only proceed with checking the routine body if really needed.
          return true;
        }
