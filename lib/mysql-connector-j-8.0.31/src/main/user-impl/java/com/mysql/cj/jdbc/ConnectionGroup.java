...
public class ConnectionGroup {
  ...
  public long registerConnectionProxy(
      LoadBalancedConnectionProxy proxy, List<String> localHostList) {
   ...
   * @param hostPortPair The host:port pair to remove.
   * @param removeExisting Whether affects existing load-balanced connections or only new ones.
   * @param waitForGracefulFailover If true instructs the load-balanced connections to fail-over the
   *     underlying active connection before removing this host, otherwise remove immediately.
   * @throws SQLException if a database access error occurs
   */
  public synchronized void removeHost(
      String hostPortPair, boolean removeExisting, boolean waitForGracefulFailover)
      throws SQLException {
    ...
    if (this.hostList.remove(hostPortPair)) {
      ...
      throw SQLError.createSQLException(
          Messages.getString("ConnectionGroup.1", new Object[] {hostPortPair}), null);
    }
