 ...
 *
 * <p>A Connection's database is able to provide information describing its tables, its supported
 * SQL grammar, its stored procedures, the capabilities of this connection, etc. This information is
...
public class ConnectionImpl implements JdbcConnection, SessionEventListener, Serializable {
  ...
  public void setProxy(JdbcConnection proxy) {
    ...
    this.topProxy = proxy;
    this.realProxy =
        this.topProxy instanceof MultiHostMySQLConnection
            ? ((MultiHostMySQLConnection) proxy).getThisAsProxy()
            : null;
  }

  // this connection has to be proxied when using multi-host settings so that statements get routed
  // to the right physical connection
  ...
  /**
   * Used as a key for caching callable statements which (may) depend on current database...In
   * 5.0.x, they don't (currently), but stored procedure names soon will, so current database is a
    ...
    public boolean equals(Object obj) {
      ...
      if (obj != null && CompoundCacheKey.class.isAssignableFrom(obj.getClass())) {
        ...
        if (this.componentOne == null
            ? another.componentOne == null
            : this.componentOne.equals(another.componentOne)) {
          return this.componentTwo == null
              ? another.componentTwo == null
  ...
  protected static synchronized int getNextRoundRobinHostIndex(String url, List<?> hostList) {
    // we really do "random" here, because you don't get even distribution when this is coupled with
    // connection pools
   ...
   * ConcurrentModificationException when statements unregister themselves while we iterate over the
   * list.
   */
  private final CopyOnWriteArrayList<JdbcStatement> openStatements = new CopyOnWriteArrayList<>();

  private LRUCache<CompoundCacheKey, CallableStatement.CallableStatementParamInfo>
  ...
  /**
   * The type map for UDTs (not implemented, but used by some third-party vendors, most notably IBM
   * WebSphere)
   */
  ...
  public ConnectionImpl(HostInfo hostInfo) throws SQLException {
    ...
    try {
      // Stash away for later, used to clone this connection for Statement.cancel and
      ...
      // we can't cache fixed values here because properties are still not initialized with user
      // provided values
      this.autoReconnectForPools =
          this.propertySet.getBooleanProperty(PropertyKey.autoReconnectForPools);
      ...
      this.emulateUnsupportedPstmts =
          this.propertySet.getBooleanProperty(PropertyKey.emulateUnsupportedPstmts);
      ...
      this.useLocalSessionState =
          this.propertySet.getBooleanProperty(PropertyKey.useLocalSessionState);
      this.useServerPrepStmts = this.propertySet.getBooleanProperty(PropertyKey.useServerPrepStmts);
      this.processEscapeCodesForPrepStmts =
          this.propertySet.getBooleanProperty(PropertyKey.processEscapeCodesForPrepStmts);
      this.useLocalTransactionState =
          this.propertySet.getBooleanProperty(PropertyKey.useLocalTransactionState);
      this.disconnectOnExpiredPasswords =
          this.propertySet.getBooleanProperty(PropertyKey.disconnectOnExpiredPasswords);
      this.readOnlyPropagatesToServer =
          this.propertySet.getBooleanProperty(PropertyKey.readOnlyPropagatesToServer);

      String exceptionInterceptorClasses =
          this.propertySet.getStringProperty(PropertyKey.exceptionInterceptors).getStringValue();
      if (exceptionInterceptorClasses != null && !"".equals(exceptionInterceptorClasses)) {
        this.exceptionInterceptor =
            new ExceptionInterceptorChain(
                exceptionInterceptorClasses, this.props, this.session.getLog());
      }
      ...
      if (this.propertySet.getBooleanProperty(PropertyKey.cacheCallableStmts).getValue()) {
        this.parsedCallableStatementCache =
            new LRUCache<>(
                this.propertySet.getIntegerProperty(PropertyKey.callableStmtCacheSize).getValue());
      }

      if (this.propertySet.getBooleanProperty(PropertyKey.allowMultiQueries).getValue()) {
        this.propertySet
            .getProperty(PropertyKey.cacheResultSetMetadata)
            .setValue(false); // we don't handle this yet
      }

      if (this.propertySet.getBooleanProperty(PropertyKey.cacheResultSetMetadata).getValue()) {
        this.resultSetMetadataCache =
            new LRUCache<>(
                this.propertySet.getIntegerProperty(PropertyKey.metadataCacheSize).getValue());
      }

      if (this.propertySet.getStringProperty(PropertyKey.socksProxyHost).getStringValue() != null) {
        this.propertySet
            .getProperty(PropertyKey.socketFactory)
            .setValue(SocksProxySocketFactory.class.getName());
      }
    ...
    try {
      ...
      AbandonedConnectionCleanupThread.trackConnection(
          this, this.getSession().getNetworkResources());
      ...
      throw SQLError.createSQLException(
          this.propertySet.getBooleanProperty(PropertyKey.paranoid).getValue()
              ? Messages.getString("Connection.0")
              : Messages.getString(
                  "Connection.1",
                  new Object[] {
                    this.session.getHostInfo().getHost(), this.session.getHostInfo().getPort()
                  }),
          MysqlErrorNumbers.SQL_STATE_COMMUNICATION_LINK_FAILURE,
          ex,
  ...
  public void unSafeQueryInterceptors() throws SQLException {
    this.queryInterceptors =
        this.queryInterceptors.stream()
            .map(u -> ((NoSubInterceptorWrapper) u).getUnderlyingInterceptor())
  ...
  public void initializeSafeQueryInterceptors() throws SQLException {
    this.queryInterceptors =
        Util.<QueryInterceptor>loadClasses(
                ...
                getExceptionInterceptor())
            .stream()
            .map(o -> new NoSubInterceptorWrapper(o.init(this, this.props, this.session.getLog())))
            .collect(Collectors.toList());
  }
  ...
  private boolean canHandleAsServerPreparedStatement(String sql) throws SQLException {
    ...
    if (this.cachePrepStmts.getValue()) {
      synchronized (this.serverSideStatementCheckCache) {
        ...
        boolean canHandle =
            StringUtils.canHandleAsServerPreparedStatementNoCache(
                sql,
                getServerVersion(),
                allowMultiQueries,
    ...
    return StringUtils.canHandleAsServerPreparedStatementNoCache(
        sql,
        getServerVersion(),
        allowMultiQueries,
        this.session.getServerSession().isNoBackslashEscapesSet(),
  ...
  public void changeUser(String userName, String newPassword) throws SQLException {
    synchronized (getConnectionMutex()) {
      ...
      try {
        ...
        // After Bug#16241992 fix the server doesn't return to previous credentials if
        // COM_CHANGE_USER attempt failed.
  ...
  public void throwConnectionClosedException() throws SQLException {
    SQLException ex =
        SQLError.createSQLException(
            Messages.getString("Connection.2"),
  ...
  @Override
  public java.sql.PreparedStatement clientPrepareStatement(
      String sql, int resultSetType, int resultSetConcurrency) throws SQLException {
    return clientPrepareStatement(sql, resultSetType, resultSetConcurrency, true);
  }

  public java.sql.PreparedStatement clientPrepareStatement(
      String sql, int resultSetType, int resultSetConcurrency, boolean processEscapeCodesIfNeeded)
      throws SQLException {

    String nativeSql =
        processEscapeCodesIfNeeded && this.processEscapeCodesForPrepStmts.getValue()
            ? nativeSQL(sql)
            : sql;
    ...
    if (this.cachePrepStmts.getValue()) {
      ...
      if (pStmtInfo == null) {
        pStmt =
            ClientPreparedStatement.getInstance(getMultiHostSafeProxy(), nativeSql, this.database);

        this.queryInfoCache.put(nativeSql, pStmt.getQueryInfo());
      } else {
        pStmt =
            ClientPreparedStatement.getInstance(
                getMultiHostSafeProxy(), nativeSql, this.database, pStmtInfo);
      }
    } else {
      pStmt =
          ClientPreparedStatement.getInstance(getMultiHostSafeProxy(), nativeSql, this.database);
  ...
  public void commit() throws SQLException {
    synchronized (getConnectionMutex()) {
      ...
      try {
        if (this.connectionLifecycleInterceptors != null) {
          IterateBlock<ConnectionLifecycleInterceptor> iter =
              new IterateBlock<ConnectionLifecycleInterceptor>(
        ...
        if (this.session.getServerSession().isAutoCommit()) {
          throw SQLError.createSQLException(
              Messages.getString("Connection.3"), getExceptionInterceptor());
        }
        ...
        this.session.execSQL(
            null, "commit", -1, null, false, this.nullStatementResultSetFactory, null, false);
      } catch (SQLException sqlException) {
        if (MysqlErrorNumbers.SQL_STATE_COMMUNICATION_LINK_FAILURE.equals(
            sqlException.getSQLState())) {
          throw SQLError.createSQLException(
              Messages.getString("Connection.4"),
  ...
  public void createNewIO(boolean isForReconnect) {
    synchronized (getConnectionMutex()) {
      // Synchronization Not needed for *new* connections, but definitely for connections going
      // through fail-over, since we might get the new connection
      // up and running *enough* to start sending cached or still-open server-side prepared
      // statements over to the backend before we get a chance to
      // re-prepare them...
  ...
  private void connectWithRetries(boolean isForReconnect) throws SQLException {
    ...
    for (int attemptCount = 0;
        (attemptCount < this.propertySet.getIntegerProperty(PropertyKey.maxReconnects).getValue())
            && !connectionGood;
        attemptCount++) {
      try {
        this.session.forceClose();
        ...
        this.session.connect(
            this.origHostInfo, this.user, this.password, this.database, getLoginTimeout(), c);
    ...
    if (!connectionGood) {
      ...
      SQLException chainedEx =
          SQLError.createSQLException(
              Messages.getString(
                  "Connection.UnableToConnectWithRetries",
                  new Object[] {
                    this.propertySet.getIntegerProperty(PropertyKey.maxReconnects).getValue()
                  }),
    ...
    if (this.propertySet.getBooleanProperty(PropertyKey.paranoid).getValue()
    ...
    if (isForReconnect) {
      ...
      //
      // We build a list of these outside the map of open statements, because in the process of
      // re-preparing, we might end up having to close a prepared
  ...
  private void connectOneTryOnly(boolean isForReconnect) throws SQLException {
    ...
    try {
      ...
      if ((EEE instanceof PasswordExpiredException
              || EEE instanceof SQLException
                  && ((SQLException) EEE).getErrorCode()
  ...
  private void createPreparedStatementCaches() throws SQLException {
    synchronized (getConnectionMutex()) {
      ...
      String queryInfoCacheFactory =
          this.propertySet.getStringProperty(PropertyKey.queryInfoCacheFactory).getValue();
      ...
      try {
        ...
        CacheAdapterFactory<String, QueryInfo> cacheFactory =
            ((CacheAdapterFactory<String, QueryInfo>) factoryClass.newInstance());

        this.queryInfoCache =
            cacheFactory.getInstance(
                this,
                this.origHostInfo.getDatabaseUrl(),
                cacheSize,
        ...
        SQLException sqlEx =
            SQLError.createSQLException(
                Messages.getString(
                    ...
                    new Object[] {queryInfoCacheFactory, PropertyKey.queryInfoCacheFactory}),
                getExceptionInterceptor());
        sqlEx.initCause(e);
      ...
      if (this.useServerPrepStmts.getValue()) {
        ...
        this.serverSideStatementCache =
            new LRUCache<CompoundCacheKey, ServerPreparedStatement>(cacheSize) {

              private static final long serialVersionUID = 7692318650375988114L;

              @Override
              protected boolean removeEldestEntry(
                  java.util.Map.Entry<CompoundCacheKey, ServerPreparedStatement> eldest) {
  ...
  public String getCatalog() throws SQLException {
    synchronized (getConnectionMutex()) {
      return this.propertySet.<DatabaseTerm>getEnumProperty(PropertyKey.databaseTerm).getValue()
              == DatabaseTerm.SCHEMA
          ? null
          : this.database;
    }
  ...
  private java.sql.DatabaseMetaData getMetaData(boolean checkClosed, boolean checkForInfoSchema)
    ...
    com.mysql.cj.jdbc.DatabaseMetaData dbmeta =
        com.mysql.cj.jdbc.DatabaseMetaData.getInstance(
    ...
    if (getSession() != null && getSession().getProtocol() != null) {
      dbmeta.setMetadataEncoding(
          getSession().getServerSession().getCharsetSettings().getMetadataEncoding());
      dbmeta.setMetadataCollationIndex(
          getSession().getServerSession().getCharsetSettings().getMetadataCollationIndex());
    }
  ...
  public int getTransactionIsolation() throws SQLException {
    ...
    synchronized (getConnectionMutex()) {
      if (!this.useLocalSessionState.getValue()) {
        String s =
            this.session.queryServerVariable(
                versionMeetsMinimum(8, 0, 3)
                        || (versionMeetsMinimum(5, 7, 20) && !versionMeetsMinimum(8, 0, 0))
        ...
        if (s != null) {
          ...
          throw SQLError.createSQLException(
              Messages.getString("Connection.12", new Object[] {s}),
              MysqlErrorNumbers.SQL_STATE_GENERAL_ERROR,
              getExceptionInterceptor());
        }
        throw SQLError.createSQLException(
            Messages.getString("Connection.13"),
            MysqlErrorNumbers.SQL_STATE_GENERAL_ERROR,
            getExceptionInterceptor());
      }
  ...
  /**
   * Sets varying properties that depend on server information. Called once we have connected to the
   * server.
   *
   * @throws SQLException if a database access error occurs
   */
  private void initializePropsFromServer() throws SQLException {
    String connectionInterceptorClasses =
        this.propertySet
    ...
    if (connectionInterceptorClasses != null) {
      try {
        this.connectionLifecycleInterceptors =
            Util.<ConnectionLifecycleInterceptor>loadClasses(
                    this.propertySet
                        .getStringProperty(PropertyKey.connectionLifecycleInterceptors)
                        .getStringValue(),
    ...
    this.autoIncrementIncrement =
        this.session.getServerSession().getServerVariable("auto_increment_increment", 1);
    ...
    ((com.mysql.cj.jdbc.DatabaseMetaData) this.dbmd)
        .setMetadataEncoding(
            this.session.getServerSession().getCharsetSettings().getMetadataEncoding());
    ((com.mysql.cj.jdbc.DatabaseMetaData) this.dbmd)
        .setMetadataCollationIndex(
            this.session.getServerSession().getCharsetSettings().getMetadataCollationIndex());
  ...
  private void handleAutoCommitDefaults() throws SQLException {
    ...
    // Server Bug#66884 (SERVER_STATUS is always initiated with SERVER_STATUS_AUTOCOMMIT=1)
    // invalidates "elideSetAutoCommits" feature.
    // TODO Turn this feature back on as soon as the server bug is fixed. Consider making it version
    // specific.
  ...
  public boolean isReadOnly(boolean useSessionStatus) throws SQLException {
    if (useSessionStatus
        ...
        && versionMeetsMinimum(5, 6, 5)
        && !this.useLocalSessionState.getValue()
  ...
  public boolean isSameResource(JdbcConnection otherConnection) {
    synchronized (getConnectionMutex()) {
      ...
      if (directCompare) {
        if (!nullSafeCompare(otherOrigDatabase, this.origHostInfo.getDatabase())
            || !nullSafeCompare(otherCurrentDb, this.database)) {
          directCompare = false;
        }
      }
      ...
      String otherResourceId =
          ((ConnectionImpl) otherConnection)
              .getPropertySet()
              .getStringProperty(PropertyKey.resourceId)
              .getValue();
  ...
  public String nativeSQL(String sql) throws SQLException {
    ...
    Object escapedSqlResult =
        EscapeProcessor.escapeSQL(
            sql,
            getMultiHostSafeProxy().getSession().getServerSession().getSessionTimeZone(),
            getMultiHostSafeProxy()
                .getSession()
                .getServerSession()
                .getCapabilities()
                .serverSupportsFracSecs(),
            getMultiHostSafeProxy().getSession().getServerSession().isServerTruncatesFracSecs(),
            getExceptionInterceptor());
  ...
  private CallableStatement parseCallableStatement(String sql) throws SQLException {
    ...
    return CallableStatement.getInstance(
        getMultiHostSafeProxy(), parsedSql, this.database, isFunctionCall);
  }
    ...
    if (!this.propertySet.getBooleanProperty(PropertyKey.cacheCallableStmts).getValue()) {
      ...
      synchronized (this.parsedCallableStatementCache) {
        ...
        CallableStatement.CallableStatementParamInfo cachedParamInfo =
            this.parsedCallableStatementCache.get(key);
  ...
  public java.sql.CallableStatement prepareCall(
    ...
    if (this.pedantic.getValue()) {
      if (resultSetHoldability != java.sql.ResultSet.HOLD_CURSORS_OVER_COMMIT) {
        throw SQLError.createSQLException(
            Messages.getString("Connection.17"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
    }
    ...
    CallableStatement cStmt =
        (com.mysql.cj.jdbc.CallableStatement) prepareCall(sql, resultSetType, resultSetConcurrency);
        ...
        if (this.cachePrepStmts.getValue()) {
                  ...
                  pStmt =
                      (ClientPreparedStatement)
                          clientPrepareStatement(
                              nativeSql, resultSetType, resultSetConcurrency, false);
          ...
          try {
            ...
            if (this.emulateUnsupportedPstmts.getValue()) {
              pStmt =
                  (ClientPreparedStatement)
                      clientPrepareStatement(nativeSql, resultSetType, resultSetConcurrency, false);
            } else {
              throw sqlEx;
            }
          }
  ...
  public java.sql.PreparedStatement prepareStatement(String sql, int[] autoGenKeyIndexes)
    ...
    ((ClientPreparedStatement) pStmt)
        .setRetrieveGeneratedKeys((autoGenKeyIndexes != null) && (autoGenKeyIndexes.length > 0));
  ...
  public java.sql.PreparedStatement prepareStatement(String sql, String[] autoGenKeyColNames)
    ...
    ((ClientPreparedStatement) pStmt)
        .setRetrieveGeneratedKeys((autoGenKeyColNames != null) && (autoGenKeyColNames.length > 0));
  ...
  public void realClose(
      boolean calledExplicitly, boolean issueRollback, boolean skipLocalTeardown, Throwable reason)
      throws SQLException {
  ...
  public void recachePreparedStatement(JdbcPreparedStatement pstmt) throws SQLException {
    synchronized (getConnectionMutex()) {
      if (this.cachePrepStmts.getValue() && pstmt.isPoolable()) {
        synchronized (this.serverSideStatementCache) {
          Object oldServerPrepStmt =
              this.serverSideStatementCache.put(
                  new CompoundCacheKey(
                      pstmt.getCurrentDatabase(),
                      ((PreparedQuery) pstmt.getQuery()).getOriginalSql()),
  ...
  public void decachePreparedStatement(JdbcPreparedStatement pstmt) throws SQLException {
    synchronized (getConnectionMutex()) {
      if (this.cachePrepStmts.getValue()) {
        synchronized (this.serverSideStatementCache) {
          this.serverSideStatementCache.remove(
              new CompoundCacheKey(
                  pstmt.getCurrentDatabase(), ((PreparedQuery) pstmt.getQuery()).getOriginalSql()));
        }
      }
    }
  }
  ...
  public void rollback() throws SQLException {
    synchronized (getConnectionMutex()) {
      ...
      try {
        ...
        if (this.session.getServerSession().isAutoCommit()) {
          throw SQLError.createSQLException(
              Messages.getString("Connection.20"),
              MysqlErrorNumbers.SQL_STATE_CONNECTION_NOT_OPEN,
              getExceptionInterceptor());
        }
        try {
          ...
          if (this.ignoreNonTxTables.getInitialValue()
              && (sqlEx.getErrorCode() == MysqlErrorNumbers.ER_WARNING_NOT_COMPLETE_ROLLBACK)) {
            return;
          }
          throw sqlEx;
        }
        ...
        if (MysqlErrorNumbers.SQL_STATE_COMMUNICATION_LINK_FAILURE.equals(
            sqlException.getSQLState())) {
          throw SQLError.createSQLException(
              Messages.getString("Connection.21"),
              MysqlErrorNumbers.SQL_STATE_TRANSACTION_RESOLUTION_UNKNOWN,
              getExceptionInterceptor());
        }
    ...
    synchronized (getConnectionMutex()) {
      ...
      try {
        ...
        try {
          ...
          if (errno == 1181) {
            ...
            if (msg != null) {
              ...
              if (indexOfError153 != -1) {
                throw SQLError.createSQLException(
                    Messages.getString(
                        "Connection.22", new Object[] {savepoint.getSavepointName()}),
                    MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
                    errno,
                    getExceptionInterceptor());
              }
          ...
          // We ignore non-transactional tables if told to do so
          if (this.ignoreNonTxTables.getValue()
              && (sqlEx.getErrorCode() != MysqlErrorNumbers.ER_WARNING_NOT_COMPLETE_ROLLBACK)) {
            throw sqlEx;
          }
          ...
          if (MysqlErrorNumbers.SQL_STATE_COMMUNICATION_LINK_FAILURE.equals(sqlEx.getSQLState())) {
            throw SQLError.createSQLException(
                Messages.getString("Connection.23"),
                MysqlErrorNumbers.SQL_STATE_TRANSACTION_RESOLUTION_UNKNOWN,
                getExceptionInterceptor());
          }
  ...
  private void rollbackNoChecks() throws SQLException {
    synchronized (getConnectionMutex()) {
      ...
      this.session.execSQL(
          null, "rollback", -1, null, false, this.nullStatementResultSetFactory, null, false);
    }
  }
  ...
  public java.sql.PreparedStatement serverPrepareStatement(String sql) throws SQLException {
    ...
    return ServerPreparedStatement.getInstance(
        getMultiHostSafeProxy(),
        nativeSql,
        this.getDatabase(),
  ...
  @Override
  public java.sql.PreparedStatement serverPrepareStatement(
      String sql, int resultSetType, int resultSetConcurrency) throws SQLException {
    String nativeSql = this.processEscapeCodesForPrepStmts.getValue() ? nativeSQL(sql) : sql;
  ...
  public void setAutoCommit(final boolean autoCommitFlag) throws SQLException {
    synchronized (getConnectionMutex()) {
      ...
      try {
        ...
        // this internal value must be set first as failover depends on it being set to true to fail
        // over (which is done by most app servers and
        // connection pools at the end of a transaction), and the driver issues an implicit set
        // based on this value when it (re)-connects to a
        ...
        // Reset to current autocommit value in case of an exception different than a communication
        // exception occurs.
  ...
  public void setCatalog(final String catalog) throws SQLException {
    if (this.propertySet.<DatabaseTerm>getEnumProperty(PropertyKey.databaseTerm).getValue()
  ...
  public void setDatabase(final String db) throws SQLException {
    synchronized (getConnectionMutex()) {
      ...
      this.session.execSQL(
          null, query.toString(), -1, null, false, this.nullStatementResultSetFactory, null, false);
    ...
    synchronized (getConnectionMutex()) {
      ...
      if (this.readOnlyPropagatesToServer.getValue() && versionMeetsMinimum(5, 6, 5)) {
        if (!this.useLocalSessionState.getValue() || (readOnlyFlag != this.readOnly)) {
          this.session.execSQL(
              null,
              "set session transaction " + (readOnlyFlag ? "read only" : "read write"),
              -1,
              null,
              false,
              this.nullStatementResultSetFactory,
      ...
      if (shouldSendSet) {
        switch (level) {
          case java.sql.Connection.TRANSACTION_NONE:
            throw SQLError.createSQLException(
                Messages.getString("Connection.24"), getExceptionInterceptor());
          ...
          default:
            throw SQLError.createSQLException(
                Messages.getString("Connection.25", new Object[] {level}),
                MysqlErrorNumbers.SQL_STATE_DRIVER_NOT_CAPABLE,
                getExceptionInterceptor());
        }

        this.session.execSQL(
            null, sql, -1, null, false, this.nullStatementResultSetFactory, null, false);
  ...
  private void setupServerForTruncationChecks() throws SQLException {
    synchronized (getConnectionMutex()) {
      RuntimeProperty<Boolean> jdbcCompliantTruncation =
          this.propertySet.getProperty(PropertyKey.jdbcCompliantTruncation);
  ...
  public void initializeResultsMetadataFromCache(
      String sql, CachedResultSetMetaData cachedMetaData, ResultSetInternalMethods resultSet)
      throws SQLException {
  ...
  public void transactionBegun() {
    synchronized (getConnectionMutex()) {
      if (this.connectionLifecycleInterceptors != null) {
        this.connectionLifecycleInterceptors.stream()
            .forEach(ConnectionLifecycleInterceptor::transactionBegun);
      }
    }
  }
  ...
  public void transactionCompleted() {
    synchronized (getConnectionMutex()) {
      if (this.connectionLifecycleInterceptors != null) {
        this.connectionLifecycleInterceptors.stream()
            .forEach(ConnectionLifecycleInterceptor::transactionCompleted);
      }
    }
  }
      ...
      if (this.session.getSessionMaxRows() != max) {
        ...
        this.session.execSQL(
            ...
            "SET SQL_SELECT_LIMIT="
                + (this.session.getSessionMaxRows() == -1
                    ? "DEFAULT"
                    : this.session.getSessionMaxRows()),
            -1,
            null,
            false,
            this.nullStatementResultSetFactory,
  ...
  public void abort(Executor executor) throws SQLException {
    ...
    if (executor == null) {
      throw SQLError.createSQLException(
          Messages.getString("Connection.26"),
          MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
          getExceptionInterceptor());
    }
    ...
    synchronized (getConnectionMutex()) {
      if (this.infoProvider == null) {
        String clientInfoProvider =
            this.propertySet.getStringProperty(PropertyKey.clientInfoProvider).getStringValue();
        try {
          try {
            this.infoProvider =
                (ClientInfoProvider)
          ...
          throw SQLError.createSQLException(
              Messages.getString(
                  "Connection.ClientInfoNotImplemented", new Object[] {clientInfoProvider}),
              MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
              getExceptionInterceptor());
        }
