...
public class MysqlSQLXML implements SQLXML {
  ...
  public MysqlSQLXML(
      ResultSetInternalMethods owner, int index, ExceptionInterceptor exceptionInterceptor) {
  ...
  private synchronized void checkClosed() throws SQLException {
    if (this.isClosed) {
      throw SQLError.createSQLException(
          Messages.getString("MysqlSQLXML.0"), this.exceptionInterceptor);
    }
  }
  ...
  public <T extends Source> T getSource(Class<T> clazz) throws SQLException {
    ...
    if (clazz == null || clazz.equals(SAXSource.class)) {
      ...
      try {
        ...
        return (T)
            new SAXSource(
                ...
                this.fromResultSet
                    ? new InputSource(
                        this.owningResultSet.getCharacterStream(this.columnIndexOfXml))
      ...
      try {
        ...
        setFeature(
            builderFactory, "http://xml.org/sax/features/external-parameter-entities", false);
        setFeature(
            builderFactory,
      ...
      return (T)
          new StreamSource(
              this.fromResultSet
                  ? this.owningResultSet.getCharacterStream(this.columnIndexOfXml)
                  : new StringReader(this.stringRep));

    } else if (clazz.equals(StAXSource.class)) {
      try {
        return (T)
            new StAXSource(
                this.inputFactory.createXMLStreamReader(
                    this.fromResultSet
                        ? this.owningResultSet.getCharacterStream(this.columnIndexOfXml)
                        : new StringReader(this.stringRep)));
      ...
      throw SQLError.createSQLException(
          Messages.getString("MysqlSQLXML.2", new Object[] {clazz.toString()}),
          MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
          this.exceptionInterceptor);
    }
  }
  ...
  public synchronized <T extends Result> T setResult(Class<T> clazz) throws SQLException {
    ...
    if (clazz == null || clazz.equals(SAXResult.class)) {
      ...
      try {
        ...
        return (T)
            new StAXResult(this.outputFactory.createXMLEventWriter(setCharacterStreamInternal()));
      } catch (XMLStreamException ex) {
        SQLException sqlEx =
            SQLError.createSQLException(
                ex.getMessage(),
      ...
      throw SQLError.createSQLException(
          Messages.getString("MysqlSQLXML.3", new Object[] {clazz.toString()}),
          MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
          this.exceptionInterceptor);
    }
  }
  ...
  private Reader binaryInputStreamStreamToReader(ByteArrayOutputStream out) {
    ...
    try {
      // There's got to be an easier way to do this, but I don't feel like coding up Appendix F of
      // the XML Spec myself, when there's a reusable way to do
      // it, and we can warn folks away from BINARY xml streams that have to be parsed to determine
