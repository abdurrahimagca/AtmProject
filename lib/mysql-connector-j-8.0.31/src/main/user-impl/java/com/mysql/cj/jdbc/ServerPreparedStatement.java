...
public class ServerPreparedStatement extends ClientPreparedStatement {
   ...
   */
  protected static ServerPreparedStatement getInstance(
      JdbcConnection conn, String sql, String db, int resultSetType, int resultSetConcurrency)
      throws SQLException {
   ...
   * @throws SQLException If an error occurs
   */
  protected ServerPreparedStatement(
      JdbcConnection conn, String sql, String db, int resultSetType, int resultSetConcurrency)
      throws SQLException {
    super(conn, db);
    ...
    prepQuery.setOriginalSql(
        statementComment == null ? sql : "/* " + statementComment + " */ " + sql);
    prepQuery.setQueryInfo(
        new QueryInfo(prepQuery.getOriginalSql(), this.session, this.charEncoding));
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (locallyScopedConn.isReadOnly()) {
        throw SQLError.createSQLException(
            Messages.getString("ServerPreparedStatement.2")
                + Messages.getString("ServerPreparedStatement.3"),
      ...
      BindValue[] oldBindValues =
          ((ServerPreparedQuery) this.query).getQueryBindings().getBindValues();
            ...
            for (commandIndex = 0; commandIndex < nbrCommands; commandIndex++) {
              ...
              try {
                if (arg instanceof String) {
                  ...
                  // We need to check types each time, as the user might have bound different types
                  // in each addBatch()

                  if (previousBindValuesForBatch != null) {
                    for (int j = 0; j < parameterBindings.length; j++) {
                      if (parameterBindings[j].getMysqlType()
                          != previousBindValuesForBatch[j].getMysqlType()) {
                        ((ServerPreparedQuery) this.query)
                            .getQueryBindings()
  ...
  private static SQLException appendMessageToException(
      SQLException sqlEx, String messageToAppend, ExceptionInterceptor interceptor) {
    String sqlState = sqlEx.getSQLState();
    int vendorErrorCode = sqlEx.getErrorCode();

    SQLException sqlExceptionWithNewMessage =
        SQLError.createSQLException(
            sqlEx.getMessage() + messageToAppend, sqlState, vendorErrorCode, interceptor);
  ...
  @Override
  protected <M extends Message> com.mysql.cj.jdbc.result.ResultSetInternalMethods executeInternal(
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      try {
        ...
        if (this.session
            .getPropertySet()
            .getBooleanProperty(PropertyKey.enablePacketDebug)
            .getValue()) {
  ...
  public void realClose(boolean calledExplicitly, boolean closeOpenResults) throws SQLException {
    ...
    synchronized (locallyScopedConn.getConnectionMutex()) {
      if (this.connection != null) {
        ...
        //
        // This will leak server resources, but if we don't do this, we'll deadlock (potentially,
        // because there's no guarantee when, what order, and
        // what concurrency finalizers will be called with). Well-behaved programs won't rely on
        // finalizers to clean up their statements.
        //
          ...
          synchronized (locallyScopedConn.getConnectionMutex()) {
            try {
              ((NativeSession) locallyScopedConn.getSession())
                  .getProtocol()
                  .sendCommand(
                      this.commandBuilder.buildComStmtClose(
                          null, ((ServerPreparedQuery) this.query).getServerStatementId()),
                      true,
                      0);
   ...
   *     PreparedStatement
   */
  protected ResultSetInternalMethods serverExecute(
      int maxRowsToRetrieve, boolean createStreamingResultSet, ColumnDefinition metadata)
      throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      this.results =
          ((ServerPreparedQuery) this.query)
              .serverExecute(
                  maxRowsToRetrieve, createStreamingResultSet, metadata, this.resultSetFactory);
      return this.results;
    }
  }
  ...
  protected void serverPrepare(String sql) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      try {
        ...
        t =
            SQLError.createCommunicationsException(
                this.connection,
        ...
        if (this.dumpQueriesOnException.getValue()) {
          StringBuilder messageBuf =
              new StringBuilder(((PreparedQuery) this.query).getOriginalSql().length() + 32);
        ...
        // Leave the I/O channel in a known state...there might be packets out there that we're not
        // interested in
        try {
  ...
  protected void checkBounds(int parameterIndex, int parameterIndexOffset) throws SQLException {
    ...
    if (paramCount == 0) {
      throw ExceptionFactory.createException(
          ...
          Messages.getString("ServerPreparedStatement.8"),
    ...
    if ((parameterIndex < 0) || (parameterIndex > paramCount)) {
      throw ExceptionFactory.createException(
          WrongArgumentException.class,
          Messages.getString("ServerPreparedStatement.9")
              + (parameterIndex + 1)
              + Messages.getString("ServerPreparedStatement.10")
              + paramCount,
          this.session.getExceptionInterceptor());
    }
  }
  ...
  protected ClientPreparedStatement prepareBatchedInsertSQL(
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      try {
        ClientPreparedStatement pstmt =
            ((Wrapper)
                    localConn.prepareStatement(
                        ((PreparedQuery) this.query).getQueryInfo().getSqlForBatch(numBatches),
                        this.resultSetConcurrency,
                        this.query.getResultType().getIntValue()))
        ...
        SQLException sqlEx =
            SQLError.createSQLException(
                Messages.getString("ServerPreparedStatement.27"),
                MysqlErrorNumbers.SQL_STATE_GENERAL_ERROR,
                this.exceptionInterceptor);
        sqlEx.initCause(e);
