 ...
 * produced by it.
 *
 * <p>Only one ResultSet per Statement can be open at any point in time. Therefore, if the reading
...
public class StatementImpl implements JdbcStatement {
  ...
  protected int maxFieldSize =
      (Integer)
          PropertyDefinitions.getPropertyDefinition(PropertyKey.maxAllowedPacket).getDefaultValue();
  ...
  public StatementImpl(JdbcConnection c, String db) throws SQLException {
    if ((c == null) || c.isClosed()) {
      throw SQLError.createSQLException(
          Messages.getString("Statement.0"), MysqlErrorNumbers.SQL_STATE_CONNECTION_NOT_OPEN, null);
    }
    ...
    this.commandBuilder =
        new NativeMessageBuilder(this.session.getServerSession().supportsQueryAttributes());
    ...
    this.continueBatchOnError =
        pset.getBooleanProperty(PropertyKey.continueBatchOnError).getValue();
    ...
    this.dontCheckOnDuplicateKeyUpdateInSQL =
        pset.getBooleanProperty(PropertyKey.dontCheckOnDuplicateKeyUpdateInSQL).getValue();
    this.doEscapeProcessing =
        pset.getBooleanProperty(PropertyKey.enableEscapeProcessing).getValue();
    ...
    this.holdResultsOpenOverClose =
        pset.getBooleanProperty(PropertyKey.holdResultsOpenOverStatementClose).getValue();
        ...
        newSession
            ...
            .sendCommand(
                new NativeMessageBuilder(newSession.getServerSession().supportsQueryAttributes())
                    .buildComQuery(
                        newSession.getSharedSendPacket(),
  ...
  protected void checkNullOrEmptyQuery(String sql) throws SQLException {
    if (sql == null) {
      throw SQLError.createSQLException(
          Messages.getString("Statement.59"),
    ...
    if (sql.length() == 0) {
      throw SQLError.createSQLException(
          Messages.getString("Statement.61"),
          MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
          getExceptionInterceptor());
    }
  }
    ...
    try {
      synchronized (checkClosed().getConnectionMutex()) {
        if (isCloseOnCompletion()
            ...
            && (this.generatedKeysResults == null
                || !this.generatedKeysResults.hasRows()
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      java.sql.PreparedStatement pStmt =
          this.connection.prepareStatement(
              sql, this.query.getResultType().getIntValue(), this.resultSetConcurrency);
  ...
  /**
   * We only stream result sets when they are forward-only, read-only, and the fetch size has been
  ...
  public void disableStreamingResults() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if (this.query.getResultFetchSize() == Integer.MIN_VALUE
  ...
  /**
   * Adjust net_write_timeout to a higher value if we're streaming result sets. More often than not,
   * someone runs into an issue where they blow net_write_timeout when using this feature, and if
   * they're willing to hold a result set open for 30 seconds or more, one more round-trip isn't
   * going to hurt.
   *
   * <p>This is reset by RowDataDynamic.close().
   *
   * @param con created this statement
   * @throws SQLException if a database error occurs
   */
  protected void setupStreamingTimeout(JdbcConnection con) throws SQLException {
    int netTimeoutForStreamingResults =
        this.session
            .getPropertySet()
  ...
  private boolean executeInternal(String sql, boolean returnGeneratedKeys) throws SQLException {
    ...
    synchronized (locallyScopedConn.getConnectionMutex()) {
      ...
      this.lastQueryIsOnDupKeyUpdate =
          returnGeneratedKeys
              && QueryInfo.firstCharOfStatementUc(
                      sql, this.session.getServerSession().isNoBackslashEscapesSet())
                  == 'I'
      ...
      if (!QueryInfo.isReadOnlySafeQuery(
        ...
        throw SQLError.createSQLException(
            Messages.getString("Statement.27") + Messages.getString("Statement.28"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
        ...
        if (this.doEscapeProcessing) {
          Object escapedSqlResult =
              EscapeProcessor.escapeSQL(
                  ...
                  getExceptionInterceptor());
          sql =
              escapedSqlResult instanceof String
                  ? (String) escapedSqlResult
          ...
          if (rs.hasRows()) {
            if (cachedMetaData != null) {
              locallyScopedConn.initializeResultsMetadataFromCache(
                  sql, cachedMetaData, this.results);
            } else if (this.session
                .getPropertySet()
                .getBooleanProperty(PropertyKey.cacheResultSetMetadata)
                .getValue()) {
              locallyScopedConn.initializeResultsMetadataFromCache(
                  sql, null /* will be created */, this.results);
            }
          }
        }
  ...
  protected long[] executeBatchInternal() throws SQLException {
    ...
    synchronized (locallyScopedConn.getConnectionMutex()) {
      if (locallyScopedConn.isReadOnly()) {
        throw SQLError.createSQLException(
            Messages.getString("Statement.34") + Messages.getString("Statement.35"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
        ...
        try {
          ...
          if (batchedArgs != null) {
            ...
            if (multiQueriesEnabled
                || this.rewriteBatchedStatements.getValue() && nbrCommands > 4) {
              return executeBatchUsingMultiQueries(
                  multiQueriesEnabled, nbrCommands, individualStatementTimeout);
            }
            ...
            for (commandIndex = 0; commandIndex < nbrCommands; commandIndex++) {
              try {
                ...
                if (timeoutTask != null) {
                  // we need to check the cancel state on each iteration to generate timeout
                  // exception if needed
                  checkCancelTimeout();
                }
                ...
                getBatchedGeneratedKeys(
                    this.results.getFirstCharOfQuery() == 'I' && containsOnDuplicateKeyInString(sql)
                        ? 1
                        : 0);
  ...
  /**
   * Rewrites batch into a single query to send to the server. This method will constrain each batch
   * to be shorter than max_allowed_packet on the server.
   *
   ...
   *     PreparedStatement
   */
  private long[] executeBatchUsingMultiQueries(
      boolean multiQueriesEnabled, int nbrCommands, int individualStatementTimeout)
      throws SQLException {
    ...
    synchronized (locallyScopedConn.getConnectionMutex()) {
      ...
      try {
        ...
        getQueryAttributesBindings()
            .runThroughAll(a -> jdbcBatchedStmt.setAttribute(a.getName(), a.getValue()));
        ...
        int numberOfBytesPerChar =
            StringUtils.startsWithIgnoreCase(connectionEncoding, "utf")
                ? 3
                : (this.session
                        .getServerSession()
                        .getCharsetSettings()
        ...
        for (commandIndex = 0; commandIndex < nbrCommands; commandIndex++) {
          ...
          if (((((queryBuf.length() + nextQuery.length()) * numberOfBytesPerChar)
                          + 1 /* for semicolon */
                          + NativeConstants.HEADER_LENGTH)
                      * escapeAdjust)
                  + 32
              > this.maxAllowedPacket.getValue()) {
            try {
              batchStmt.execute(queryBuf.toString(), java.sql.Statement.RETURN_GENERATED_KEYS);
            } catch (SQLException ex) {
              sqlEx =
                  handleExceptionForBatch(commandIndex, argumentSetsInBatchSoFar, updateCounts, ex);
            }
  ...
  protected int processMultiCountsAndKeys(
      StatementImpl batchedStatement, int updateCountCounter, long[] updateCounts)
      throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
  ...
  protected SQLException handleExceptionForBatch(
      int endOfBatchIndex, int numValuesPerBatch, long[] updateCounts, SQLException ex)
  ...
  public void executeSimpleNonQuery(JdbcConnection c, String nonQuery) throws SQLException {
    synchronized (c.getConnectionMutex()) {
      ((NativeSession) c.getSession())
          .<ResultSetImpl>execSQL(
              this, nonQuery, -1, null, false, getResultSetFactory(), null, false)
          .close();
    }
  }
  ...
  protected long executeUpdateInternal(String sql, boolean isBatch, boolean returnGeneratedKeys)
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (!isNonResultSetProducingQuery(sql)) {
        throw SQLError.createSQLException(
            Messages.getString("Statement.46"), "01S03", getExceptionInterceptor());
      }

      this.retrieveGeneratedKeys = returnGeneratedKeys;

      this.lastQueryIsOnDupKeyUpdate =
          returnGeneratedKeys && firstStatementChar == 'I' && containsOnDuplicateKeyInString(sql);
      ...
      if (locallyScopedConn.isReadOnly(false)) {
        throw SQLError.createSQLException(
            Messages.getString("Statement.42") + Messages.getString("Statement.43"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
      ...
      // The checking and changing of databases must happen in sequence, so synchronize on the same
      // mutex that _conn is using
      ...
      try {
        ...
        rs =
            ((NativeSession) locallyScopedConn.getSession())
                .execSQL(this, sql, -1, null, false, getResultSetFactory(), null, isBatch);
  ...
  public java.sql.ResultSet getGeneratedKeys() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if (!this.retrieveGeneratedKeys) {
        throw SQLError.createSQLException(
            Messages.getString("Statement.GeneratedKeysNotRequested"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
      ...
      fields[0] =
          new Field("", "GENERATED_KEY", collationIndex, encoding, MysqlType.BIGINT_UNSIGNED, 20);

      this.generatedKeysResults =
          this.resultSetFactory.createFromResultsetRows(
              ResultSet.CONCUR_READ_ONLY,
              ResultSet.TYPE_SCROLL_INSENSITIVE,
              new ResultsetRowsStatic(
                  this.batchedGeneratedKeys, new DefaultColumnDefinition(fields)));
  ...
  protected ResultSetInternalMethods getGeneratedKeysInternal(long numKeys) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (this.results != null) {
        ...
        if ((beginAt != 0 /* BIGINT UNSIGNED can wrap the protocol representation */)
            && (numKeys > 0)) {
   ...
   *
   * <p>This method returns longs as MySQL server returns 64-bit values for update counts
   *
  ...
  public boolean getMoreResults(int current) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (streamingMode) {
        if (this.results.hasRows()) {
          while (this.results.next()) {
            // need to drain remaining rows to get to server status which tells us whether more
            // results actually exist or not
          }
        }
      }
      ...
      switch (current) {
        ...
        default:
          throw SQLError.createSQLException(
              Messages.getString("Statement.19"),
              MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
              getExceptionInterceptor());
      }
  ...
  public java.sql.ResultSet getResultSet() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      return ((this.results != null) && this.results.hasRows())
          ? (java.sql.ResultSet) this.results
          : null;
    }
  }
  ...
  public java.sql.SQLWarning getWarnings() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      SQLWarning pendingWarningsFromServer =
          this.session.getProtocol().convertShowWarningsToSQLWarnings(false);
  ...
  protected void realClose(boolean calledExplicitly, boolean closeOpenResults) throws SQLException {
    ...
    // do it ASAP to reduce the chance of calling this method concurrently from
  ...
  public void setFetchDirection(int direction) throws SQLException {
    switch (direction) {
      ...
      default:
        throw SQLError.createSQLException(
            Messages.getString("Statement.5"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
    }
  }
  ...
  public void setFetchSize(int rows) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if (((rows < 0) && (rows != Integer.MIN_VALUE))
          || ((this.maxRows > 0) && (rows > this.getMaxRows()))) {
        throw SQLError.createSQLException(
            Messages.getString("Statement.7"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
  ...
  public void setMaxFieldSize(int max) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if (max < 0) {
        throw SQLError.createSQLException(
            Messages.getString("Statement.11"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
      ...
      if (max > maxBuf) {
        throw SQLError.createSQLException(
            Messages.getString("Statement.13", new Object[] {Long.valueOf(maxBuf)}),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
  ...
  public void setQueryTimeout(int seconds) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if (seconds < 0) {
        throw SQLError.createSQLException(
            Messages.getString("Statement.21"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
  ...
  void setResultSetConcurrency(int concurrencyFlag) throws SQLException {
    try {
      ...
      // FIXME: Can't break interface atm, we'll get the exception later when you try and do
      // something useful with a closed statement...
    }
  }
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      if (this.retrieveGeneratedKeys) {
        ...
        try {
          ...
          while (rs.next()) {
            this.batchedGeneratedKeys.add(
                new ByteArrayRow(new byte[][] {rs.getBytes(1)}, getExceptionInterceptor()));
          }
        } finally {
          if (rs != null) {
            rs.close();
  ...
  protected boolean containsOnDuplicateKeyInString(String sql) {
    return (!this.dontCheckOnDuplicateKeyUpdateInSQL || this.rewriteBatchedStatements.getValue())
        && QueryInfo.containsOnDuplicateKeyUpdateClause(
            sql, this.session.getServerSession().isNoBackslashEscapesSet());
  }
  ...
  public long executeLargeUpdate(String sql, int autoGeneratedKeys) throws SQLException {
    return executeUpdateInternal(
        sql, false, autoGeneratedKeys == java.sql.Statement.RETURN_GENERATED_KEYS);
  }
  ...
  public void setLargeMaxRows(long max) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if ((max > MAX_ROWS) || (max < 0)) {
        throw SQLError.createSQLException(
            Messages.getString("Statement.15") + max + " > " + MAX_ROWS + ".",
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            getExceptionInterceptor());
      }
  ...
  public long getServerStatementId() {
    throw ExceptionFactory.createException(
        CJOperationNotSupportedException.class, Messages.getString("Statement.65"));
  }
