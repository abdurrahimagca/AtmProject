 ...
 * and/or user defined settings, namely when one of the conditions set in 'secondsBeforeRetrySource'
 * or 'queriesBeforeRetrySource' is met.
 */
public class FailoverConnectionProxy extends MultiHostConnectionProxy {
  ...
  /**
   * Proxy class to intercept and deal with errors that may occur in any object bound to the current
   * connection. Additionally intercepts query executions and triggers an execution count on the
   * outer class.
   */
  class FailoverJdbcInterfaceProxy extends JdbcInterfaceProxy {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      ...
      if (FailoverConnectionProxy.this.explicitlyAutoCommit
          && isExecute
  ...
  private FailoverConnectionProxy(ConnectionUrl connectionUrl) throws SQLException {
    ...
    this.secondsBeforeRetryPrimaryHost =
        connProps.getIntegerProperty(PropertyKey.secondsBeforeRetrySource).getValue();
    this.queriesBeforeRetryPrimaryHost =
        connProps.getIntegerProperty(PropertyKey.queriesBeforeRetrySource).getValue();
    ...
    this.enableFallBackToPrimaryHost =
        this.secondsBeforeRetryPrimaryHost > 0 || this.queriesBeforeRetryPrimaryHost > 0;
      ...
      if (this.currentConnection != null) {
        StringBuilder msg =
            new StringBuilder("Connection to ")
                .append(isPrimaryHostIndex(hostIndex) ? "primary" : "secondary")
                .append(" host '")
                .append(this.hostsList.get(hostIndex))
                .append("' failed");
        try {
  ...
  private synchronized void failOver(int failedHostIdx) throws SQLException {
    ...
    boolean firstConnOrPassedByPrimaryHost =
        prevHostIndex == NO_CONNECTION_INDEX || isPrimaryHostIndex(prevHostIndex);
    do {
      try {
        firstConnOrPassedByPrimaryHost =
            firstConnOrPassedByPrimaryHost || isPrimaryHostIndex(nextHostIndex);
        ...
        if (shouldExceptionTriggerConnectionSwitch(e)) {
          ...
          if (newNextHostIndex == firstHostIndexTried
              && newNextHostIndex
                  == (newNextHostIndex = nextHost(nextHostIndex, true))) { // Full turn
            attempts++;
   ...
   * but it may skip the index for the primary host. General rules to include the primary host are:
   * - not currently connected to any host. - primary host is vouched (usually because connection to
   * all secondary hosts has failed). - conditions to fall back to primary host are met (or they are
   * disabled).
   *
  ...
  private int nextHost(int currHostIdx, boolean vouchForPrimaryHost) {
    ...
    if (isPrimaryHostIndex(nextHostIdx)
        && isConnected()
        && !vouchForPrimaryHost
        && this.enableFallBackToPrimaryHost
        && !readyToFallBackToPrimaryHost()) {
  ...
  /**
   * Checks if at least one of the required conditions to fall back to primary host is met, which is
   * determined by the properties 'queriesBeforeRetrySource' and 'secondsBeforeRetrySource'.
   *
   * @return true if ready
   */
  synchronized boolean readyToFallBackToPrimaryHost() {
    return this.enableFallBackToPrimaryHost
  ...
  private synchronized boolean secondsBeforeRetryPrimaryHostIsMet() {
    return this.secondsBeforeRetryPrimaryHost > 0
        && Util.secondsSinceMillis(this.primaryHostFailTimeMillis)
            >= this.secondsBeforeRetryPrimaryHost;
  }
  ...
  private synchronized boolean queriesBeforeRetryPrimaryHostIsMet() {
    return this.queriesBeforeRetryPrimaryHost > 0
        && this.queriesIssuedSinceFailover >= this.queriesBeforeRetryPrimaryHost;
  }
  ...
  public Object invokeMore(Object proxy, Method method, Object[] args) throws Throwable {
    ...
    if ((this.explicitlyAutoCommit
            || METHOD_COMMIT.equals(methodName)
            || METHOD_ROLLBACK.equals(methodName))
        && readyToFallBackToPrimaryHost()) {
      // Fall back to primary host at transaction boundary
      fallBackToPrimaryIfAvailable();
    }
