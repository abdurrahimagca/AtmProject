...
public class LoadBalancedAutoCommitInterceptor implements QueryInterceptor {
  ...
  public <T extends Resultset> T postProcess(
    ...
    try {
      // Don't count SETs, SHOWs neither USEs. Those are mostly used internally and must not trigger
      // a connection switch.
      if (!this.countStatements
      ...
      if (this.proxy == null && this.conn.isProxySet()) {
        ...
        while (connParentProxy != null
            && !(connParentProxy instanceof LoadBalancedMySQLConnection)) {
      ...
      if (this.matchingAfterStatementRegex == null
          || sql.get().matches(this.matchingAfterStatementRegex)) {
      ...
      if (this.matchingAfterStatementCount >= this.matchingAfterStatementThreshold) {
        ...
        try {
          ...
          // eat this exception, the auto-commit statement completed, but we could not rebalance for
          // some reason.  User may get exception when using
          // connection next.
        }
      }
