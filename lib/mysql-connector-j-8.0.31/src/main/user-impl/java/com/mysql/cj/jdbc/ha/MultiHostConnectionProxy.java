...
public abstract class MultiHostConnectionProxy implements InvocationHandler {
  ...
  // Keep track of the last exception processed in 'dealWithInvocationException()' in order to avoid
  // creating connections repeatedly from each time the same
  ...
  int initializeHostsSpecs(ConnectionUrl connUrl, List<HostInfo> hosts) {
    ...
    this.autoReconnect =
        "true".equalsIgnoreCase(props.getProperty(PropertyKey.autoReconnect.getKeyName()))
            || "true"
                .equalsIgnoreCase(
                    props.getProperty(PropertyKey.autoReconnectForPools.getKeyName()));
  ...
  /**
   * Get this connection's proxy. A multi-host connection may not be at top level in the multi-host
   * connections chain. In such case the first connection in the chain is available as a proxy.
   *
  ...
  /**
   * If the given return type is or implements a JDBC interface, proxies the given object so that we
   * can catch SQL errors and fire a connection switch.
   *
  ...
  /**
   * Picks the "best" connection to use from now on. Each subclass needs to implement its connection
   * switch strategy on it.
   *
   * @throws SQLException if an error occurs
   */
   ...
   * methods and generic methods. Subclasses have to override this to complete the method invocation
   * process, deal with exceptions and decide when to switch connection. To avoid unnecessary
   * additional exception handling overriders should consult #canDealWith(Method) before chaining
   * here.
   *
  ...
  protected boolean allowedOnClosedConnection(Method method) {
    ...
    return methodName.equals(METHOD_GET_AUTO_COMMIT)
        ...
        || methodName.equals(METHOD_GET_DATABASE)
