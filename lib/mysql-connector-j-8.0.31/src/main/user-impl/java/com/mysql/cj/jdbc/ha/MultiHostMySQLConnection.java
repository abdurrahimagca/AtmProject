 ...
 *
 * <p>While this class implements MySQLConnection directly, MultiHostConnectionProxy does the same
 * but via a dynamic proxy.
 *
 * <p>Most of the methods in this class refer directly to the active connection from its
 ...
 *
 * <p>Referring to the higher level proxy connection is needed when some operation needs to be
 * extended to all open sub-connections existing in this multi-host structure as opposed to just
 * refer to the active current connection, such as with close() which is most likely required to
 * close all sub-connections as well.
 */
public class MultiHostMySQLConnection implements JdbcConnection {

  /**
   * thisAsProxy holds the proxy (MultiHostConnectionProxy or one of its subclasses) this connection
   * is associated with. It is used as a gateway to the current active sub-connection managed by
   * this multi-host structure or as a target to where some of the methods implemented here in this
   * class refer to.
   */
  ...
  @Override
  public PreparedStatement clientPrepareStatement(
  ...
  public PreparedStatement clientPrepareStatement(
    ...
    return getActiveMySQLConnection()
        .clientPrepareStatement(sql, resultSetType, resultSetConcurrency);
  }
  ...
  public void realClose(
    ...
    getActiveMySQLConnection()
        .realClose(calledExplicitly, issueRollback, skipLocalTeardown, reason);
  }
  ...
  @Override
  public PreparedStatement serverPrepareStatement(
      String sql, int resultSetType, int resultSetConcurrency, int resultSetHoldability)
      throws SQLException {
    return getActiveMySQLConnection()
  ...
  public PreparedStatement serverPrepareStatement(
      String sql, int resultSetType, int resultSetConcurrency) throws SQLException {
    return getActiveMySQLConnection()
        .serverPrepareStatement(sql, resultSetType, resultSetConcurrency);
  }
