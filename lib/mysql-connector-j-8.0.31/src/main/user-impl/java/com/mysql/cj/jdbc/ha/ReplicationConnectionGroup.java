...
/**
 * Group of connection objects which can be configured as a group. This is used for
 * promotion/demotion of replicas and sources in a replication configuration, and for exposing
 * metrics around replication-aware connections.
 */
public class ReplicationConnectionGroup {
  ...
  public long registerReplicationConnection(
      ReplicationConnection conn, List<String> localSourceList, List<String> localReplicaList) {
   ...
   *
   * <p>We can safely assume that if this host was added to the replicas list, then it must be added
   * to each one of the replication connections from this group as well. Unnecessary calls to {@link
   * ReplicationConnection#addReplicaHost(String)} could result in undesirable locking issues,
   * assuming that this method is synchronized by nature.
   *
   * <p>This is a no-op if the group already has this host in a replica role.
   *
   ...
   *
   * <p>We can safely assume that if this host was removed from the replicas list, then it must be
   * removed from each one of the replication connections from this group as well. Unnecessary calls
   * to {@link ReplicationConnection#removeReplica(String, boolean)} could result in undesirable
   * locking issues, assuming that this method is synchronized by nature.
   *
   * <p>This is a no-op if the group doesn't have this host in a replica role.
   *
   * @param hostPortPair "host:port"
   ...
   *
   * <p>We can safely assume that if this host was removed from the replicas list or added to the
   * sources list, then the same host promotion must happen in each one of the replication
   * connections from this group as well. Unnecessary calls to {@link
   * ReplicationConnection#promoteReplicaToSource(String)} could result in undesirable locking
   * issues, assuming that this method is synchronized by nature.
   *
   * <p>This is a no-op if the group already has this host in a source role and not in replica role.
   *
   * @param hostPortPair "host:port"
   * @throws SQLException if an error occurs
   */
  public void promoteReplicaToSource(String hostPortPair) throws SQLException {
    // remove host from replicas AND add host to sources, note that both operands need to be
    // evaluated.
   ...
   * Removes a host from the sources hosts list.
   *
   * <p>We can safely assume that if this host was removed from the sources list, then it must be
   * removed from each one of the replication connections from this group as well. Unnecessary calls
   * to {@link ReplicationConnection#removeSourceHost(String, boolean)} could result in undesirable
   * locking issues, assuming that this method is synchronized by nature.
   *
   * <p>This is a no-op if the group doesn't have this host in a source role.
   *
   * @param hostPortPair "host:port"
   * @param closeGently remove host when it's not in use
   * @throws SQLException if an error occurs
  ...
  public String toString() {
    return "ReplicationConnectionGroup[groupName="
        + this.groupName
        + ",sourceHostList="
        + this.sourceHostList
        + ",replicaHostList="
        + this.replicaHostList
        + "]";
  }
}
