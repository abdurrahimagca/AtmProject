...
/**
 * Connection that opens two connections, one two a replication source, and another to one or more
 * replicas, and decides to use source when the connection is not read-only, and use replica(s) when
...
public class ReplicationConnectionProxy extends MultiHostConnectionProxy implements PingTarget {
   ...
   */
  public static ReplicationConnection createProxyInstance(ConnectionUrl connectionUrl)
      throws SQLException {
    ...
    return (ReplicationConnection)
        java.lang.reflect.Proxy.newProxyInstance(
            ...
            new Class<?>[] {ReplicationConnection.class, JdbcConnection.class},
            connProxy);
  }

  /**
   * Creates a proxy for java.sql.Connection that routes requests to a load-balanced connection of
   * source servers or a load-balanced connection of replica servers. Each sub-connection is created
   * with its own set of independent properties.
   *
   * @param connectionUrl The connection URL containing the hosts in a replication setup.
   * @throws SQLException if an error occurs
   */
  private ReplicationConnectionProxy(ConnectionUrl connectionUrl) throws SQLException {
    ...
    try {
      ...
      throw SQLError.createSQLException(
          Messages.getString(
              "MultihostConnection.badValueForHaEnableJMX", new Object[] {enableJMXAsString}),
          MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
          null);
    }

    String allowSourceDownConnectionsAsString =
        props.getProperty(PropertyKey.allowSourceDownConnections.getKeyName(), "false");
    try {
      this.allowSourceDownConnections = Boolean.parseBoolean(allowSourceDownConnectionsAsString);
    } catch (Exception e) {
      throw SQLError.createSQLException(
          Messages.getString(
              "ReplicationConnectionProxy.badValueForAllowSourceDownConnections",
              new Object[] {enableJMXAsString}),
          MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
          null);
    }

    String allowReplicaDownConnectionsAsString =
        props.getProperty(PropertyKey.allowReplicaDownConnections.getKeyName(), "false");
    try {
      this.allowReplicaDownConnections = Boolean.parseBoolean(allowReplicaDownConnectionsAsString);
    } catch (Exception e) {
      throw SQLError.createSQLException(
          Messages.getString(
              "ReplicationConnectionProxy.badValueForAllowReplicaDownConnections",
              new Object[] {allowReplicaDownConnectionsAsString}),
          MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
          null);
    }

    String readFromSourceWhenNoReplicasAsString =
        props.getProperty(PropertyKey.readFromSourceWhenNoReplicas.getKeyName());
    try {
      this.readFromSourceWhenNoReplicasOriginal =
          Boolean.parseBoolean(readFromSourceWhenNoReplicasAsString);

    } catch (Exception e) {
      throw SQLError.createSQLException(
          Messages.getString(
              "ReplicationConnectionProxy.badValueForReadFromSourceWhenNoReplicas",
              new Object[] {readFromSourceWhenNoReplicasAsString}),
          MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
          null);
    }
    ...
    if (!isNullOrEmpty(group)
        && ReplicationConnectionUrl.class.isAssignableFrom(connectionUrl.getClass())) {
      ...
      this.connectionGroupID =
          this.connectionGroup.registerReplicationConnection(
              this.thisAsReplicationConnection,
      ...
      this.sourceHosts =
          ((ReplicationConnectionUrl) connectionUrl)
              .getSourceHostsListFromHostPortPairs(this.connectionGroup.getSourceHosts());
      this.replicaHosts =
          ((ReplicationConnectionUrl) connectionUrl)
              .getReplicaHostsListFromHostPortPairs(this.connectionGroup.getReplicaHosts());
    } else {
    ...
    // Initialize replicas connection first so that it is ready to be used in case the sources
    // connection fails and 'allowSourceDownConnections=true'.
    try {
    ...
    if (this.currentConnection == null) {
      if (this.allowSourceDownConnections && this.replicasConnection != null) {
        ...
        throw SQLError.createSQLException(
            Messages.getString("ReplicationConnectionProxy.initializationWithEmptyHostsLists"),
            MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT,
            null);
      }
    }
  ...
  void syncSessionState(JdbcConnection source, JdbcConnection target, boolean readonly)
    ...
    try {
      ...
      try {
        // Try again. It may happen that the connection had recovered in the meantime but the right
        // syncing wasn't done yet.
        super.syncSessionState(source, target, readonly);
      } catch (SQLException e2) {
      }
      // Swallow both exceptions. Replication connections must continue to "work" after swapping
  ...
  /**
   * Proxies method invocation on the java.sql.Connection interface. This is the continuation of
        ...
        if (invokeAgain) {
          invokeAgain = false;
            ...
            && ((SQLException) e.getCause()).getErrorCode()
                == MysqlErrorNumbers.ERROR_CODE_NULL_LOAD_BALANCED_CONNECTION) {
          try {
  ...
  /**
   * Checks if this connection is in a state capable to invoke the provided method. If the
   * connection is in an inconsistent state, i.e. it has no hosts for both sub-connections, then
   * throw an invalid transaction state exception. Nevertheless, the methods defined in the
   * ReplicationConnection interface will be allowed as they are the only way to leave from an empty
   * hosts lists situation.
   *
   * @param method method
  ...
  private void checkConnectionCapabilityForMethod(Method method) throws Throwable {
    if (this.sourceHosts.isEmpty()
        && this.replicaHosts.isEmpty()
        && !ReplicationConnection.class.isAssignableFrom(method.getDeclaringClass())) {
      throw SQLError.createSQLException(
          Messages.getString("ReplicationConnectionProxy.noHostsInconsistentState"),
          MysqlErrorNumbers.SQL_STATE_INVALID_TRANSACTION_STATE,
          MysqlErrorNumbers.ERROR_CODE_REPLICATION_CONNECTION_WITH_NO_HOSTS,
          true,
          null);
    }
  }
  ...
  public void doPing() throws SQLException {
    ...
    } else if (!isSourceConn
        && (replicasPingException != null || this.replicasConnection == null)) {
      // Switch to sources connection, setting read-only state, if
      // 'readFromSourceWhenNoReplicas=true'.
      if (this.sourceConnection != null
          && this.readFromSourceWhenNoReplicas
  ...
  private JdbcConnection initializeSourceConnection() throws SQLException {
    ...
    LoadBalancedConnection newSourceConn =
        LoadBalancedConnectionProxy.createProxyInstance(
            new LoadBalanceConnectionUrl(
                this.sourceHosts, this.connectionUrl.getOriginalProperties()));
  ...
  private JdbcConnection initializeReplicasConnection() throws SQLException {
    ...
    LoadBalancedConnection newReplicasConn =
        LoadBalancedConnectionProxy.createProxyInstance(
            new LoadBalanceConnectionUrl(
                this.replicaHosts, this.connectionUrl.getOriginalProperties()));
  ...
  private boolean switchToReplicasConnectionIfNecessary() throws SQLException {
    ...
    // - If we're currently on the source and in read-only mode - we didn't have any replicas to use
    // until now. Or,
    // - If we're currently on a closed source connection and there are no sources to connect to.
    // Or,
    // - If we're currently not on a source connection that is closed - means that we were on a
    // closed replicas connection before it was re-initialized.
    if (this.currentConnection == null
        || isSourceConnection()
            && (this.readOnly || this.sourceHosts.isEmpty() && this.currentConnection.isClosed())
  ...
  public synchronized void removeSourceHost(String hostPortPair, boolean waitUntilNotInUse)
      throws SQLException {
   ...
   */
  @Deprecated
  public synchronized void removeMasterHost(String hostPortPair, boolean waitUntilNotInUse)
      throws SQLException {
    removeSourceHost(hostPortPair, waitUntilNotInUse);
  }

  public synchronized void removeSourceHost(
      String hostPortPair, boolean waitUntilNotInUse, boolean isNowReplica) throws SQLException {
    ...
    // The source connection may have been implicitly closed by a previous op., don't let it stop
    // us.
    if (this.sourceConnection == null || this.sourceConnection.isClosed()) {
      this.sourceConnection = null;
      return;
   ...
   */
  @Deprecated
  public synchronized void removeMasterHost(
      String hostPortPair, boolean waitUntilNotInUse, boolean isNowReplica) throws SQLException {
  ...
  public boolean isHostSource(String hostPortPair) {
    ...
    return this.sourceHosts.stream()
        .anyMatch(hi -> hostPortPair.equalsIgnoreCase(hi.getHostPortPair()));
  }
  ...
  public synchronized void removeReplica(String hostPortPair, boolean closeGently)
      throws SQLException {
    HostInfo host = getReplicaHost(hostPortPair);
    if (host == null) {
      return;
   ...
   */
  @Deprecated
  public synchronized void removeSlave(String hostPortPair, boolean closeGently)
      throws SQLException {
  ...
  public boolean isHostReplica(String hostPortPair) {
    ...
    return this.replicaHosts.stream()
        .anyMatch(hi -> hostPortPair.equalsIgnoreCase(hi.getHostPortPair()));
  }
  ...
  private void resetReadFromSourceWhenNoReplicas() {
    this.readFromSourceWhenNoReplicas =
        this.replicaHosts.isEmpty() || this.readFromSourceWhenNoReplicasOriginal;
  }

  private HostInfo getSourceHost(String hostPortPair) {
    return this.sourceHosts.stream()
        .filter(hi -> hostPortPair.equalsIgnoreCase(hi.getHostPortPair()))
        .findFirst()
        .orElse(null);
  }
