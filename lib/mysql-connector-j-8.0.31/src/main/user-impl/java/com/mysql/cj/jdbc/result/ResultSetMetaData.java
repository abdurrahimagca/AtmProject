...
public class ResultSetMetaData implements java.sql.ResultSetMetaData {
  ...
  public String getColumnCharacterSet(int column) throws SQLException {
    return this.session
        .getServerSession()
        .getCharsetSettings()
        .getMysqlCharsetNameForCollationIndex(getField(column).getCollationIndex());
  }
  ...
  protected Field getField(int columnIndex) throws SQLException {
    if ((columnIndex < 1) || (columnIndex > this.fields.length)) {
      throw SQLError.createSQLException(
          Messages.getString("ResultSetMetaData.46"),
  ...
  public int getPrecision(int column) throws SQLException {
    ...
    switch (f.getMysqlType()) {
      ...
      case MEDIUMBLOB:
      case LONGBLOB:
        // for binary BLOBS returning the full length, *TEXT types are handled by default clause
        // below
        return clampedGetLength(f);

      default:
        return f.getMysqlType().isDecimal()
            ? clampedGetLength(f)
            : clampedGetLength(f)
                / this.session
                    .getServerSession()
  ...
  public String getTableName(int column) throws SQLException {
    String res =
        this.useOldAliasBehavior
            ? getField(column).getTableName()
            : getField(column).getOriginalTableName();
    return res == null ? "" : res;
  }
      ...
      case SET:
        String collationName =
            this.session
                .getServerSession()
                .getCharsetSettings()
                .getCollationNameForCollationIndex(field.getCollationIndex());
