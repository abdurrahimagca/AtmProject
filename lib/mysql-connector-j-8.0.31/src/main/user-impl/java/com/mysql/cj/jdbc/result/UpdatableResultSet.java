...
public class UpdatableResultSet extends ResultSetImpl {
  ...
  public UpdatableResultSet(ResultsetRows tuples, JdbcConnection conn, StatementImpl creatorStmt)
    ...
    this.charEncoding =
        this.session.getPropertySet().getStringProperty(PropertyKey.characterEncoding).getValue();
    this.populateInserterWithDefaultValues =
        this.getSession()
            ...
            .getValue();
    this.pedantic =
        this.getSession().getPropertySet().getBooleanProperty(PropertyKey.pedantic).getValue();
  ...
  public void checkUpdatability() throws SQLException {
    try {
      if (this.getMetadata() == null) {
        // we've been created to be populated with cached metadata, and we don't have the metadata
        // yet, we'll be called again by
      ...
      // We can only do this if we know that there is a currently selected database, or if we're
      // talking to a > 4.1 version of MySQL server (as it returns
      ...
      if ((this.db == null) || (this.db.length() == 0)) {
        ...
        if ((this.db == null) || (this.db.length() == 0)) {
          throw SQLError.createSQLException(
              Messages.getString("UpdatableResultSet.43"),
      ...
      if (this.getSession()
          .getPropertySet()
          .getBooleanProperty(PropertyKey.strictUpdates)
          .getValue()) {
              ...
              if (originalName != null) {
                if (primaryKeyNames.remove(originalName.toUpperCase()) == null) {
                  ...
                  this.notUpdatableReason =
                      Messages.getString("NotUpdatableReason.6", new Object[] {originalName});
        ...
        if (!this.isUpdatable) {
          this.notUpdatableReason =
              existingPrimaryKeysCount > 1
                  ? Messages.getString("NotUpdatableReason.7")
  ...
  public void deleteRow() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (this.onInsertRow) {
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.1"), getExceptionInterceptor());
      } else if (this.rowData.size() == 0) {
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.2"), getExceptionInterceptor());
      } else if (isBeforeFirst()) {
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.3"), getExceptionInterceptor());
      } else if (isAfterLast()) {
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.4"), getExceptionInterceptor());
      }
      ...
      if (this.deleter == null) {
        ...
        this.deleter =
            (ClientPreparedStatement) this.connection.clientPrepareStatement(this.deleteSQL);
      }
      ...
      for (int i = 0; i < numKeys; i++) {
        ...
        this.setParamValue(
            this.deleter, i + 1, this.thisRow, index, this.getMetadata().getFields()[index]);
      }
  ...
  private void setParamValue(ClientPreparedStatement ps, int psIdx, Row row, int rsIdx, Field field)
    ...
    switch (field.getMysqlType()) {
      ...
      case TIMESTAMP:
        ps.setObject(
            psIdx, getObject(rsIdx + 1, Timestamp.class), MysqlType.TIMESTAMP, field.getDecimals());
        ps.getQueryBindings().getBinding(psIdx - 1, false).setKeepOrigNanos(true);
        break;
      case DATETIME:
        ps.setObject(
            psIdx,
            getObject(rsIdx + 1, LocalDateTime.class),
            MysqlType.DATETIME,
            field.getDecimals());
        ps.getQueryBindings().getBinding(psIdx - 1, false).setKeepOrigNanos(true);
        break;
  ...
  private void extractDefaultValues() throws SQLException {
    ...
    for (Map.Entry<String, Map<String, Map<String, Integer>>> dbEntry :
        this.databasesUsedToTablesUsed.entrySet()) {
  ...
  protected void generateStatements() throws SQLException {
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      if (fields[i].getOriginalTableName() != null) {
        ...
        String fqTableName =
            StringUtils.getFullyQualifiedName(databaseName, tableOnlyName, quotedId, this.pedantic);
      ...
      String columnName =
          this.hasLongColumnInfo && originalColumnName != null && originalColumnName.length() > 0
              ? originalColumnName
              : fields[i].getName();
      ...
      String tableName =
          this.hasLongColumnInfo && originalTableName != null && originalTableName.length() > 0
              ? originalTableName
          ...
          new StringBuilder() //
              .append(
                  StringUtils.getFullyQualifiedName(
                      databaseName, tableName, quotedId, this.pedantic)) //
              .append('.') //
              .append(StringUtils.quoteIdentifier(columnName, quotedId, this.pedantic))
              .toString();
    ...
    this.refreshSQL =
        "SELECT "
            ...
            + keyValues.toString();
    this.deleteSQL =
        "DELETE FROM " + this.qualifiedAndQuotedTableName + " WHERE " + keyValues.toString();
  }

  private Map<String, Integer> getColumnsToIndexMapForTableAndDB(
      String databaseName, String tableName) {
    Map<String, Integer> nameToIndex;
    Map<String, Map<String, Integer>> tablesUsedToColumnsMap =
        this.databasesUsedToTablesUsed.get(databaseName);

    if (tablesUsedToColumnsMap == null) {
      tablesUsedToColumnsMap =
          this.session.getServerSession().isLowerCaseTableNames()
  ...
  public void insertRow() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if (!this.onInsertRow) {
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.7"), getExceptionInterceptor());
      }
      ...
      for (int i = 0; i < fields.length; i++) {
        ...
        // WARN: This non-variant only holds if MySQL never allows more than one auto-increment key
        // (which is the way it is _today_)
      ...
      // inserter is always a client-side prepared statement, so it's safe to use it with
      // ByteArrayRow for server-side prepared statement too
  ...
  public void moveToInsertRow() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (this.inserter == null) {
        ...
        this.inserter =
            (ClientPreparedStatement) this.getConnection().clientPrepareStatement(this.insertSQL);
      ...
      for (int i = 0; i < numFields; i++) {
        if (!this.populateInserterWithDefaultValues) {
          ...
          if (this.defaultColumnValue[i] != null) {
            ...
            System.arraycopy(
                this.defaultColumnValue[i], 0, defaultValueCopy, 0, defaultValueCopy.length);
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (this.useUsageAdvisor) {
        if ((this.deleter == null)
            ...
            && (this.refresher == null)
            && (this.updater == null)) {
          this.eventSink.processEvent(
              ProfilerEvent.TYPE_USAGE,
              this.session,
              this.getOwningStatement(),
              this,
              0,
              new Throwable(),
  ...
  public void refreshRow() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (this.onInsertRow) {
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.8"), getExceptionInterceptor());
      } else if (this.rowData.size() == 0) {
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.9"), getExceptionInterceptor());
      } else if (isBeforeFirst()) {
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.10"), getExceptionInterceptor());
      } else if (isAfterLast()) {
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.11"), getExceptionInterceptor());
      }
  ...
  private void refreshRow(ClientPreparedStatement updateInsertStmt, Row rowToRefresh)
    ...
    if (this.refresher == null) {
      ...
      // We're going to copy bytes from refresher results to rowToRefresh, thus we need them to have
      // the same protocol encoding

      this.refresher =
          ((ResultsetRow) this.thisRow).isBinaryEncoded()
              ? (ClientPreparedStatement)
                  this.getConnection().serverPrepareStatement(this.refreshSQL)
              : (ClientPreparedStatement)
                  this.getConnection().clientPrepareStatement(this.refreshSQL);
    ...
    for (int i = 0; i < numKeys; i++) {
      ...
      if (!this.doingUpdates && !this.onInsertRow) {
        this.setParamValue(
            this.refresher, i + 1, this.thisRow, index, this.getMetadata().getFields()[index]);
        continue;
      }
      ...
      this.refresher
          .getQueryBindings()
          .setFromBindValue(i, updateInsertStmt.getQueryBindings().getBindValues()[index]);
    }
    ...
    try {
      ...
      if (rs.next()) {
        ...
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.12"),
  ...
  public void setResultSetConcurrency(int concurrencyFlag) {
    ...
    // TODO: FIXME: Issue warning when asked for updatable result set, but result set is not
    // updatable
    //
  ...
  /**
   * Reset UPDATE prepared statement to value in current row. This_Row MUST point to current, valid
   * row.
   *
   * @throws SQLException if an error occurs
   */
  protected void syncUpdate() throws SQLException {
    if (this.updater == null) {
      ...
      this.updater =
          (ClientPreparedStatement) this.getConnection().clientPrepareStatement(this.updateSQL);
    ...
    for (int i = 0; i < numFields; i++) {
      if (this.thisRow.getBytes(i) != null) {
        switch (fields[i].getMysqlType()) {
          ...
          case TIME:
          case TIMESTAMP:
            // TODO this is a temporary workaround until Bug#71143 "Calling ResultSet.updateRow
            // should not set all field values in UPDATE" is fixed.
            // We handle these types separately to avoid fractional seconds truncation (when
            // sendFractionalSeconds=true)
            // that happens for fields we don't touch with ResultSet.updateNN(). For those fields we
            // should pass the value as is or,
  ...
  public void updateRow() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (this.doingUpdates) {
        ...
        throw SQLError.createSQLException(
            Messages.getString("UpdatableResultSet.44"), getExceptionInterceptor());
      }
  ...
  public void updateBigDecimal(int columnIndex, BigDecimal x) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if (!this.onInsertRow) {
        ...
        this.thisRow.setBytes(
            columnIndex - 1, x == null ? null : StringUtils.getBytes(x.toString()));
      }
    }
  }
  ...
  /**
   * Internal setObject implementation. Although targetType is not part of default ResultSet methods
   * signatures, it is used for type conversions from JDBC42UpdatableResultSet new JDBC 4.2
   ...
   * @throws SQLException if an error occurs
   */
  protected void updateObjectInternal(
      int columnIndex, Object x, Integer targetType, int scaleOrLength) throws SQLException {
    try {
      ...
      throw SQLError.createSQLFeatureNotSupportedException(
          Messages.getString("Statement.UnsupportedSQLType") + JDBCType.valueOf(targetType),
   ...
   */
  protected void updateObjectInternal(
      int columnIndex, Object x, SQLType targetType, int scaleOrLength) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if (!this.onInsertRow) {
        if (!this.doingUpdates) {
          this.doingUpdates = true;
  ...
  public void updateString(int columnIndex, String x) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      if (!this.onInsertRow) {
        ...
        this.thisRow.setBytes(
            columnIndex - 1, x == null ? null : StringUtils.getBytes(x, this.charEncoding));
      }
    }
  }
  ...
  public void updateNString(int columnIndex, String x) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (!this.onInsertRow) {
        ...
        this.thisRow.setBytes(
            columnIndex - 1, x == null ? null : StringUtils.getBytes(x, fieldEncoding));
      }
    }
  }
