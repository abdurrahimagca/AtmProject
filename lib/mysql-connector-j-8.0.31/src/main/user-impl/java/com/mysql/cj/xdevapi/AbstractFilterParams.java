...
public abstract class AbstractFilterParams implements FilterParams {
   ...
   */
  public AbstractFilterParams(
      String schemaName, String collectionName, boolean supportsOffset, boolean isRelational) {
  ...
  public void setOrder(String... orderExpression) {
    ...
    this.order =
        new ExprParser(
                Arrays.stream(orderExpression).collect(Collectors.joining(", ")), this.isRelational)
            .parseOrderSpec();
  }
  ...
  public void verifyAllArgsBound() {
    if (this.args != null) {
      IntStream.range(0, this.args.length)
          ...
          .mapToObj(
              i ->
                  this.placeholderNameToPosition.entrySet().stream()
  ...
  public void setGrouping(String... groupBy) {
    ...
    this.grouping =
        new ExprParser(Arrays.stream(groupBy).collect(Collectors.joining(", ")), isRelational())
            .parseExprList();
  }
