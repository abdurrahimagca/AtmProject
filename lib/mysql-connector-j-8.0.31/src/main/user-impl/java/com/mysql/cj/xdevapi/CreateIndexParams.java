...
public class CreateIndexParams {
   ...
   * @param indexDefinition special JSON document containing index definition; see {@link
   ...
   *
   * @param indexName index name
   * @param jsonIndexDefinition special JSON document containing index definition; see {@link
   *     Collection#createIndex(String, String)} description
   */
  public CreateIndexParams(String indexName, String jsonIndexDefinition) {
    if (jsonIndexDefinition == null || jsonIndexDefinition.trim().length() == 0) {
      throw new XDevAPIError(
          Messages.getString("CreateIndexParams.0", new String[] {"jsonIndexDefinition"}));
    }
    try {
  ...
  private void init(String idxName, DbDoc indexDefinition) {
    ...
    if (indexDefinition == null) {
      throw new XDevAPIError(
          Messages.getString("CreateIndexParams.0", new String[] {"indexDefinition"}));
    }
  ...
  /**
   * Internally used object parsed from indexDefinition; see {@link Collection#createIndex(String,
   * DbDoc)} description.
   */
  public static class IndexField {
    ...
    /**
     * One of the supported SQL column types to map the field into. For numeric types, the optional
     * UNSIGNED keyword may follow. For the TEXT type, the length to consider for indexing may be
     * added.
     */
    private String type;

    /**
     * (optional) true if the field is required to exist in the document. defaults to false, except
     * for GEOJSON where it defaults to true *
     */
     ...
     * Constructor.
     *
     * @param indexField a special JSON document, part of indexDefinition document, consisting of
     ...
     *           numeric types, the optional UNSIGNED keyword may follow. For the TEXT type, the
     *           length to consider for indexing may be added. Type descriptions are case
     *           insensitive.
    ...
    public IndexField(DbDoc indexField) {
      ...
      if (val != null) {
        if (val instanceof JsonString) {
          ...
          // we're waiting for clarification about whether we set some default on client side in
          // that case, eg.:
      ...
      if (val != null) {
        if (GEOJSON.equalsIgnoreCase(this.type)) {
          ...
          throw new XDevAPIError(
              "Index field 'options' member should not be used for field types other than"
                  + " GEOJSON.");
        }
      }
