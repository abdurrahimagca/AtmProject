...
public class ExprParser {
  ...
  public ExprParser(String s, boolean allowRelationalColumns) {
    this.string = s;
    lex();
    // java.util.stream.IntStream.range(0, this.tokens.size()).forEach(i -> System.err.println("[" +
    // i + "] = " + this.tokens.get(i)));
  ...
  static class Token {
    ...
    public String toString() {
      if (this.type == TokenType.IDENT
          || this.type == TokenType.LNUM_INT
          || this.type == TokenType.LNUM_DOUBLE
          || this.type == TokenType.LSTRING) {
  ...
  /**
   * Helper function to match integer or floating point numbers. This function should be called when
   * the position is on the first character of the number (a digit or '.').
   *
   * @param i The current position in the string
      ...
      if (Character.isWhitespace(c)) {
        ...
        switch (c) {
          ...
          case '.':
            ...
            } else if (i + 1 < this.string.length()
                && Character.isDigit(this.string.charAt(i + 1))) {
              i = lexNumber(i);
            } else {
          ...
          case '`':
            ...
            try {
              ...
              for (c = this.string.charAt(++i);
                  c != quoteChar
                      || escapeNextChar
                      || (i + 1 < this.string.length() && this.string.charAt(i + 1) == quoteChar);
                  c = this.string.charAt(++i)) {
                if (escapeNextChar) {
                  if (escapeChars.containsKey(c)) {
                    ...
                    try {
                      ...
                      throw new WrongArgumentException(
                          "Invalid Unicode code point '" + hexCodePoint + "'");
                    }
                    i += 3;
                  } else {
                    val.append('\\').append(c);
            ...
            this.tokens.add(
                new Token(quoteChar == '`' ? TokenType.IDENT : TokenType.LSTRING, val.toString()));
            break;
          default:
        ...
        for (;
            i < this.string.length() && Character.isUnicodeIdentifierPart(this.string.charAt(i));
            ++i) {}
  ...
  void assertTokenAt(int pos, TokenType type) {
    if (this.tokens.size() <= pos) {
      throw new WrongArgumentException(
          "No more tokens when expecting " + type + " at token position " + pos);
    }
  ...
  DocumentPathItem docPathMember() {
    ...
    if (currentTokenTypeEquals(TokenType.IDENT)) {
      ...
      if (!t.value.equals(ExprUnparser.quoteIdentifier(t.value))) {
        throw new WrongArgumentException(
            "'" + t.value + "' is not a valid JSON/ECMAScript identifier");
      }
      consumeToken(TokenType.IDENT);
      memberName = t.value;
      ...
      throw new WrongArgumentException(
          "Expected token type IDENT or LSTRING in JSON path at token position " + this.tokenPos);
    }
  ...
  DocumentPathItem docPathArrayLoc() {
    ...
    if (currentTokenTypeEquals(TokenType.STAR)) {
      ...
      throw new WrongArgumentException(
          "Expected token type STAR or LNUM_INT in JSON path array index at token position "
              + this.tokenPos);
    }
  }
  ...
  public List<DocumentPathItem> documentPath() {
    ...
    while (true) {
      if (currentTokenTypeEquals(TokenType.DOT)) {
        ...
        items.add(
            DocumentPathItem.newBuilder().setType(DocumentPathItem.Type.MEMBER_ASTERISK).build());
        ...
        items.add(
            DocumentPathItem.newBuilder().setType(DocumentPathItem.Type.DOUBLE_ASTERISK).build());
      } else {
        break;
      }
    }
    if (items.size() > 0
        && items.get(items.size() - 1).getType() == DocumentPathItem.Type.DOUBLE_ASTERISK) {
  ...
  public Expr documentField() {
    ...
    if (currentTokenTypeEquals(TokenType.IDENT)) {
      builder.addDocumentPath(
          DocumentPathItem.newBuilder()
              .setType(DocumentPathItem.Type.MEMBER)
              .setValue(consumeToken(TokenType.IDENT))
              .build());
    }
  ...
  Expr atomicExpr() { // constant, identifier, variable, function call, etc
    if (this.tokenPos >= this.tokens.size()) {
      throw new WrongArgumentException(
          "No more tokens when expecting one at token position " + this.tokenPos);
    }
    Token t = this.tokens.get(this.tokenPos);
    ...
    switch (t.type) {
      ...
      case COLON:
        {
          ...
          if (currentTokenTypeEquals(TokenType.LNUM_INT)) {
            ...
            throw new WrongArgumentException(
                "Invalid placeholder name at token position " + this.tokenPos);
          }
      ...
      case CAST:
        {
          ...
          Operator.Builder builder =
              Operator.newBuilder().setName(TokenType.CAST.toString().toLowerCase());
          builder.addParam(expr());
          consumeToken(TokenType.AS);
          StringBuilder typeStr =
              new StringBuilder(this.tokens.get(this.tokenPos).value.toUpperCase());
          ...
          } else if (currentTokenTypeEquals(TokenType.CHAR)
              || currentTokenTypeEquals(TokenType.BINARY)) {
            this.tokenPos++;
            if (currentTokenTypeEquals(TokenType.LPAREN)) {
              typeStr.append(consumeToken(TokenType.LPAREN));
              typeStr.append(consumeToken(TokenType.LNUM_INT));
          ...
          } else if (currentTokenTypeEquals(TokenType.UNSIGNED)
              || currentTokenTypeEquals(TokenType.SIGNED)) {
            this.tokenPos++;
          ...
          } else if (currentTokenTypeEquals(TokenType.JSON)
              || currentTokenTypeEquals(TokenType.DATE)
              || currentTokenTypeEquals(TokenType.DATETIME)
              || currentTokenTypeEquals(TokenType.TIME)) {
            this.tokenPos++;
          } else {
            throw new WrongArgumentException(
                "Expected valid CAST type argument at " + this.tokenPos);
          }
          consumeToken(TokenType.RPAREN);
          // TODO charset?
      ...
      case MINUS:
        if (currentTokenTypeEquals(TokenType.LNUM_INT)
            || currentTokenTypeEquals(TokenType.LNUM_DOUBLE)) {
      ...
      case IDENT:
        ...
        if (nextTokenTypeEquals(TokenType.LPAREN)
            || (posTokenTypeEquals(this.tokenPos + 1, TokenType.DOT)
                && posTokenTypeEquals(this.tokenPos + 2, TokenType.IDENT)
                && posTokenTypeEquals(this.tokenPos + 3, TokenType.LPAREN))) {
          return functionCall();
        }
    ...
    throw new WrongArgumentException(
        "Cannot find atomic expression at token position " + (this.tokenPos - 1));
  }
  ...
  Expr parseLeftAssocBinaryOpExpr(TokenType[] types, ParseExpr innerParser) {
    ...
    while (this.tokenPos < this.tokens.size()
        && Arrays.asList(types).contains(this.tokens.get(this.tokenPos).type)) {
      Operator.Builder builder =
          Operator.newBuilder().setName(this.tokens.get(this.tokenPos).value).addParam(lhs);
      this.tokenPos++;
  ...
  Expr addSubIntervalExpr() {
    Expr lhs = atomicExpr();
    while ((currentTokenTypeEquals(TokenType.PLUS) || currentTokenTypeEquals(TokenType.MINUS))
        && nextTokenTypeEquals(TokenType.INTERVAL)) {
      ...
      if (currentTokenTypeEquals(TokenType.MICROSECOND)
          || currentTokenTypeEquals(TokenType.SECOND)
          || currentTokenTypeEquals(TokenType.MINUTE)
          || currentTokenTypeEquals(TokenType.HOUR)
          || currentTokenTypeEquals(TokenType.DAY)
          || currentTokenTypeEquals(TokenType.WEEK)
          || currentTokenTypeEquals(TokenType.MONTH)
          || currentTokenTypeEquals(TokenType.QUARTER)
          || currentTokenTypeEquals(TokenType.YEAR)
          || currentTokenTypeEquals(TokenType.SECOND_MICROSECOND)
          || currentTokenTypeEquals(TokenType.MINUTE_MICROSECOND)
          || currentTokenTypeEquals(TokenType.MINUTE_SECOND)
          || currentTokenTypeEquals(TokenType.HOUR_MICROSECOND)
          || currentTokenTypeEquals(TokenType.HOUR_SECOND)
          || currentTokenTypeEquals(TokenType.HOUR_MINUTE)
          || currentTokenTypeEquals(TokenType.DAY_MICROSECOND)
          || currentTokenTypeEquals(TokenType.DAY_SECOND)
          || currentTokenTypeEquals(TokenType.DAY_MINUTE)
          || currentTokenTypeEquals(TokenType.DAY_HOUR)
      ...
      // TODO: we need to propagate the appropriate encoding here? it's ascii but it might not
      // *always* be a superset encoding??
      builder.addParam(
          ExprUtil.buildLiteralScalar(
              this.tokens.get(this.tokenPos).value.toUpperCase().getBytes()));
      this.tokenPos++;
  ...
  Expr mulDivExpr() {
    return parseLeftAssocBinaryOpExpr(
        new TokenType[] {TokenType.STAR, TokenType.SLASH, TokenType.MOD}, this::addSubIntervalExpr);
  }

  Expr addSubExpr() {
    return parseLeftAssocBinaryOpExpr(
        new TokenType[] {TokenType.PLUS, TokenType.MINUS}, this::mulDivExpr);
  }

  Expr shiftExpr() {
    return parseLeftAssocBinaryOpExpr(
        new TokenType[] {TokenType.LSHIFT, TokenType.RSHIFT}, this::addSubExpr);
  }

  Expr bitExpr() {
    return parseLeftAssocBinaryOpExpr(
        new TokenType[] {TokenType.BITAND, TokenType.BITOR, TokenType.BITXOR}, this::shiftExpr);
  }
  ...
  Expr ilriExpr() {
    ...
    while (this.tokenPos < this.tokens.size()
        && expected.contains(this.tokens.get(this.tokenPos).type)) {
      boolean isNot = false;
      ...
      if (this.tokenPos < this.tokens.size()) {
        ...
        switch (this.tokens.get(this.tokenPos).type) {
          ...
          default:
            throw new WrongArgumentException(
                "Unknown token after NOT at position " + this.tokenPos);
        }
        if (isNot) {
          opName = "not_" + opName;
        }
  ...
  Expr andExpr() {
    return parseLeftAssocBinaryOpExpr(
        new TokenType[] {TokenType.AND, TokenType.ANDAND}, this::ilriExpr);
  }

  Expr orExpr() {
    return parseLeftAssocBinaryOpExpr(
        new TokenType[] {TokenType.OR, TokenType.OROR}, this::andExpr);
  }
  ...
  public Expr parse() {
    try {
      ...
      if (this.tokenPos != this.tokens.size()) {
        throw new WrongArgumentException(
            "Only " + this.tokenPos + " tokens consumed, out of " + this.tokens.size());
      }
      return e;
