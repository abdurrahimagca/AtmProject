...
public abstract class PreparableStatement<RES_T> {
  ...
  /**
   * Mark this preparable statement to be deallocated on next execution, if it is currently
   * prepared, or cancel the next prepare.
   */
  ...
  /**
   * Mark this preparable statement to be deallocated and re-prepared on next execution, if it is
   * currently prepared.
   */
   ...
   * Prepared statements are supported by the server. 2. The statement is executed repeatedly
    ...
    for (; ; ) {
      switch (this.preparedState) {
        ...
        case UNPREPARED:
          ...
          return result;
        case SUSPENDED:
          // An error occurred in some previous prepare. If the server doesn't support prepared
          // statements then mark it as unsupported, otherwise wait
          // until the server is ready to accept new prepare attempts, executing non-prepared in the
          // meantime.
          ...
          break;
        case PREPARE:
          // Prepare this statement. If it succeeds then immediately follow with its execution,
          // otherwise mark it as prepare suspended and let the
          // following iteration to decide what to do next.
          this.preparedState =
              prepareStatement() ? PreparedState.PREPARED : PreparedState.SUSPENDED;
          break;
        case PREPARED:
          // The statement is already prepared and can be executed safely.
          return executePreparedStatement();
        case DEALLOCATE:
          // Deallocate this statement and set it as unprepared so that it may be prepared later
          // again.
          deallocatePrepared();
          this.preparedState = PreparedState.UNPREPARED;
          break;
        case REPREPARE:
          // Deallocate this statement and set it as to prepare so that it gets prepared and
          // executed right away.
          deallocatePrepared();
          this.preparedState = PreparedState.PREPARE;
          break;
  ...
  /**
   * Executes the statement directly (non-prepared). Implementation is dependent on the statement
   * type.
   *
  ...
  /**
   * Returns the {@link XMessage} needed to prepare this statement. Implementation is dependent on
   * the statement type.
   *
   ...
   *
   * @return <code>true</code> if the statement was successfully prepared, <code>false</code>
   *     otherwise
   */
  ...
  /**
   * Executes a previously server-prepared statement. Implementation is dependent on the statement
   * type.
   *
   * @return the object returned from the lower level statement execution
   */
    ...
    if (this.preparedState == PreparedState.PREPARED
      ...
      try {
        this.mysqlxSession.query(
            getMessageBuilder().buildPrepareDeallocate(this.preparedStatementId),
  ...
  /**
   * {@link PhantomReference} to track prepared statement ids. An instance of this class must be
   * kept until the prepared statement is properly deallocated. If proper deallocation does not
   * happen, this is used to identify abandoned prepared statements and proceed with its
   * deallocation after the object is destructed by using a {@link ReferenceQueue}.
   */
  public static class PreparableStatementFinalizer
    ...
    public PreparableStatementFinalizer(
        PreparableStatement<?> referent,
        ReferenceQueue<? super PreparableStatement<?>> q,
