...
public class SchemaImpl implements Schema {
  ...
  public DbObjectStatus existsInDatabase() {
    ...
    return this.mysqlxSession.getDataStoreMetadata().schemaExists(this.name)
        ? DbObjectStatus.EXISTS
        : DbObjectStatus.NOT_EXISTS;
  }
  ...
  public List<Collection> getCollections(String pattern) {
    ...
    List<String> objectNames =
        this.mysqlxSession.query(
            this.xbuilder.buildListObjects(this.name, pattern),
            rowFiler,
            rowToName,
            Collectors.toList());
  ...
  public List<Table> getTables(String pattern) {
    Set<String> strTypes =
        Arrays.stream(
                new DbObjectType[] {
                  DbObjectType.TABLE, DbObjectType.VIEW, DbObjectType.COLLECTION_VIEW
                })
            .map(DatabaseObject.DbObjectType::toString)
            .collect(Collectors.toSet());
    Predicate<com.mysql.cj.result.Row> rowFiler = r -> (strTypes).contains(r.getValue(1, this.svf));
  ...
  public Collection createCollection(String collectionName) {
    this.mysqlxSession.query(
        this.xbuilder.buildCreateCollection(this.name, collectionName),
  ...
  public Collection createCollection(String collectionName, CreateCollectionOptions options) {
    try {
      this.mysqlxSession.query(
          this.xbuilder.buildCreateCollection(this.name, collectionName, options),
          new UpdateResultBuilder<>());
      return new CollectionImpl(this.mysqlxSession, this, collectionName);
    } catch (XProtocolError ex) {
      if (ex.getErrorCode() == MysqlErrorNumbers.ER_X_CMD_NUM_ARGUMENTS) {
        throw ExceptionFactory.createException(
            WrongArgumentException.class, Messages.getString("Schema.CreateCollection"), ex);
      }
      throw ex;
    }
  }
  ...
  public void modifyCollection(String collectionName, ModifyCollectionOptions options) {
    try {
      this.mysqlxSession.query(
          this.xbuilder.buildModifyCollectionOptions(this.name, collectionName, options),
          new UpdateResultBuilder<>());
    } catch (XProtocolError ex) {
  ...
  public void dropCollection(String collectionName) {
    try {
      this.mysqlxSession.query(
          this.xbuilder.buildDropCollection(this.name, collectionName),
          new UpdateResultBuilder<>());
