...
public class SqlMultiResult implements SqlResult, ResultStreamer {
   ...
   * Constructor.
   *
   * @param resultStream Supplies the result stream depending on query type. Could be {@link
   *     SqlSingleResult}, {@link SqlUpdateResult} etc.
   */
  ...
  public boolean nextResult() {
    ...
    try {
      ...
      // propagate any exception but clear the current result so we don't try to read any more
      // results
      this.currentResult = null;
    }
    this.currentResult =
        this.pendingResults.size() > 0 ? this.pendingResults.remove(0) : this.resultStream.get();
