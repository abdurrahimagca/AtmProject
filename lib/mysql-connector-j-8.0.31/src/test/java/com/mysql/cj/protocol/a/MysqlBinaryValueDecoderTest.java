...
public class MysqlBinaryValueDecoderTest {
  ...
  public void testInt8Limits() {
    ...
    byte[] signedInt8Min =
        ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN).putLong(Long.MIN_VALUE).array();
    decoded = this.valueDecoder.decodeInt8(signedInt8Min, 0, 8, vf);
    assertEquals(String.valueOf(Long.MIN_VALUE), decoded);

    byte[] signedInt8Max =
        ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN).putLong(Long.MAX_VALUE).array();
