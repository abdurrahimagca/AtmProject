...
public class SimplePacketSenderTest extends PacketSenderTestBase {
  ...
  private SimplePacketSender sender =
      new SimplePacketSender(new BufferedOutputStream(this.outputStream));
  ...
  public void splitPacketTest() throws IOException {
    ...
    for (int multiPackets = 2; multiPackets <= 4; ++multiPackets) {
      final int packetLen =
          ((multiPackets - 1) * NativeConstants.MAX_PACKET_SIZE) + leftoverPacketLen;
      ...
      int sizeOfAllPackets =
          (NativeConstants.HEADER_LENGTH * multiPackets)
      ...
      for (int i = 1; i <= multiPackets; ++i) {
        ...
        assertEquals(
            20 + i, sentPacket[offset + NativeConstants.HEADER_LENGTH + packetLenInHeader - 1]);
        packetSequence++;
  ...
  public void packetSizeMultipleOfMaxTest() throws IOException {
    ...
    for (int multiple = 1; multiple <= 3; ++multiple) {
      ...
      // size of ALL packets written to output stream
      int sizeOfAllPackets =
          (NativeConstants.HEADER_LENGTH * (multiple + 1))
