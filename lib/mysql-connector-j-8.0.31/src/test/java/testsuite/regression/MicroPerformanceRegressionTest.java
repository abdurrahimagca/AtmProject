...
public class MicroPerformanceRegressionTest extends BaseTestCase {
  ...
  private static final double ORIGINAL_LOOP_TIME_MS = 2300.0;

  // (Used to be 10.0 for JVM < 1.7 but since HW and VMs are much faster now a minimal disruption
  // can cause significant deviations)
  private static final double LEEWAY = 50.0; // account for VMs

  private static final Map<String, Double> BASELINE_TIMES = new HashMap<>();
  ...
  private synchronized void checkTime(String testType, double avgExecTimeMs) throws Exception {
    ...
    double acceptableTime =
        LEEWAY * baselineExecTimeMs.doubleValue() * scaleFactor * adjustForVendor;

    System.out.println(
        testType + ": avg time = " + avgExecTimeMs + ", acceptable time = " + acceptableTime);

    assertTrue(
        avgExecTimeMs <= acceptableTime,
        "Average execution time of "
            + avgExecTimeMs
            + " ms. exceeded baseline * leeway of "
            + acceptableTime
            + " ms.");
  }
  ...
  public void testResultSetAccessors() throws Exception {
    createTable(
        "marktest",
        "(intField INT, floatField DOUBLE, timeField TIME, datetimeField DATETIME, stringField"
            + " VARCHAR(64))");
    this.stmt.executeUpdate(
        "INSERT INTO marktest VALUES (123456789, 12345.6789, NOW(), NOW(),"
    ...
    for (int i = 0; i < numLoops; i++) {
      this.rs.getTime(3);
      // If we don't clear the warnings here, we add one for every loop and spend a huge amount of
      // time iterating and updating the linked list
  ...
  public void testPreparedStatementTimes() throws Exception {
    ...
    for (int i = 0; i < numPrepares; i++) {
      ...
      PreparedStatement pStmt =
          this.conn.prepareStatement("INSERT INTO test.marktest VALUES (?, ?, ?, ?, ?)");
      pStmt.close();
    }
    ...
    PreparedStatement pStmt =
        this.conn.prepareStatement("INSERT INTO marktest VALUES (?, ?, ?, ?, ?)");
  ...
  public void testBug6359() throws Exception {
    if (runLongTests()) {
      ...
      createTable(
          "testBug6359",
          "(pk_field INT PRIMARY KEY NOT NULL AUTO_INCREMENT, field1 INT, field2 INT, field3 INT,"
              + " field4 INT, field5 INT, field6 INT, field7 INT, field8 INT, field9 INT,  INDEX"
              + " (field1))");

      PreparedStatement pStmt =
          this.conn.prepareStatement(
              "INSERT INTO testBug6359 (field1, field2, field3, field4, field5, field6, field7,"
                  + " field8, field9) VALUES (?, 1, 2, 3, 4, 5, 6, 7, 8)");
      ...
      for (int i = 0; i < numSelects; i++) {
        this.rs =
            this.stmt.executeQuery("SELECT pk_field FROM testBug6359 WHERE field1 BETWEEN 1 AND 5");
      }
      ...
      logDebug(
          "time to execute " + numSelects + " single queries: " + secondsSingleQuery + " seconds");

      checkTime("single selects", secondsSingleQuery);

      PreparedStatement pStmt2 =
          this.conn.prepareStatement(
              "SELECT field2, field3, field4, field5 FROM testBug6359 WHERE pk_field=?");
