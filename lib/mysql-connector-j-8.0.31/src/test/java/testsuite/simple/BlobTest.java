...
public class BlobTest extends BaseTestCase {
  ...
  public void testByteStreamInsert() throws Exception {
    ...
    assumeTrue(
        this.rs.getInt(2) >= len,
        "You need to increase max_allowed_packet to at least "
            + (len)
    ...
    if (versionMeetsMinimum(5, 6, 20) && !versionMeetsMinimum(5, 7)) {
      ...
      this.rs.next();
      assumeFalse(
          this.rs.getInt(2) < 10 * testBlobFile.length(),
          "You need to increase innodb_log_file_size to at least "
              + (10 * testBlobFile.length())
              + " before running this test!");
    }
  ...
  private boolean checkBlob(byte[] retrBytes) throws Exception {
    ...
    try {
      ...
      if (retrBytes.length == fileLength) {
        ...
        passed = false;
        System.out.println(
            "retrBytes.length(" + retrBytes.length + ") != testBlob.length(" + fileLength + ")");
      }
  ...
  private void doRetrieval() throws Exception {
    ...
    passed = checkBlob(retrBytes);
    assertTrue(
        passed, "Inserted BLOB data did not match retrieved BLOB data for getBinaryStream().");
    ...
    passed = checkBlob(retrBytes);
    assertTrue(
        passed, "Inserted BLOB data did not match retrieved BLOB data for getAsciiStream().");
    ...
    passed = checkBlob(retrBytes);
    assertTrue(
        passed, "Inserted BLOB data did not match retrieved BLOB data for getUnicodeStream().");
  }

  private static final String TEST_BLOB_FILE_PREFIX = "cmj-testblob";
