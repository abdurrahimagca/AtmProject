...
public class UpdatabilityTest extends BaseTestCase {
  ...
  /**
   * Tests that the driver does not let you update result sets that come from tables that don't have
   * primary keys
   *
   * @throws SQLException
   */
  ...
  public void testMultiKeyTable() throws SQLException {
    ...
    this.stmt.executeUpdate(
        "CREATE TABLE MULTI_UPDATABLE (field1 int NOT NULL, field2 int NOT NULL, PRIMARY KEY"
            + " (field1, field2))");
    ...
    try {
      ...
      assertThrows(
          "ResultSet.moveToInsertRow() should not succeed on query that does not select all primary"
              + " keys",
          NotUpdatable.class,
          () -> {
            this.rs.moveToInsertRow();
  ...
  private void createTestTable() throws SQLException {
    ...
    this.stmt.executeUpdate(
        "CREATE TABLE UPDATABLE (pos1 int not null, pos2 int not null, char_field VARCHAR(32),"
            + " PRIMARY KEY (pos1, pos2))");

    for (int i = 0; i < 100; i++) {
      this.stmt.executeUpdate(
          "INSERT INTO UPDATABLE VALUES (" + i + ", " + i + ",'StringData" + i + "')");
    }
  }
}
