...
public class DevApiBaseTestCase extends InternalXBaseTestCase {
  ...
  protected void sqlUpdate(String sql) {
    try {
      ...
      mysqlxSession.query(
          mysqlxSession.getMessageBuilder().buildSqlStatement(sql), new UpdateResultBuilder<>());
    } catch (Exception e) {
      e.printStackTrace();
  ...
  int getPrepPrepareCount(Session sess) {
    return Integer.parseInt(
        sess.sql("SHOW SESSION STATUS LIKE 'mysqlx_prep_prepare'")
            .execute()
            .fetchOne()
            .getString(1));
  }
  ...
  int getPrepExecuteCount(Session sess) {
    return Integer.parseInt(
        sess.sql("SHOW SESSION STATUS LIKE 'mysqlx_prep_execute'")
            .execute()
            .fetchOne()
            .getString(1));
  }
  ...
  int getPrepDeallocateCount(Session sess) {
    return Integer.parseInt(
        sess.sql("SHOW SESSION STATUS LIKE 'mysqlx_prep_deallocate'")
            .execute()
            .fetchOne()
            .getString(1));
  }
  ...
  int getPreparedStatementsCount() {
    return this.session
        .sql("SELECT COUNT(*) FROM performance_schema.prepared_statements_instances")
        .execute()
        .fetchOne()
        .getInt(0);
  }
  ...
  int getPreparedStatementsCount(int threadId) {
    return this.session
        .sql(
            "SELECT COUNT(*) FROM performance_schema.prepared_statements_instances  WHERE"
                + " owner_thread_id = "
                + threadId)
        .execute()
        .fetchOne()
  ...
  int getPreparedStatementsCount(Session sess) {
    return sess.sql(
            "SELECT COUNT(*) FROM performance_schema.prepared_statements_instances psi INNER JOIN"
                + " performance_schema.threads t ON psi.owner_thread_id = t.thread_id WHERE"
  ...
  int getPreparedStatementExecutionsCount(Session sess, int prepStmtId) {
    SqlResult res =
        sess.sql(
                "SELECT psi.count_execute FROM performance_schema.prepared_statements_instances psi"
                    + " INNER JOIN performance_schema.threads t ON psi.owner_thread_id ="
                    + " t.thread_id WHERE t.processlist_id = connection_id() AND psi.statement_id ="
  ...
  protected boolean supportsTestCertificates(Session sess) {
    ...
    if (res.hasNext()) {
      Row r = res.next();
      return (StringUtils.isNullOrEmpty(r.getString(1)) && r.getString(2).contains("ssl-test-certs")
          || r.getString(1).contains("ssl-test-certs"));
    }
    return false;
  }
  ...
  protected void assertPreparedStatementsCountsAndId(
      Session sess,
  ...
  protected void assertPreparedStatementsStatusCounts(
      Session sess, int expectedPrep, int expectedExec, int expectedDealloc) {
