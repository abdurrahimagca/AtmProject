...
public class Ipv6SupportTest extends DevApiBaseTestCase {
  ...
  public void setupIpv6SupportTest() {
    ...
    if (setupTestSession()) {
      List<Inet6Address> ipv6List =
          isMysqlRunningLocally() ? TestUtils.getIpv6List() : TestUtils.getIpv6List(getTestHost());
      this.ipv6Addrs =
  ...
  public void testIpv6SupportInSession() {
    assumeTrue(
        ...
        "Server version 5.7.17 or higher is required.");

    // Although per specification IPv6 addresses must be enclosed by square brackets, we actually
    // support them directly.
    String[] urls =
        new String[] {
          "mysqlx://%s:%s@%s:%d",
          "mysqlx://%s:%s@[%s]:%d",
          "mysqlx://%s:%s@(address=%s:%d)",
          "mysqlx://%s:%s@(address=[%s]:%d)",
          "mysqlx://%s:%s@address=(host=%s)(port=%d)",
          "mysqlx://%s:%s@address=(host=[%s])(port=%d)"
        };
    ...
    for (String host : this.ipv6Addrs) {
      if (TestUtils.serverListening(host, port)) {
        ...
        for (String url : urls) {
          String ipv6Url =
              String.format(url, this.testUser, this.testUser, TestUtils.encodePercent(host), port);
    ...
    if (!atLeastOne) {
      String errMsg =
          "None of the tested hosts have server sockets listening on the port "
              + port
              + ". This test requires a MySQL server with X Protocol running in local host with"
              + " IPv6 support enabled (set '--mysqlx-bind-address = *' if needed.";
