...
public class MetadataTest extends BaseTableTestCase {
  ...
  public void setupTableTest() {
    if (this.isSetForXTests) {
      ...
      sqlUpdate(
          "create table example_metadata (_id varchar(32), name varchar(20), birthday date, age"
              + " int)");
    }
  }
  ...
  public void example_metadata() {
    ...
    if ("utf8mb4_0900_ai_ci".equals(this.dbCollation)) {
      assertEquals(
          128,
          idCol
              .getLength()); // TODO is it an xplugin bug after changing default charset to utf8mb4?
    } else {
  ...
  public void docAsTableIsJSON() {
    ...
    try {
      ...
      // Unlike ordinary tables, collections are always created in uft8mb4 charset, but collation
      // was changed in 8.0.1.
  ...
  /**
   * Some metadata fields have no sense with concrete SQL data type. The following table from {@link
   * ColumnMetaData} describes which fields are relevant to each type:
   *
   * <pre>
   *     ================= ============ ======= ========== ====== ========
  ...
  public void exhaustTypes() {
    ...
    try {
      ...
      sqlUpdate(
          "create table "
              + tableName
              + " (a bit, b char(20) not null, c int, d tinyint unsigned primary key, e bigint, f"
              + " double, g decimal(20, 3), h time, i datetime, j timestamp, k date, l"
              + " set('1','2'), m enum('1','2'), unique (a), key(b, c))");
      ...
      // assertEquals(0, c.getFractionalDigits()); // irrelevant, we shouldn't expect any concrete
      // value
      // assertEquals(false, c.isNumberSigned()); // irrelevant, we shouldn't expect any concrete
      // value
      // assertEquals(null, c.getCollationName()); // irrelevant, we shouldn't expect any concrete
      // value
      // assertEquals(null, c.getCharacterSetName()); // irrelevant, we shouldn't expect any
      // concrete value
      ...
      // assertEquals(false, c.isAutoIncrement()); // irrelevant, we shouldn't expect any concrete
      // value
      // assertEquals(false, c.isPrimaryKey()); // irrelevant, we shouldn't expect any concrete
      // value
      ...
      if ("utf8mb4_0900_ai_ci".equals(this.dbCollation)) {
        assertEquals(
            80,
            c.getLength()); // TODO is it an xplugin bug after changing default charset to utf8mb4?
      } else {
      ...
      assertEquals(true, c.isNullable()); // irrelevant, we shouldn't expect any concrete value
      assertEquals(
          false, c.isAutoIncrement()); // irrelevant, we shouldn't expect any concrete value
  ...
  public void testGetColumnInfoFromnSession() throws Exception {
    ...
    try {
      ...
      sqlUpdate(
          "create table xyz (i int auto_increment primary key,j bigint not null,k tinyint unsigned"
              + " unique,l char(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_spanish2_ci,m"
              + " decimal(16,10),key(l))");
      ...
      SqlResult sRes =
          this.session
              .sql("select * from  " + this.schema.getName() + ".xyz , qadatabase.xyz mytable")
              .execute();
  ...
  public void testGetType() throws Exception {
    ...
    try {
      sqlUpdate("drop table if exists qatable");
      sqlUpdate(
          "create table qatable (_id varchar(32), a char(20), b date, c int,d double,e datetime,f"
              + " time,g linestring,h tinyint,i mediumint,j bigint,k float, l set('1','2'), m"
              + " enum('1','2'),n decimal(20,10),o bit)");
  ...
  public void testIsNumberSigned() throws Exception {
    ...
    try {
      sqlUpdate("drop table if exists qatable");
      sqlUpdate(
          "create table qatable (_id varchar(32), a char(20), b date, c int,d double signed,e"
              + " datetime,f time,g linestring,h tinyint unsigned,i mediumint,j bigint unsigned,k"
              + " float, l set('1','2'), m enum('1','2'),n decimal(20,10),o bit)");
  ...
  public void testGetLength() throws Exception {
    ...
    try {
      ...
      sqlUpdate(
          "create table lengthTest.qatable (_id varchar(32), a char(20), b date, c int,d double,e"
              + " datetime,f time,g linestring,h tinyint,i mediumint,j bigint,k float, l"
              + " set('1','2'), m enum('1','2'),n decimal(20,10),o bit(3))");
  ...
  public void testIsPrimaryKeyAndisUniqueKeyAndisPartKey() throws Exception {
    ...
    try {
      sqlUpdate("drop table if exists qatable");
      sqlUpdate(
          "create table qatable (a int Not Null,b bigint unique,c char(30) Not Null unique,d"
              + " tinyint,e json ,f INT GENERATED ALWAYS AS (JSON_EXTRACT(e, '$.id')),g BIGINT"
              + " GENERATED ALWAYS AS (JSON_EXTRACT(e, '$.id2')) STORED NOT NULL"
              + " ,key(b,g,f),primary key(a,c,d), INDEX i (f))");

      table = this.schema.getTable("qatable");
      table
          .insert("a", "c", "d", "e")
          .values(1, "S", 10, "{\"id2\": \"12345677890123\", \"name\": \"Fred\"}")
          .execute();
      rows = table.select("*").execute();
      List<Column> metadata = rows.getColumns();
  ...
  public void testGetColumnNameAndgetColumnLabel() throws Exception {
    ...
    try {
      sqlUpdate("drop table if exists qatable");
      sqlUpdate(
          "create table qatable (a varchar(32), b bigint, c double, d int,`b+c+d`  DOUBLE AS"
              + " (c+b+d))");
      sqlUpdate(
      ...
      table
          .insert("a", "b", "c", "d")
          .values("AB", 12345677890123L, -12345677890123.9, 1)
          .execute();
      rows =
          table
              .select(
                  "a as col1, b as c, c as b,d as col1,d+1 as `sum()`, `b+c+d`,sum(c),count(*)/-1")
              .execute();
      List<Column> metadata = rows.getColumns();
  ...
  public void testMultiSelects() throws Exception {
    ...
    try {
      ...
      table1
          .insert("a", "b")
          .values("AB", 12345677890123L)
          .values("CD", 123456778901234L)
          .values("EF", 1234567789012345L)
          .execute();
      table2
          .insert("a1", "b1")
          .values(1234, 4321.123)
          .values(12345, 54321.123)
          .values(123456, 654321.123)
          .execute();
  ...
  public void testIsPaddedAndisNullableAndisAutoIncrement() throws Exception {
    ...
    try {
      sqlUpdate("drop table if exists qatable");
      sqlUpdate(
          "create table qatable (a int Not Null auto_increment,b char(10),c char(30) Not Null,d"
              + " bigint  zerofill not null,primary key(a))");
  ...
  public void testWithUnsignedData() throws Exception {
    ...
    try {
      ...
      sqlUpdate("drop table if exists qatable");
      sqlUpdate(
          "create table qatable (a int unsigned ,b bigint unsigned,c tinyint unsigned,d smallint"
              + " unsigned, e float unsigned,f double unsigned, g TEXT,h MEDIUMINT unsigned)");
